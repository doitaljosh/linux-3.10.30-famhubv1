 ...
 * module must be it must be within 32MB of the kernel text
 */
#define MODULES_START	ALIGN((unsigned long)_etext - SZ_32M, PAGE_SIZE)
#else
/* smaller range for Thumb-2 symbols relocation (2^24) */
#define MODULES_START	ALIGN((unsigned long)_etext - SZ_16M, PAGE_SIZE)
...
{
	return __vmalloc_node_range(size, 1, max(MODULES_VADDR, MODULES_START),
