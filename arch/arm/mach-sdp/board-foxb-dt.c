...
static char const *sdp1207_dt_compat[] __initdata = {
	"samsung,sdp1207",
	NULL
};
...
static __cpuinit int foxb_install_warp(unsigned int cpu)
{
	void __iomem *base;
	u32 instr_addr = 0x3C;
...
struct sdp_power_ops sdp1207_power_ops = {
	.powerup_cpu	= NULL,
	.powerdown_cpu	= NULL,
	.install_warp	= foxb_install_warp,
};

static void __init sdp1207_init_early(void)
{
...
static void __init foxb_map_io(void)
{
	...
	sdp_platsmp_init(&sdp1207_power_ops);
...
static void __init sdp1207_init_time(void)
{
#ifdef CONFIG_OF
	of_clk_init(NULL);
	clocksource_of_init();
...
DT_MACHINE_START(SDP1207_DT, "Samsung SDP1207(Flattened Device Tree)")
	/* Maintainer: */
	.smp		= smp_ops(sdp_smp_ops),
	.map_io		= foxb_map_io,
	.init_machine	= sdp_dt_init_machine,
	.init_early	= sdp1207_init_early,
	.init_time = sdp1207_init_time,
	.dt_compat	= sdp1207_dt_compat,
	.restart	= sdp_restart,
MACHINE_END
