...
static char const *sdp1202_dt_compat[] __initdata = {
	"samsung,sdp1202",
	NULL
};
...
static __cpuinit int foxp_install_warp(unsigned int cpu)
{
	void __iomem *base;
	u32 instr_addr = 0x3C;
...
struct sdp_power_ops sdp1202_power_ops = {
	.powerup_cpu	= NULL,
	.powerdown_cpu	= NULL,
	.install_warp	= foxp_install_warp,
};

static void __init sdp1202_init_early(void)
{
...
static void __init foxp_map_io(void)
{
	iotable_init(sdp1202_io_desc, ARRAY_SIZE(sdp1202_io_desc));
	sdp_platsmp_init(&sdp1202_power_ops);
...
static void __init sdp1202_init_time(void)
{
#ifdef CONFIG_OF
	of_clk_init(NULL);
	clocksource_of_init();
...
DT_MACHINE_START(sdp1202_DT, "Samsung sdp1202(Flattened Device Tree)")
	/* Maintainer: */
	.init_irq	= sdp_init_irq,
	.smp		= smp_ops(sdp_smp_ops),
	.map_io		= foxp_map_io,
	.init_machine	= sdp_dt_init_machine,
	.init_early	= sdp1202_init_early,
	.init_time = sdp1202_init_time,
	.dt_compat	= sdp1202_dt_compat,
	.restart	= sdp_restart,
MACHINE_END
