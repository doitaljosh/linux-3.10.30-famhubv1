...
#else
//#define MXT_DEV_NAME	"Atmel MXTXXXS" 	//original
#define MXT_DEV_NAME 	"sec_touchscreen" 	//hsguy.son (modify)
#endif
...
//#define MXT_DEFAULT_FIRMWARE_NAME	"20150306_D6A497.fw"
//#define MXT_DEFAULT_FIRMWARE_NAME	"20150306_DBD197.fw"
//#define MXT_DEFAULT_FIRMWARE_NAME	"20150306_E8BF17.fw"
//#define MXT_DEFAULT_FIRMWARE_NAME	"20150313_9B4054.fw"
//#define MXT_DEFAULT_FIRMWARE_NAME	"20151117_CBECBD.fw"
//#define MXT_DEFAULT_FIRMWARE_NAME	"20151126_575211.fw"
#define MXT_DEFAULT_FIRMWARE_NAME	"20151221_86DD03.fw"
...
#define MXT_RESET_INTEVAL_TIME	102	/* msec */

#define MXT_SW_RESET_TIME		200	/* 101.37 msec for mXT2952T2 */
#define MXT_HW_RESET_TIME		160	/* 82 msec for mXT2952T2 */
...
//#define MXT_BOOT_LOW		0x24 	//original
//#define MXT_BOOT_HIGH		0x27

#define MXT_BOOT_LOW		0x26 	//hsguy.son (add)
#define MXT_BOOT_HIGH		0x27
...
#define FAMILY_HUB 		1 			//hsguy.son (add)
#define MXT_TCH_I2C_ADDR 	0x4a 	//hsguy.son (add)
#define TSP_PRINT_COORDINATE
#define SYSFS_SAMPLE_RATE
#define FOR_BRINGUP 	0
#define DRIVER_LEVEL_SCALING 1

#define DISABLE 		0
#define ENABLE 			1

#define BUILTIN_FIRMWARE
//#define WATCHDOG_ENABLE
#define MXT_WATCHDOG_TIMEOUT 	3000
#define MXT_I2C_NAME 	"sec_touchscreen"
enum {
	BOOT_IMCOMPLETE = 0,
	BOOT_COMPLETE,
};
//static __u32 twi_id = 2; 		//hsguy.son (add)
...
struct mxt_platform_data {
	unsigned int num_xnode;
	unsigned int num_ynode;
	unsigned int max_x;
	unsigned int max_y;
	unsigned int gpio_irq;	//hsguy.son (add)
	unsigned int gpio_reset;	//hsguy.son (add)
	unsigned int sck;	//hsguy.son (add)
	unsigned int sda;	//hsguy.son (add)
	...
	int (*power_ctrl) (void *ddata, bool on); 	//hsguy.son (add)
...
struct atmel_dbg {
	...
	u8 tsp_print_coordinate; 	/* to represent x,y point coordinate */ 	//hsguy.son (add)
	u8 tsp_sampling_rate; 	/* to represent touch sampling rate */ 	//hsguy.son (add)
	u8 tsp_sleep_mode; 	/* to represent touch sleep mode */ 	//hsguy.son (add)
	u8 tsp_report_lock; 	/* to prevent report input event */ 	//hsguy.son (add)
};
#endif
 ...
 *  remove below boolean...*/
	...
	char phys[128]; 	//hsguy.son (add)
	wait_queue_head_t 			wait_q; 	//hsguy.son (add)
	struct work_struct 			watchdog_work; 	//hsguy.son (add)
	struct timer_list 			watchdog_timer; 	//hsguy.son (add)
	bool irq_state; 							//hsguy.son (add)
	bool watchdog_timer_flag; 				//hsguy.son (add)
#ifdef SYSFS_SAMPLE_RATE
	struct sample_rate_struct 		sample_rate; 	//hsguy.son (add)
#endif
};
