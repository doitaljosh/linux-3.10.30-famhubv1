...
static SINT32	WakeANACLK_PD(UINT8 *pbRegPD, UINT8 *pbAP);
...
)
{
	...
	if (sdRet == MCDRV_SUCCESS) {
		McResCtrl_GetAecInfo(&sAecInfo);
		bReg	= (sAecInfo.sAecVBox.bSrc3_Thru<<4)
				| sAecInfo.sAecVBox.bSrc3_Fs;
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
		...
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
				...
				sAecInfo.sAecVBox.bFdsp_Po_Source);

		bReg	= (UINT8)(sAecInfo.sAecAudioengine.bMixerIn3Src<<7)
			| (UINT8)(sAecInfo.sAecAudioengine.bMixerIn2Src<<6)
			| (UINT8)(sAecInfo.sAecAudioengine.bMixerIn1Src<<5)
			| (UINT8)(sAecInfo.sAecAudioengine.bMixerIn0Src<<4)
			| (UINT8)(sAecInfo.sAecAudioengine.bBDspAE1Src<<1)
			| sAecInfo.sAecAudioengine.bBDspAE0Src;
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
				| MCDRV_PACKET_REGTYPE_MA
				| (UINT32)MCI_BDSP_SOURCE,
				bReg);

		bReg	= (UINT8)(sAecInfo.sAecVBox.bLPt2_VSource<<5)
			| (UINT8)(sAecInfo.sAecVBox.bISrc3_VSource<<4)
			| (UINT8)(sAecInfo.sAecVBox.bISrc2_Ch1_VSource<<3)
			| (UINT8)(sAecInfo.sAecVBox.bISrc2_VSource<<2)
			| sAecInfo.sAecVBox.bSrc3_Ctrl;
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
				| MCDRV_PACKET_REGTYPE_MA
				| (UINT32)MCI_SRC_VSOURCE,
				bReg);
		bReg	= (UINT8)(sAecInfo.sAecVBox.bLPt2_Mix_VolO<<4)
			| sAecInfo.sAecVBox.bLPt2_Mix_VolI;
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
				| MCDRV_PACKET_REGTYPE_MA
 ...
 *			ANACLK_PD powerup.
 *	Arguments:
 *			pbRegPD	A_REG[2] state
 *			pbAP	ANA_REG[2] state
 *	Return:
 *			MCDRV_SUCCESS
 *			MCDRV_ERROR_RESOURCEOVER
 *			MCDRV_ERROR_TIMEOUT
		...
		if ((bStarted&MCDRV_DSP_START_C) == 0) {
			...
			if (sAecInfo.sAecVBox.bCDspFuncAOnOff != 0) {
				...
				&& (sAecInfo.sAecVBox.bCDspFuncAOnOff == 0x11))
					bReg	= sAecInfo.sAecVBox.bSrc3_Fs;
				else
					bReg	= bLP2Fs;
...
)
{
	...
	bReg	= (sInfo.sAecVBox.bSrc3_Thru<<4) | sInfo.sAecVBox.bSrc3_Fs;
	McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
			| MCDRV_PACKET_REGTYPE_MB
			| (UINT32)MCI_SRC3,
			bReg);
