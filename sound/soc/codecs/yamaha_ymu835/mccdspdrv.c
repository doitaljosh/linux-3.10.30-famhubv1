...
#define AEC_CDSP_TAG_DATA		(0x00001400)
#define DATA_FIX_SIZE			(9)
				...
				struct MCDRV_CDSP_AEC_CDSP_INFO *psCdspInfo)
{
	...
	while (dChunkTop < dSize) {
		...
		case AEC_CDSP_TAG_DATA:
			if (dChunkSize < (UINT32)DATA_FIX_SIZE)
				return MCDRV_ERROR_ARGUMENT;

			if (psFuncInfo->pbData != NULL)
				return MCDRV_ERROR_ARGUMENT;
			...
			if (dChunkSize < (dTemp + (UINT32)DATA_FIX_SIZE))
				return MCDRV_ERROR_ARGUMENT;

			psFuncInfo->pbData = &pbPrm[dChunkTop];
			break;
 ...
 *
 ****************************************************************************/
static SINT32 DownloadData(const UINT8 *pbData)
{
	UINT8 bData;
	SINT32 sdRet;
	struct FSQ_DATA_INFO sDataInfo;
	...
	/* Transfer Data */
	sDataInfo.pbData = &pbData[9];
	sDataInfo.wScramble = MAKEWORD(pbData[0], 0x00);
	sDataInfo.wLoadAddr = MAKEWORD(pbData[4], pbData[3]);
	sDataInfo.wSize = MAKEWORD(pbData[8], pbData[7]);
	sDataInfo.bMsel = (UINT8)MSEL_DATA;
	sdRet = WriteProgram(&sDataInfo);
 ...
 *
 ****************************************************************************/
static SINT32 SetData(struct AEC_CDSP_FUNC_INFO *psFuncInfo)
{
	struct DEC_INFO *psDecInfo;

	if (NULL == psFuncInfo->pbData)
		return MCDRV_SUCCESS;
	...
	return DownloadData(psFuncInfo->pbData);
}
