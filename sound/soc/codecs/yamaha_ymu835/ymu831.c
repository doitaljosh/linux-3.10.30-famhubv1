 ...
 *
 * Copyright (c) 2012-2015 Yamaha Corporation
 *
...
#include <linux/of.h> 	//hsguy.son (add)
#include <linux/spi/spi.h>
#elif (BUS_SELECT == BUS_SEL_SLIM)
#include <linux/slimbus/slimbus.h>
#endif

#define MC_ASOC_DRIVER_VERSION	"2.2.0"
...
static unsigned int mc_asoc_read_reg(struct snd_soc_codec *codec, unsigned int reg);
static int mc_asoc_write_reg(struct snd_soc_codec *codec, unsigned int reg, unsigned int value);

#if (BUS_SELECT == BUS_SEL_I2C)
void mc_asoc_set_codec_ldod(int a) {return;}
#endif
#endif
...
/* Volume Assignment */
static const char * const vol_asgn_dig_param_text[] = {
	"MUSICIN", "EXTIN", "VOICEIN", "REFIN", "ADIF0", "ADIF1", "ADIF2",
	"MUSICOUT", "EXTOUT", "VOICEOUT", "REFOUT",
	"DAC0", "DAC1", "DPATHDA", "DPATHAD"
};
static const char * const vol_asgn_ana_param_text[] = {
	"LIN1", "MIC1", "MIC2", "MIC3", "MIC4"
};
static const struct soc_enum	master_play_vol_asgn_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_MASTER_PLAY_VOL_ASGN, 0,
		ARRAY_SIZE(vol_asgn_dig_param_text), vol_asgn_dig_param_text);
static const struct soc_enum	voice_play_vol_asgn_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_VOICE_PLAY_VOL_ASGN, 0,
		ARRAY_SIZE(vol_asgn_dig_param_text), vol_asgn_dig_param_text);
static const struct soc_enum	voice_rec_vol_asgn_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_VOICE_REC_VOL_ASGN, 0,
		ARRAY_SIZE(vol_asgn_dig_param_text), vol_asgn_dig_param_text);
static const struct soc_enum	input_play_ana_vol_asgn_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_INPUT_PLAY_ANA_VOL_ASGN, 0,
		ARRAY_SIZE(vol_asgn_ana_param_text), vol_asgn_ana_param_text);
static const struct soc_enum	input_play_dig_vol_asgn_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_INPUT_PLAY_DIG_VOL_ASGN, 0,
		ARRAY_SIZE(vol_asgn_dig_param_text), vol_asgn_dig_param_text);

/* Audio Mode */
#if (MIXER_ADD_DEL_MODE == MIXER_ADD_DEL_MODE_OFF)
static const char * const audio_mode_play_param_text[] = {
	"off", "audio", "audioae", "audiovbm", "audiovbv",
	"incall", "incallvb",
	"audio+incall", "audio+incallvb",
	"audioae+incall", "audioae+incallvb"
};
...
static const char * const audio_mode_cap_param_text[] = {
	"off", "audio", "audiovbr", "audiovbv", "incall", "audio+incall",
	"vbox"
};
...
#else
static const char * const audio_mode_play_add_del_param_text[] = {
	"none", "audio", "audioae", "audiovbm", "audiovbv",
	"incall", "incallvb"
};
static const struct soc_enum	audio_mode_play_add_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_AUDIO_MODE_PLAY_ADD, 0,
		ARRAY_SIZE(audio_mode_play_add_del_param_text),
		audio_mode_play_add_del_param_text);
static const struct soc_enum	audio_mode_play_del_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_AUDIO_MODE_PLAY_DEL, 0,
		ARRAY_SIZE(audio_mode_play_add_del_param_text),
		audio_mode_play_add_del_param_text);

static const char * const audio_mode_cap_add_del_param_text[] = {
	"none", "audio", "audiovbr", "audiovbv", "incall", "vbox"
};
static const struct soc_enum	audio_mode_cap_add_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_AUDIO_MODE_CAP_ADD, 0,
		ARRAY_SIZE(audio_mode_cap_add_del_param_text),
		audio_mode_cap_add_del_param_text);
static const struct soc_enum	audio_mode_cap_del_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_AUDIO_MODE_CAP_DEL, 0,
		ARRAY_SIZE(audio_mode_cap_add_del_param_text),
		audio_mode_cap_add_del_param_text);
#endif
...
#else
static const char * const output_path_add_del_param_text[] = {
	"none",
	"SP", "RC", "HP", "HS", "LO1", "LO2", "BT"
};
static const struct soc_enum	output_path_add_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_OUTPUT_PATH_ADD, 0,
		ARRAY_SIZE(output_path_add_del_param_text),
		output_path_add_del_param_text);
static const struct soc_enum	output_path_del_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_OUTPUT_PATH_DEL, 0,
		ARRAY_SIZE(output_path_add_del_param_text),
		output_path_add_del_param_text);
#endif
...
static const char * const adif_src_param_text[] = {
	"off",
	"ADC0L", "ADC0R", "ADC1",
	"PDM0L", "PDM0R", "PDM1L", "PDM1R",
	"DAC0REF", "DAC1REF"
};
static const struct soc_enum	adif_src[]	= {
	SOC_ENUM_SINGLE(MC_ASOC_ADIF0L_SOURCE, 0, 8, adif_src_param_text),
	SOC_ENUM_SINGLE(MC_ASOC_ADIF0R_SOURCE, 0, 8, adif_src_param_text),
	SOC_ENUM_SINGLE(MC_ASOC_ADIF1L_SOURCE, 0, 8, adif_src_param_text),
	SOC_ENUM_SINGLE(MC_ASOC_ADIF1R_SOURCE, 0, 8, adif_src_param_text),
	SOC_ENUM_SINGLE(MC_ASOC_ADIF2L_SOURCE, 0, 10, adif_src_param_text),
	SOC_ENUM_SINGLE(MC_ASOC_ADIF2R_SOURCE, 0, 10, adif_src_param_text)
};

/* Adc Source */
static const char * const adc0_src_param_text[] = {
	"off", "MainMIC", "SubMIC", "ThirdMIC", "Headset", "Line1", "Line1M"
};
static const char * const adc1_src_param_text[] = {
	"off", "MainMIC", "SubMIC", "ThirdMIC", "Headset", "Line1M"
};
static const struct soc_enum	adc_src[]	= {
	SOC_ENUM_SINGLE(MC_ASOC_ADC0L_SOURCE, 0, 7, adc0_src_param_text),
	SOC_ENUM_SINGLE(MC_ASOC_ADC0R_SOURCE, 0, 7, adc0_src_param_text),
	SOC_ENUM_SINGLE(MC_ASOC_ADC1_SOURCE, 0, 6, adc1_src_param_text),
};
...
static const struct soc_enum	input_playback_path_sw_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_INPUT_PLAYBACK_PATH, 0,
...
static const struct soc_enum	sub_mic_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_SUB_MIC, 0, ARRAY_SIZE(mic_param_text),
		mic_param_text);
/*	3rd Mic	*/
static const struct soc_enum	third_mic_param_enum =
	SOC_ENUM_SINGLE(MC_ASOC_THIRD_MIC, 0, ARRAY_SIZE(mic_param_text),
		mic_param_text);
/*	Headset Mic	*/
...
static const struct snd_kcontrol_new	mc_asoc_snd_controls[]	= {
	...
	SOC_ENUM("Master Playback Volume Assignment",
					master_play_vol_asgn_param_enum),
	SOC_ENUM("Voice Playback Volume Assignment",
					voice_play_vol_asgn_param_enum),
	SOC_ENUM("Voice Recording Volume Assignment",
					voice_rec_vol_asgn_param_enum),
	SOC_ENUM("Input Playback Analog Volume Assignment",
					input_play_ana_vol_asgn_param_enum),
	SOC_ENUM("Input Playback Digital Volume Assignment",
					input_play_dig_vol_asgn_param_enum),
	...
	SOC_ENUM("ADIF0L Source", adif_src[0]),
	SOC_ENUM("ADIF0R Source", adif_src[1]),
	SOC_ENUM("ADIF1L Source", adif_src[2]),
	SOC_ENUM("ADIF1R Source", adif_src[3]),
	SOC_ENUM("ADIF2L Source", adif_src[4]),
	SOC_ENUM("ADIF2R Source", adif_src[5]),

	SOC_ENUM("ADC0L Source", adc_src[0]),
	SOC_ENUM("ADC0R Source", adc_src[1]),
	SOC_ENUM("ADC1 Source", adc_src[2]),

	SOC_ENUM("Input Playback Path", input_playback_path_sw_param_enum),
...
#if (MIXER_ADD_DEL_MODE == MIXER_ADD_DEL_MODE_ON)
	SOC_ENUM("Audio Mode Playback Add", audio_mode_play_add_param_enum),
	SOC_ENUM("Audio Mode Playback Del", audio_mode_play_del_param_enum),
	SOC_ENUM("Audio Mode Capture Add", audio_mode_cap_add_param_enum),
	SOC_ENUM("Audio Mode Capture Del", audio_mode_cap_del_param_enum),
	SOC_ENUM("Output Path Add", output_path_add_param_enum),
	SOC_ENUM("Output Path Del", output_path_del_param_enum),
#endif
	...
	SOC_ENUM("Third Mic", third_mic_param_enum),
	SOC_ENUM("Headset Mic", hs_mic_param_enum),

	/* karaoke mixer */
	SOC_SINGLE("Karaoke Music Volume", MC_ASOC_K_MSC_VOL, 0, 31, 0),
	SOC_SINGLE("Karaoke Mic Volume", MC_ASOC_K_MIC_VOL, 0, 15, 0),
	SOC_SINGLE("Karaoke Echo Volume", MC_ASOC_K_ECH_VOL, 0, 15, 0),
	SOC_SINGLE("Karaoke Echo Feedback Gain", MC_ASOC_K_ECH_FB_GIN, 0, 7, 0),
	SOC_SINGLE("Karaoke Echo Feedback Delay", MC_ASOC_K_ECH_FB_DLY,
								0, 7, 0),
	SOC_SINGLE("Karaoke Echo Room Type", MC_ASOC_K_ECH_RM_TYP, 0, 7, 0),
	SOC_SINGLE("Karaoke Key Control", MC_ASOC_K_KEY_CTR, 0, 15, 0),
	SOC_SINGLE("Karaoke Vocal Cancel", MC_ASOC_K_V_CNC, 0, 3, 0),
	SOC_SINGLE("Karaoke Mic Equalizer 0", MC_ASOC_K_MIC_EQ_0, 0, 15, 0),
	SOC_SINGLE("Karaoke Mic Equalizer 1", MC_ASOC_K_MIC_EQ_1, 0, 15, 0),
	SOC_SINGLE("Karaoke Mic Equalizer 2", MC_ASOC_K_MIC_EQ_2, 0, 15, 0),
	SOC_SINGLE("Karaoke Mic Equalizer 3", MC_ASOC_K_MIC_EQ_3, 0, 15, 0),
	SOC_SINGLE("Karaoke Mic Equalizer 4", MC_ASOC_K_MIC_EQ_4, 0, 15, 0),
...
#if CONFIG_OEM_DEFINED
	SOC_SINGLE("Dsp Parameter2", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0),
	SOC_SINGLE("Dsp Parameter3", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0),
	SOC_SINGLE("Dsp Parameter4", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0),
	SOC_SINGLE("Dsp Parameter5", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0),
	SOC_SINGLE("Dsp Parameter6", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0),
	SOC_SINGLE("Dsp Parameter7", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0),
	SOC_SINGLE("Dsp Parameter8", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0),
	SOC_SINGLE("Dsp Parameter9", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0),
	SOC_SINGLE("Dsp Parameter10", MC_ASOC_DSP_PARAM, 0,
					ARRAY_SIZE(firmware_name)-1, 0)
#endif
};
...
static void ymu831_set_ldod(void)
{
	...
	ldod_reset = of_get_named_gpio(np, "gpio_reset", 0);
	if(gpio_is_valid(ldod_reset))
	{
		ret = gpio_request_one(ldod_reset, GPIOF_DIR_OUT, "gpio_reset");
		if(ret)
		{
			dev_err(dev, "Unable to request ldod gpio [%d]\n", (int)ldod_reset);
		}
	}

	gpio_set_value(ldod_reset, 0);
	msleep(10);
	gpio_set_value(ldod_reset, 1);
	msleep(30);
}
//hsguy.son (add_end)


#ifdef CONFIG_OEM_DEFINED
struct mc_asoc_platform_data mc_asoc_pdata = {
...
static UINT8	mc_asoc_third_mic	= THIRD_MIC;
...
#define K_ECH_VOL		(3)
#define K_MSC_VOL		(4)
#define K_ECH_RM_TYP		(4)
#define K_ECH_FB_GIN		(5)
#define K_ECH_FB_DLY		(5)
#define K_KEY_CTR		(6)
#define K_V_CNC			(7)
#define K_PRM_NUM		(8)

#define K_MIC_EQ_0_REG		(84)
#define K_MIC_EQ_1_REG		(84)
#define K_MIC_EQ_2_REG		(85)
#define K_MIC_EQ_3_REG		(85)
#define K_MIC_EQ_4_REG		(86)
#define K_MIC_VOL_REG		(87)
#define K_ECH_VOL_REG		(87)
#define K_MSC_VOL_REG		(88)
#define K_ECH_RM_TYP_REG	(88)
#define K_ECH_FB_GIN_REG	(89)
#define K_ECH_FB_DLY_REG	(89)
#define K_KEY_CTR_REG		(90)
#define K_V_CNC_REG		(91)

static int mc_asoc_kprm[K_PRM_NUM];
 ...
 * Function
 */
UINT8 mc_asoc_get_KPrm(UINT8 *KPrm, UINT8 size)
{
	int i;
...
SINT32	mc_asoc_enable_clock(int enable,
				struct MCDRV_CLOCKSW_INFO *psClockSwInfo)
{
	...
	if (mc_asoc->penableclkfn == NULL) {
		psClockSwInfo->bClkSrc	= MCDRV_CLKSW_CLKB;
		return 0;
	}

	err	= (*mc_asoc->penableclkfn)(NULL, enable, 0);
	if (enable) {
		if (err != 0)
			psClockSwInfo->bClkSrc	= MCDRV_CLKSW_CLKB;
		else
			psClockSwInfo->bClkSrc	= MCDRV_CLKSW_CLKA;
...
static void free_dsp_mem(UINT8 *p)
{
#ifdef DSP_MEM_STATIC
#else
	if (p != NULL) {
...
static int get_third_mic_block_on(void)
{
	return get_mic_block_on(mc_asoc_third_mic);
}
...
)
{
	...
	mixer_ctl_info->adif0l_src	=
		read_cache(codec, MC_ASOC_ADIF0L_SOURCE);
	if (mixer_ctl_info->adif0l_src < 0)
		return -EIO;

	mixer_ctl_info->adif0r_src	=
		read_cache(codec, MC_ASOC_ADIF0R_SOURCE);
	if (mixer_ctl_info->adif0r_src < 0)
		return -EIO;

	mixer_ctl_info->adif1l_src	=
		read_cache(codec, MC_ASOC_ADIF1L_SOURCE);
	if (mixer_ctl_info->adif1l_src < 0)
		return -EIO;

	mixer_ctl_info->adif1r_src	=
		read_cache(codec, MC_ASOC_ADIF1R_SOURCE);
	if (mixer_ctl_info->adif1r_src < 0)
		return -EIO;

	mixer_ctl_info->adif2l_src	=
		read_cache(codec, MC_ASOC_ADIF2L_SOURCE);
	if (mixer_ctl_info->adif2l_src < 0)
		return -EIO;

	mixer_ctl_info->adif2r_src	=
		read_cache(codec, MC_ASOC_ADIF2R_SOURCE);
	if (mixer_ctl_info->adif2r_src < 0)
		return -EIO;

	mixer_ctl_info->adc0l_src	=
		read_cache(codec, MC_ASOC_ADC0L_SOURCE);
	if (mixer_ctl_info->adc0l_src < 0)
		return -EIO;

	mixer_ctl_info->adc0r_src	=
		read_cache(codec, MC_ASOC_ADC0R_SOURCE);
	if (mixer_ctl_info->adc0r_src < 0)
		return -EIO;

	mixer_ctl_info->adc1_src	=
		read_cache(codec, MC_ASOC_ADC1_SOURCE);
	if (mixer_ctl_info->adc1_src < 0)
		return -EIO;

	mixer_ctl_info->input_play =
		read_cache(codec, MC_ASOC_INPUT_PLAYBACK_PATH);
	if (mixer_ctl_info->input_play < 0)
		return -EIO;
...
{
	int	audio_mode_play	= mixer_ctl_info->audio_mode_play;
	int	audio_mode_cap	= mixer_ctl_info->audio_mode_cap;

	if ((audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_INCALL)
	|| (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_INCALLVB)
	) {
		if ((audio_mode_cap != MC_ASOC_AUDIO_MODE_CAP_INCALL)
		&& (audio_mode_cap != MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL)) {
			;
			audio_mode_play	= MC_ASOC_AUDIO_MODE_PLAY_OFF;
		}
	} else if ((audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALL)
		|| (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALLVB)
	) {
		if ((audio_mode_cap != MC_ASOC_AUDIO_MODE_CAP_INCALL)
		&& (audio_mode_cap != MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL)) {
			;
			audio_mode_play	= MC_ASOC_AUDIO_MODE_PLAY_AUDIO;
		}
	} else if ((audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALL)
		|| (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALLVB)
	) {
		if ((audio_mode_cap != MC_ASOC_AUDIO_MODE_CAP_INCALL)
		&& (audio_mode_cap != MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL)) {
			;
			audio_mode_play	= MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE;
		}
	} else {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			;
			audio_mode_cap	= MC_ASOC_AUDIO_MODE_CAP_OFF;
		} else if (audio_mode_cap
				== MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL) {
			;
			audio_mode_cap	= MC_ASOC_AUDIO_MODE_CAP_AUDIO;
		}
	}

	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIO) {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_OFF) {
			/*	playback:audio, capture:off	*/
		...
		} else if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_AUDIO) {
			/*	playback:audio, capture:audio	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				if ((mc_asoc_port_rate == MCDRV_FS_96000)
				|| (mc_asoc_port_rate == MCDRV_FS_192000)) {
		...
		} else if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_AUDIOVBR) {
			/*	playback:audio, capture:audiovbr	*/
			if ((mc_asoc_port_rate == MCDRV_FS_96000)
			|| (mc_asoc_port_rate == MCDRV_FS_192000)) {
				;
				return -1;
		...
		} else if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_AUDIOVBV) {
			/*	playback:audio, capture:audiovbv	*/
			if ((mc_asoc_port_rate == MCDRV_FS_96000)
			|| (mc_asoc_port_rate == MCDRV_FS_192000)) {
				;
				return -1;
		...
		} else if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_VBOX) {
			/*	playback:audio, capture:vbox	*/
			if ((mc_asoc_port_rate == MCDRV_FS_96000)
			|| (mc_asoc_port_rate == MCDRV_FS_192000)) {
				;
				return -1;
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE) {
		if ((mc_asoc_port_rate == MCDRV_FS_96000)
		|| (mc_asoc_port_rate == MCDRV_FS_192000)) {
			;
			return -1;
		...
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_OFF) {
			...
			/*	playback:audioae, capture:audio	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 18;
			} else if ((mixer_ctl_info->output_path
			...
			/*	playback:audioae, capture:audiovbr	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 31;
			} else if ((mixer_ctl_info->output_path
			...
			/*	playback:audioae, capture:audiovbv	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 44;
			} else if ((mixer_ctl_info->output_path
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIOVBM) {
		if ((mc_asoc_port_rate == MCDRV_FS_96000)
		|| (mc_asoc_port_rate == MCDRV_FS_192000)) {
			;
			return -1;
		...
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_OFF) {
			...
			/*	playback:audiovbm, capture:audio	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 21;
			} else if ((mixer_ctl_info->output_path
			...
			/*	playback:audiovbm, capture:audiovbr	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 34;
			} else if ((mixer_ctl_info->output_path
			...
			/*	playback:audiovbm, capture:audiovbv	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 47;
			} else if ((mixer_ctl_info->output_path
			...
			/*	playback:audiovbm, capture:vbox	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 99;
			} else if ((mixer_ctl_info->output_path
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIOVBV) {
		if ((mc_asoc_port_rate == MCDRV_FS_96000)
		|| (mc_asoc_port_rate == MCDRV_FS_192000)) {
			;
			return -1;
		...
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_OFF) {
			...
			/*	playback:audiovbv, capture:audio	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 24;
			} else if ((mixer_ctl_info->output_path
			...
			/*	playback:audiovbv, capture:audiovbr	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 37;
			} else if ((mixer_ctl_info->output_path
			...
			/*	playback:audiovbv, capture:audiovbv	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 50;
			} else if ((mixer_ctl_info->output_path
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_OFF) {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_AUDIO) {
			/*	playback:off, capture:audio	*/
			if ((mc_asoc_port_rate == MCDRV_FS_96000)
			|| (mc_asoc_port_rate == MCDRV_FS_192000)) {
				;
				return 89;
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_INCALL) {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			/*	playback:incall, capture:incall	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 53;
			} else if ((mixer_ctl_info->output_path
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALL) {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			/*	playback:audio+incall, capture:incall	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 56;
			} else if ((mixer_ctl_info->output_path
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALL) {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			/*	playback:audioae+incall, capture:incall	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 59;
			} else if ((mixer_ctl_info->output_path
				...
				== MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL) {
			/*	playback:audioae+incall, capture:audio+incall
			*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 68;
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_INCALLVB) {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			/*	playback:incallvb, capture:incall	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 71;
			} else if ((mixer_ctl_info->output_path
				...
				== MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL) {
			/*	playback:incallvb, capture:audio+incall	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 80;
			} else if ((mixer_ctl_info->output_path
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALLVB) {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			/*	playback:audio+incallvb, capture:incall	*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 74;
			} else if ((mixer_ctl_info->output_path
				...
				== MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL) {
			/*	playback:audio+incallvb, capture:audio+incall
			*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 83;
	...
	if (audio_mode_play == MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALLVB) {
		if (audio_mode_cap == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			/*	playback:audioae+incallvb, capture:incall
			*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 77;
				...
				== MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL) {
			/*	playback:audioae+incallvb, capture:audio+incall
			*/
			if (mixer_ctl_info->output_path
				== MC_ASOC_OUTPUT_PATH_BT) {
				return 86;
...
{
	if ((preset_idx >= 52) && (preset_idx <= 87)) {
		;
		return 1;
	}
...
static int get_assign_dig_vol_reg(
	int	value
)
{
	int	reg;
	...
	case	2:
		...
		break;
	case	3:
		reg	= MC_ASOC_DVOL_REFIN;
		break;
	case	4:
		reg	= MC_ASOC_DVOL_ADIF0IN;
		break;
	case	5:
		reg	= MC_ASOC_DVOL_ADIF1IN;
		break;
	case	6:
		reg	= MC_ASOC_DVOL_ADIF2IN;
		break;
	case	7:
		reg	= MC_ASOC_DVOL_MUSICOUT;
		break;
	case	8:
		reg	= MC_ASOC_DVOL_EXTOUT;
		break;
	case	9:
		reg	= MC_ASOC_DVOL_VOICEOUT;
		break;
	case	10:
		reg	= MC_ASOC_DVOL_REFOUT;
		break;
	case	11:
	...
	case	12:
		...
		break;
	case	13:
		reg	= MC_ASOC_DVOL_DPATHDA;
		break;
	case	14:
		reg	= MC_ASOC_DVOL_DPATHAD;
		break;
	default:
		return -EIO;
	}
...
static int get_assign_ana_vol_reg(
	int	value
)
{
	int	reg;
...
)
{
	...
	cache	= read_cache(codec, MC_ASOC_MASTER_PLAY_VOL_ASGN);
	if (reg != get_assign_dig_vol_reg(cache)) {
		;
		return 0;
	}
...
)
{
	...
	cache	= read_cache(codec, MC_ASOC_VOICE_PLAY_VOL_ASGN);
	if (reg != get_assign_dig_vol_reg(cache)) {
		;
		return 0;
	}
...
static int get_inputplay_dig_vol(
	struct snd_soc_codec *codec,
	SINT16	*db,
	int	reg,
	int	i
)
{
	...
	cache	= read_cache(codec, MC_ASOC_INPUT_PLAY_DIG_VOL_ASGN);
	if (reg != get_assign_dig_vol_reg(cache)) {
		;
		return 0;
	}

	cache	= read_cache(codec, MC_ASOC_DVOL_APLAY_D);
	if (cache < 0) {
		;
		return -EIO;
	}
	...
	v	= (cache >> (i*8)) & 0xff;
	vol	= (v & 0x80) ? (v & 0x7f) : 0;
	*db	= vreg_map[MC_ASOC_DVOL_APLAY_D].volmap[vol];
	return 0;
}

static int get_inputplay_ana_vol(
	struct snd_soc_codec *codec,
	SINT16	*db,
	int	reg,
	int	i
)
{
	...
	cache	= read_cache(codec, MC_ASOC_INPUT_PLAY_ANA_VOL_ASGN);
	if (reg != get_assign_ana_vol_reg(cache)) {
		;
		return 0;
	}

	cache	= read_cache(codec, MC_ASOC_DVOL_APLAY_A);
	if (cache < 0) {
		;
		return -EIO;
	}
	...
	v	= (cache >> (i*8)) & 0xff;
	vol	= (v & 0x80) ? (v & 0x7f) : 0;
	*db	= vreg_map[MC_ASOC_DVOL_APLAY_A].volmap[vol];
	return 0;
}
...
)
{
	...
	vp	= (SINT16 *)((void *)vol_info + vreg_map[reg].offset);
	...
	for (ch = 0; ch < vreg_map[reg].channels; ch++, vp++) {
		unsigned int	val;
		SINT16	db;
		int	asgn;
		...
		if ((val & 0x80) == 0) {
			...
			db	= vreg_map[reg].volmap[(val & 0x7f)];
		}
		...
		case	MC_ASOC_DVOL_DPATHAD:
			if (read_cache(codec, MC_ASOC_VOICE_RECORDING) == 0) {
				if ((is_incall(preset_idx) != 0)
				|| (mixer_ctl_info->audio_mode_cap
					== MC_ASOC_AUDIO_MODE_CAP_AUDIOVBV)) {
					asgn	= read_cache(codec,
						MC_ASOC_VOICE_REC_VOL_ASGN);
					if (reg == get_assign_dig_vol_reg(asgn)
					) {
						db	=
							vreg_map[reg].volmap[0];
						break;
					}
				}
			}
			if (get_master_vol(codec, &db, reg, ch) != 0) {
				;
				return -EIO;
			}
			if (get_voice_play_vol(codec, &db, reg, ch) != 0) {
				;
				return -EIO;
			}
			if ((mixer_ctl_info->input_play
					!= MC_ASOC_INPUT_PLAYBACK_PATH_OFF)
			&& (get_inputplay_dig_vol(codec, &db, reg, ch) != 0)) {
				;
				return -EIO;
			}
			break;
		...
		case	MC_ASOC_AVOL_MIC4:
			if ((mixer_ctl_info->input_play
					!= MC_ASOC_INPUT_PLAYBACK_PATH_OFF)
			&& (get_inputplay_ana_vol(codec, &db, reg, ch) != 0)) {
				;
				return -EIO;
			}
			break;
...
)
{
	...
	for (reg = MC_ASOC_DVOL_MUSICIN; reg <= MC_ASOC_AVOL_LINEOUT2; reg++) {
...
)
{
	...
	&& (mixer_ctl_info->output_path != MC_ASOC_OUTPUT_PATH_LO2_LO1)) {
		...
		for (bCh = 0; bCh < LOUT1_PATH_CHANNELS; bCh++) {
			;
			path_info->asLout1[bCh].dSrcOnOff	= 0x002AAAAA;
		}
	}
	...
	&& (mixer_ctl_info->output_path != MC_ASOC_OUTPUT_PATH_LO2_LO1)) {
		...
		for (bCh = 0; bCh < LOUT2_PATH_CHANNELS; bCh++) {
			;
			path_info->asLout2[bCh].dSrcOnOff	= 0x002AAAAA;
		}
	}

	if ((bSpUse == 0) && (bLo2Use == 0))
		for (bCh = 0; bCh < DAC1_PATH_CHANNELS; bCh++)
			path_info->asDac1[bCh].dSrcOnOff = 0x00AAAAAA;

	if ((bLo1Use == 0) && (bRcUse == 0) && (bHpUse == 0))
		for (bCh = 0; bCh < DAC0_PATH_CHANNELS; bCh++)
			path_info->asDac0[bCh].dSrcOnOff = 0x00AAAAAA;
}
...
)
{
	...
	&& (output_path != MC_ASOC_OUTPUT_PATH_LO2_BT)) {
		for (bCh = 0; bCh < VOICEOUT_PATH_CHANNELS; bCh++) {
			;
			path_info->asVoiceOut[bCh].dSrcOnOff	= 0x00AAAAAA;
		}
	}
}
...
)
{
	...
	int	third_mic_block_on	= get_third_mic_block_on();
	int	hs_mic_block_on		= get_hs_mic_block_on();

	switch (mixer_ctl_info->adc0l_src) {
	case	MC_ASOC_ADC_SOURCE_MAIN_MIC:
		if (main_mic_block_on != -1) {
			;
			path_info->asAdc0[0].dSrcOnOff |= main_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_SUB_MIC:
		if (sub_mic_block_on != -1) {
			;
			path_info->asAdc0[0].dSrcOnOff |= sub_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_THIRD_MIC:
		if (third_mic_block_on != -1) {
			;
			path_info->asAdc0[0].dSrcOnOff |= third_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_HS_MIC:
		if (hs_mic_block_on != -1) {
			;
			path_info->asAdc0[0].dSrcOnOff |= hs_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_LINE1:
		path_info->asAdc0[0].dSrcOnOff |= MCDRV_ASRC_LINEIN1_L_ON;
		break;
	case	MC_ASOC_ADC_SOURCE_LINE1M:
		path_info->asAdc0[0].dSrcOnOff |= MCDRV_ASRC_LINEIN1_M_ON;
		break;
	}
	switch (mixer_ctl_info->adc0r_src) {
	case	MC_ASOC_ADC_SOURCE_MAIN_MIC:
		if (main_mic_block_on != -1) {
			;
			path_info->asAdc0[1].dSrcOnOff |= main_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_SUB_MIC:
		if (sub_mic_block_on != -1) {
			;
			path_info->asAdc0[1].dSrcOnOff |= sub_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_THIRD_MIC:
		if (third_mic_block_on != -1) {
			;
			path_info->asAdc0[1].dSrcOnOff |= third_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_HS_MIC:
		if (hs_mic_block_on != -1) {
			;
			path_info->asAdc0[1].dSrcOnOff |= hs_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_LINE1:
		path_info->asAdc0[1].dSrcOnOff |= MCDRV_ASRC_LINEIN1_R_ON;
		break;
	case	MC_ASOC_ADC_SOURCE_LINE1M:
		path_info->asAdc0[1].dSrcOnOff |= MCDRV_ASRC_LINEIN1_M_ON;
		break;
	}
	switch (mixer_ctl_info->adc1_src) {
	case	MC_ASOC_ADC_SOURCE_MAIN_MIC:
		if (main_mic_block_on != -1) {
			;
			path_info->asAdc1[0].dSrcOnOff |= main_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_SUB_MIC:
		if (sub_mic_block_on != -1) {
			;
			path_info->asAdc1[0].dSrcOnOff |= sub_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_THIRD_MIC:
		if (third_mic_block_on != -1) {
			;
			path_info->asAdc1[0].dSrcOnOff |= third_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_HS_MIC:
		if (hs_mic_block_on != -1) {
			;
			path_info->asAdc1[0].dSrcOnOff |= hs_mic_block_on;
		}
		break;
	case	MC_ASOC_ADC_SOURCE_LINE1:
		path_info->asAdc1[0].dSrcOnOff |= MCDRV_ASRC_LINEIN1_M_ON;
		break;
	}

	set_adif_src(mixer_ctl_info->adif0l_src,
				&path_info->asAdif0[0].dSrcOnOff);
	set_adif_src((UINT8)(mixer_ctl_info->adif0r_src),
				&path_info->asAdif0[1].dSrcOnOff);

	set_adif_src((UINT8)mixer_ctl_info->adif1l_src,
				&path_info->asAdif1[0].dSrcOnOff);
	set_adif_src((UINT8)(mixer_ctl_info->adif1r_src),
				&path_info->asAdif1[1].dSrcOnOff);

	set_adif_src((UINT8)mixer_ctl_info->adif2l_src,
				&path_info->asAdif2[0].dSrcOnOff);
	set_adif_src((UINT8)(mixer_ctl_info->adif2r_src),
				&path_info->asAdif2[1].dSrcOnOff);
}

static void mask_InputPath(
	UINT32	*dSrcOnOff,
	int	input_path
)
{
	switch (input_path) {
	case	MC_ASOC_INPUT_PATH_ADIF0:
		*dSrcOnOff	&= ~(MCDRV_D1SRC_ADIF1_ON
					|MCDRV_D1SRC_ADIF2_ON
					|MCDRV_D1SRC_VBOXOUT_ON);
		break;
	case	MC_ASOC_INPUT_PATH_ADIF1:
		*dSrcOnOff	&= ~(MCDRV_D1SRC_ADIF0_ON
					|MCDRV_D1SRC_ADIF2_ON
					|MCDRV_D1SRC_VBOXOUT_ON);
		break;
	case	MC_ASOC_INPUT_PATH_ADIF2:
		*dSrcOnOff	&= ~(MCDRV_D1SRC_ADIF0_ON
...
static void set_input_play_path(
	struct MCDRV_PATH_INFO	*path_info,
	const struct mc_asoc_mixer_path_ctl_info	*mixer_ctl_info,
	int	preset_idx
)
{
	int	idx	= InputPlaybackPathMapping[mixer_ctl_info->input_play-1]
						[preset_idx];
	int	adc_src[2]	= {
					MC_ASOC_ADC_SOURCE_OFF,
					MC_ASOC_ADC_SOURCE_OFF
				};
	UINT8	bCh;
	...
	if (idx >= ARRAY_SIZE(InputPlaybackPath)) {
		dbg_info("\n*******\nInputPlaybackPathMapping err\n*******\n");
		return;
	}

	switch (mixer_ctl_info->input_path) {
	case	MC_ASOC_INPUT_PATH_ADIF0:
		switch (mixer_ctl_info->adif0l_src) {
		case	1:
			/*	ADC0L	*/
			adc_src[0]	= mixer_ctl_info->adc0l_src;
			break;
		case	2:
			/*	ADC0R	*/
			adc_src[0]	= mixer_ctl_info->adc0r_src;
			break;
		case	3:
			/*	ADC1	*/
			adc_src[0]	= mixer_ctl_info->adc1_src;
			if (adc_src[0] == MC_ASOC_ADC_SOURCE_LINE1) {
				;
				adc_src[0]	= MC_ASOC_ADC_SOURCE_LINE1M;
			}
			break;
		default:
			break;
		}
		switch (mixer_ctl_info->adif0r_src) {
		case	1:
			/*	ADC0L	*/
			adc_src[1]	= mixer_ctl_info->adc0l_src;
			break;
		case	2:
			/*	ADC0R	*/
			adc_src[1]	= mixer_ctl_info->adc0r_src;
			break;
		case	3:
			/*	ADC1	*/
			adc_src[1]	= mixer_ctl_info->adc1_src;
			if (adc_src[1] == MC_ASOC_ADC_SOURCE_LINE1) {
				;
				adc_src[1]	= MC_ASOC_ADC_SOURCE_LINE1M;
			}
			break;
		default:
			break;
		...
		break;
	case	MC_ASOC_INPUT_PATH_ADIF1:
		switch (mixer_ctl_info->adif1l_src) {
		case	1:
			/*	ADC0L	*/
			adc_src[0]	= mixer_ctl_info->adc0l_src;
			break;
		...
		switch (mixer_ctl_info->adif1r_src) {
		case	1:
			/*	ADC0L	*/
			adc_src[1]	= mixer_ctl_info->adc0l_src;
			break;
		...
		break;
	case	MC_ASOC_INPUT_PATH_ADIF2:
		switch (mixer_ctl_info->adif2l_src) {
		case	1:
			/*	ADC0L	*/
			adc_src[0]	= mixer_ctl_info->adc0l_src;
			break;
		...
		switch (mixer_ctl_info->adif2r_src) {
		case	1:
			/*	ADC0L	*/
			adc_src[1]	= mixer_ctl_info->adc0l_src;
			break;
	...
	&& (adc_src[0] == MC_ASOC_ADC_SOURCE_OFF)
	&& (adc_src[1] == MC_ASOC_ADC_SOURCE_OFF)) {
		;
		return;
	}

	add_path_info(path_info,
			(struct MCDRV_PATH_INFO *)&InputPlaybackPath[idx]);
	if (mixer_ctl_info->input_path != MC_ASOC_INPUT_PATH_BT) {
		set_ADC_src(path_info, mixer_ctl_info);
		for (bCh = 0; bCh < VBOXIOIN_PATH_CHANNELS; bCh++) {
			;
			path_info->asVboxIoIn[bCh].dSrcOnOff	= 0x00AAAAAA;
		}
	}
	for (bCh = 0; bCh < AE_PATH_CHANNELS; bCh++) {
		;
		mask_InputPath(&path_info->asAe0[bCh].dSrcOnOff,
					mixer_ctl_info->input_path);
	}
	...
	&& (mixer_ctl_info->output_path != MC_ASOC_OUTPUT_PATH_LO1_BT)
	&& (mixer_ctl_info->output_path != MC_ASOC_OUTPUT_PATH_LO2_BT)) {
		path_info->asVboxMixIn[0].dSrcOnOff	= 0x00AAAAAA;
		path_info->asVboxMixIn[1].dSrcOnOff	= 0x00AAAAAA;
		if (mixer_ctl_info->input_play
				== MC_ASOC_INPUT_PLAYBACK_PATH_ONVB) {
			mask_InputPath(&path_info->asVboxMixIn[2].dSrcOnOff,
						mixer_ctl_info->input_path);
			mask_InputPath(&path_info->asVboxMixIn[3].dSrcOnOff,
						mixer_ctl_info->input_path);
		}
		for (bCh = 0; bCh < DAC0_PATH_CHANNELS; bCh++) {
			mask_InputPath(&path_info->asDac0[bCh].dSrcOnOff,
						mixer_ctl_info->input_path);
		}
		for (bCh = 0; bCh < DAC1_PATH_CHANNELS; bCh++) {
			mask_InputPath(&path_info->asDac1[bCh].dSrcOnOff,
						mixer_ctl_info->input_path);
		}
		mask_BTOut_src(path_info, mixer_ctl_info->output_path);
	} else {
		if (mixer_ctl_info->input_play
				== MC_ASOC_INPUT_PLAYBACK_PATH_ON) {
			mask_InputPath(&path_info->asVboxMixIn[0].dSrcOnOff,
						mixer_ctl_info->input_path);
			mask_InputPath(&path_info->asVboxMixIn[1].dSrcOnOff,
						mixer_ctl_info->input_path);
		} else {
			mask_InputPath(&path_info->asVboxMixIn[2].dSrcOnOff,
						mixer_ctl_info->input_path);
		...
		if (mixer_ctl_info->output_path == MC_ASOC_OUTPUT_PATH_BT) {
			for (bCh = 0; bCh < DAC0_PATH_CHANNELS; bCh++) {
				path_info->asDac0[bCh].dSrcOnOff
								= 0x00AAAAAA;
			}
			for (bCh = 0; bCh < DAC1_PATH_CHANNELS; bCh++) {
				path_info->asDac1[bCh].dSrcOnOff
								= 0x00AAAAAA;
			}
		} else {
			for (bCh = 0; bCh < DAC0_PATH_CHANNELS; bCh++) {
	...
	mask_AnaOut_src(path_info, mixer_ctl_info);
}
...
)
{
	...
	if (mixer_ctl_info->input_play != MC_ASOC_INPUT_PLAYBACK_PATH_OFF) {
		;
		set_input_play_path(path_info, mixer_ctl_info, preset_idx);
	}
	...
	if (is_incall(preset_idx) != 0) {
		...
		for (bCh = 0; bCh < AE_PATH_CHANNELS; bCh++) {
			;
			mask_InputPath(&path_info->asAe1[bCh].dSrcOnOff,
						mixer_ctl_info->input_path);
		}
		if (mixer_ctl_info->input_path != MC_ASOC_INPUT_PATH_BT) {
			for (bCh = 0; bCh < VBOXIOIN_PATH_CHANNELS; bCh++) {
				;
				path_info->asVboxIoIn[bCh].dSrcOnOff	=
								0x00AAAAAA;
			}
		}
		return;
	...
	|| (preset_idx == 25)
		...
		for (bCh = 0; bCh < MUSICOUT_PATH_CHANNELS; bCh++) {
			;
			mask_InputPath(&path_info->asMusicOut[bCh].dSrcOnOff,
						mixer_ctl_info->input_path);
		}
		...
		|| (mixer_ctl_info->output_path == MC_ASOC_OUTPUT_PATH_LO2_BT)
			...
			for (bCh = 0; bCh < MUSICOUT_PATH_CHANNELS; bCh++) {
				;
				mask_InputPath(
					&path_info->asMusicOut[bCh].dSrcOnOff,
						mixer_ctl_info->input_path);
			}
		}
		for (bCh = 0; bCh < AE_PATH_CHANNELS; bCh++) {
		...
		for (bCh = 0; bCh < VBOXMIXIN_PATH_CHANNELS; bCh++) {
			;
			mask_InputPath(&path_info->asVboxMixIn[bCh].dSrcOnOff,
						mixer_ctl_info->input_path);
		}
	}
}
...
)
{
	...
	if (port_prm->stream == 0) {
		...
		port->sDioCommon.bAutoFs	= MCDRV_AUTOFS_ON;
#else
	...
	unsigned int value)
{
	...
	old_value	= read_cache(codec, reg);
	if (old_value < 0) {
		;
		return -EIO;
	}
	...
	memset(&vol_info, 0, sizeof(struct MCDRV_VOL_INFO));
	switch (reg) {
	case	MC_ASOC_MASTER_PLAY_VOL_ASGN:
		reg	= get_assign_dig_vol_reg(old_value);
		err	= set_vol_info(codec, &vol_info, reg, &mixer_ctl_info,
								preset_idx);
		if (err < 0) {
			;
	...
	case	MC_ASOC_DVOL_MASTER:
		value	= read_cache(codec, MC_ASOC_MASTER_PLAY_VOL_ASGN);
		reg	= get_assign_dig_vol_reg(value);
		break;

	case	MC_ASOC_VOICE_PLAY_VOL_ASGN:
		reg	= get_assign_dig_vol_reg(old_value);
		err	= set_vol_info(codec, &vol_info, reg, &mixer_ctl_info,
								preset_idx);
		if (err < 0) {
	...
	case	MC_ASOC_DVOL_VOICE:
		value	= read_cache(codec, MC_ASOC_VOICE_PLAY_VOL_ASGN);
		reg	= get_assign_dig_vol_reg(value);
		break;

	case	MC_ASOC_INPUT_PLAY_ANA_VOL_ASGN:
		reg	= get_assign_ana_vol_reg(old_value);
		err	= set_vol_info(codec, &vol_info, reg, &mixer_ctl_info,
								preset_idx);
		if (err < 0) {
			;
	...
	case	MC_ASOC_DVOL_APLAY_A:
		value	= read_cache(codec, MC_ASOC_INPUT_PLAY_ANA_VOL_ASGN);
		reg	= get_assign_ana_vol_reg(value);
		break;

	case	MC_ASOC_INPUT_PLAY_DIG_VOL_ASGN:
		reg	= get_assign_dig_vol_reg(old_value);
		err	= set_vol_info(codec, &vol_info, reg, &mixer_ctl_info,
								preset_idx);
		if (err < 0) {
	...
	case	MC_ASOC_DVOL_APLAY_D:
		value	= read_cache(codec, MC_ASOC_INPUT_PLAY_DIG_VOL_ASGN);
		reg	= get_assign_dig_vol_reg(value);
		break;

	case	MC_ASOC_VOICE_REC_VOL_ASGN:
		reg	= get_assign_dig_vol_reg(old_value);
		err	= set_vol_info(codec, &vol_info, reg, &mixer_ctl_info,
								preset_idx);
		if (err < 0) {
	...
	case	MC_ASOC_VOICE_RECORDING:
		value	= read_cache(codec, MC_ASOC_VOICE_REC_VOL_ASGN);
		reg	= get_assign_dig_vol_reg(value);
		break;
...
#if (MIXER_ADD_DEL_MODE == MIXER_ADD_DEL_MODE_ON)
static int p_mode_play;
module_param(p_mode_play, int, 444);
MODULE_PARM_DESC(p_mode_play, "audio mode play");
#endif
...
#if (MIXER_ADD_DEL_MODE == MIXER_ADD_DEL_MODE_ON)
static int p_mode_cap;
module_param(p_mode_cap, int, 444);
MODULE_PARM_DESC(p_mode_cap, "audio mode capture");
#endif
...
#if (MIXER_ADD_DEL_MODE == MIXER_ADD_DEL_MODE_ON)
static int add_audio_mode_play(
	struct snd_soc_codec *codec,
	unsigned int value
)
{
	...
	switch (audio_mode) {
	case	MC_ASOC_AUDIO_MODE_PLAY_OFF:
		audio_mode	= value;
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIO:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_INCALL) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALL;
		} else if (value == MC_ASOC_AUDIO_MODE_PLAY_INCALLVB) {
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALLVB;
		} else {
			return 0;
		}
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_INCALL) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALL;
		} else if (value == MC_ASOC_AUDIO_MODE_PLAY_INCALLVB) {
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALLVB;
		} else {
			return 0;
		}
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOVBM:
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOVBV:
		return -EINVAL;
	case	MC_ASOC_AUDIO_MODE_PLAY_INCALL:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIO) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALL;
		} else if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE) {
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALL;
		} else {
			return 0;
		...
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_INCALLVB:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIO) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALLVB;
		...
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALL:
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALLVB:
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALL:
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALLVB:
	default:
		return 0;
	}

	return set_audio_mode_play(codec, audio_mode);
}

static int del_audio_mode_play(
	struct snd_soc_codec *codec,
	unsigned int value
)
{
	...
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIO:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIO) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_OFF;
		} else {
			return 0;
		}
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_OFF;
		} else {
		...
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOVBM:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIOVBM) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_OFF;
		} else {
		...
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOVBV:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIOVBV) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_OFF;
		} else {
	...
	case	MC_ASOC_AUDIO_MODE_PLAY_INCALL:
		...
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_INCALLVB:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_INCALLVB) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_OFF;
		} else {
	...
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALL:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIO) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_INCALL;
		} else if (value == MC_ASOC_AUDIO_MODE_PLAY_INCALL) {
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIO;
		} else {
			return 0;
		}
		break;
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIO_INCALLVB:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIO) {
			;
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_INCALLVB;
		} else if (value == MC_ASOC_AUDIO_MODE_PLAY_INCALLVB) {
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIO;
		} else {
	...
	case	MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE_INCALL:
		if (value == MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE) {
			...
			audio_mode	=
				MC_ASOC_AUDIO_MODE_PLAY_AUDIOAE;
		} else {
			return 0;
		}
		break;
...
)
{
	...
	audio_mode	= read_cache(codec, MC_ASOC_AUDIO_MODE_CAP);
	if (audio_mode < 0) {
		;
		return 0;
	}
	...
	switch (audio_mode) {
	case	MC_ASOC_AUDIO_MODE_CAP_OFF:
		audio_mode	= value;
		break;
	case	MC_ASOC_AUDIO_MODE_CAP_AUDIO:
		if (value == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			;
			audio_mode	= MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL;
		} else {
			return 0;
		}
		break;
	case	MC_ASOC_AUDIO_MODE_CAP_AUDIOVBR:
	case	MC_ASOC_AUDIO_MODE_CAP_AUDIOVBV:
		return 0;
	case	MC_ASOC_AUDIO_MODE_CAP_INCALL:
		if (value == MC_ASOC_AUDIO_MODE_CAP_AUDIO) {
			;
			audio_mode	= MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL;
		} else {
			return 0;
		...
		break;
	case	MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL:
	case	MC_ASOC_AUDIO_MODE_CAP_VBOX:
	default:
		return 0;
	}

	return set_audio_mode_cap(codec, audio_mode);
}

static int del_audio_mode_cap(
	struct snd_soc_codec *codec,
	unsigned int value
)
{
	...
	case	MC_ASOC_AUDIO_MODE_CAP_AUDIO:
		if (value == MC_ASOC_AUDIO_MODE_CAP_AUDIO) {
			;
			audio_mode	= MC_ASOC_AUDIO_MODE_CAP_OFF;
		} else {
			return 0;
		}
		break;
	case	MC_ASOC_AUDIO_MODE_CAP_AUDIOVBR:
		if (value == MC_ASOC_AUDIO_MODE_CAP_AUDIOVBR) {
			;
			audio_mode	= MC_ASOC_AUDIO_MODE_CAP_OFF;
		} else {
			return 0;
		...
		break;
	case	MC_ASOC_AUDIO_MODE_CAP_AUDIOVBV:
		if (value == MC_ASOC_AUDIO_MODE_CAP_AUDIOVBV) {
			;
			audio_mode	= MC_ASOC_AUDIO_MODE_CAP_OFF;
		} else {
			return 0;
	...
	case	MC_ASOC_AUDIO_MODE_CAP_AUDIO_INCALL:
		if (value == MC_ASOC_AUDIO_MODE_CAP_AUDIO) {
			;
			audio_mode	= MC_ASOC_AUDIO_MODE_CAP_INCALL;
		} else if (value == MC_ASOC_AUDIO_MODE_CAP_INCALL) {
			audio_mode	= MC_ASOC_AUDIO_MODE_CAP_AUDIO;
		} else {
			return 0;
		}
		break;
	case	MC_ASOC_AUDIO_MODE_CAP_VBOX:
		if (value == MC_ASOC_AUDIO_MODE_CAP_VBOX) {
			;
			audio_mode	= MC_ASOC_AUDIO_MODE_CAP_OFF;
		} else {
			return 0;
...
static int p_output_path;
module_param(p_output_path, int, 444);
MODULE_PARM_DESC(p_output_path, "output path");

static int add_output_path(
	struct snd_soc_codec *codec,
	unsigned int value
)
{
	...
	if (output_path_add == 0) {
		output_path	= value;
	} else {
		switch (output_path) {
		case	MC_ASOC_OUTPUT_PATH_SP:
			if (value == MC_ASOC_OUTPUT_PATH_RC) {
				;
				output_path	= MC_ASOC_OUTPUT_PATH_SP_RC;
			} else if (value == MC_ASOC_OUTPUT_PATH_HP) {
				output_path	= MC_ASOC_OUTPUT_PATH_SP_HP;
			} else if (value == MC_ASOC_OUTPUT_PATH_LO1) {
				output_path	= MC_ASOC_OUTPUT_PATH_SP_LO1;
			} else if (value == MC_ASOC_OUTPUT_PATH_LO2) {
				output_path	= MC_ASOC_OUTPUT_PATH_SP_LO2;
			} else if (value == MC_ASOC_OUTPUT_PATH_BT) {
				output_path	= MC_ASOC_OUTPUT_PATH_SP_BT;
			} else {
				return 0;
			}
		...
		case	MC_ASOC_OUTPUT_PATH_RC:
			if (value == MC_ASOC_OUTPUT_PATH_SP) {
				;
				output_path	= MC_ASOC_OUTPUT_PATH_SP_RC;
			} else if (value == MC_ASOC_OUTPUT_PATH_LO1) {
	...
	ret	= write_cache(codec, MC_ASOC_OUTPUT_PATH, output_path);
	if (ret == 0) {
		p_output_path = output_path;
		output_path_add	= output_path + 1;
		dbg_info("output_path:%d\n",
				read_cache(codec, MC_ASOC_OUTPUT_PATH));
		dbg_info("->output_path_add:%d\n", output_path_add);
	}
	return ret;
...
)
{
	...
	case	MC_ASOC_OUTPUT_PATH_RC:
		...
		break;
	case	MC_ASOC_OUTPUT_PATH_HP:
		if (value == MC_ASOC_OUTPUT_PATH_HP) {
			output_path_add	= 0;
			dbg_info("->output_path_add:%d\n", output_path_add);
			return 0;
		} else {
		...
		break;
	case	MC_ASOC_OUTPUT_PATH_HS:
		if (value == MC_ASOC_OUTPUT_PATH_HS) {
			output_path_add	= 0;
			dbg_info("->output_path_add:%d\n", output_path_add);
			return 0;
		} else {
		...
		break;
	case	MC_ASOC_OUTPUT_PATH_LO1:
		if (value == MC_ASOC_OUTPUT_PATH_LO1) {
			output_path_add	= 0;
			dbg_info("->output_path_add:%d\n", output_path_add);
			return 0;
		} else {
		...
		break;
	case	MC_ASOC_OUTPUT_PATH_LO2:
		if (value == MC_ASOC_OUTPUT_PATH_LO2) {
			output_path_add	= 0;
			dbg_info("->output_path_add:%d\n", output_path_add);
			return 0;
		} else {
		...
		break;
	case	MC_ASOC_OUTPUT_PATH_BT:
		if (value == MC_ASOC_OUTPUT_PATH_BT) {
			output_path_add	= 0;
			dbg_info("->output_path_add:%d\n", output_path_add);
			return 0;
		} else {
	...
	case	MC_ASOC_OUTPUT_PATH_SP_RC:
		if (value == MC_ASOC_OUTPUT_PATH_SP) {
			;
			output_path	= MC_ASOC_OUTPUT_PATH_RC;
		} else if (value == MC_ASOC_OUTPUT_PATH_RC) {
	...
	case	MC_ASOC_OUTPUT_PATH_SP_BT:
		if (value == MC_ASOC_OUTPUT_PATH_SP) {
			;
			output_path	= MC_ASOC_OUTPUT_PATH_BT;
		} else if (value == MC_ASOC_OUTPUT_PATH_BT) {
			output_path	= MC_ASOC_OUTPUT_PATH_SP;
		} else {
			return 0;
...
static int set_input_playback_path(
	struct snd_soc_codec *codec,
	unsigned int reg,
	unsigned int value
)
{
	...
	dbg_info("input_playback_path=%d\n", value);
...
static void karaoke_update_reg(int reg, int data)
{
	struct MCDRV_REG_INFO reg_info;

	reg_info.bRegType = MCDRV_REGTYPE_IF;
	reg_info.bAddress = 3;
	_McDrv_Ctrl(MCDRV_READ_REG, &reg_info, NULL, 0);
	if ((reg_info.bData & 0x02) != 0)
		return;

	reg_info.bRegType = MCDRV_REGTYPE_F;
	reg_info.bAddress = 32;
	_McDrv_Ctrl(MCDRV_READ_REG, &reg_info, NULL, 0);
	if ((reg_info.bData & 0x01) == 0)
		return;

	reg_info.bRegType = MCDRV_REGTYPE_F;
	reg_info.bAddress = reg;
	reg_info.bData = data;
	_McDrv_Ctrl(MCDRV_WRITE_REG, &reg_info, NULL, 0);
	return;
}

static int write_adif_cache(struct snd_soc_codec *codec,
					unsigned int reg, int val)
{
	...
	switch (reg) {
	case	MC_ASOC_ADIF0L_SOURCE:
		l_val = val;
		r_val = read_cache(codec, MC_ASOC_ADIF0R_SOURCE);
		break;
	case	MC_ASOC_ADIF0R_SOURCE:
		l_val = read_cache(codec, MC_ASOC_ADIF0L_SOURCE);
		r_val = val;
		break;
	case	MC_ASOC_ADIF1L_SOURCE:
		l_val = val;
		r_val = read_cache(codec, MC_ASOC_ADIF1R_SOURCE);
		break;
	case	MC_ASOC_ADIF1R_SOURCE:
		l_val = read_cache(codec, MC_ASOC_ADIF1L_SOURCE);
		r_val = val;
		break;
	case	MC_ASOC_ADIF2L_SOURCE:
		l_val = val;
		r_val = read_cache(codec, MC_ASOC_ADIF2R_SOURCE);
		break;
	case	MC_ASOC_ADIF2R_SOURCE:
		l_val = read_cache(codec, MC_ASOC_ADIF2L_SOURCE);
		r_val = val;
		break;
	default:
		return -EINVAL;
	...
	return write_cache(codec, reg, val);
}
	...
	unsigned int value)
{
	...
	if (reg <= MC_ASOC_N_VOL_REG) {
		...
		case	MC_ASOC_OUTPUT_PATH:
			...
			break;
#else
		case	MC_ASOC_AUDIO_MODE_PLAY_ADD:
			err	= add_audio_mode_play(codec, value);
			break;
		case	MC_ASOC_AUDIO_MODE_PLAY_DEL:
			err	= del_audio_mode_play(codec, value);
			break;
		case	MC_ASOC_AUDIO_MODE_CAP_ADD:
			err	= add_audio_mode_cap(codec, value);
			break;
		case	MC_ASOC_AUDIO_MODE_CAP_DEL:
			err	= del_audio_mode_cap(codec, value);
			break;
		case	MC_ASOC_OUTPUT_PATH_ADD:
			err	= add_output_path(codec, value);
			break;
		case	MC_ASOC_OUTPUT_PATH_DEL:
			err	= del_output_path(codec, value);
			break;
#endif
		...
		case	MC_ASOC_ADIF2L_SOURCE:
		case	MC_ASOC_ADIF2R_SOURCE:
			err	= write_adif_cache(codec, reg, value);
			break;
		case	MC_ASOC_ADC0L_SOURCE:
		case	MC_ASOC_ADC0R_SOURCE:
		case	MC_ASOC_ADC1_SOURCE:
			err	= write_cache(codec, reg, value);
			break;
		case	MC_ASOC_INPUT_PLAYBACK_PATH:
			err	= set_input_playback_path(codec, reg, value);
			break;
...
printk("param:%p\n", param);
			...
			err	= _McDrv_Ctrl(MCDRV_SET_DSP, param, NULL, size);
		...
		case	MC_ASOC_SUB_MIC:
			...
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_THIRD_MIC:
			mc_asoc_third_mic	= value;
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_HS_MIC:
			...
			break;
		/* karaoke mixer */
		case	MC_ASOC_K_MIC_EQ_0:
			mc_asoc_kprm[K_MIC_EQ_0] &= ~0x0F;
			mc_asoc_kprm[K_MIC_EQ_0] |= (value & 0x0F);
			karaoke_update_reg(K_MIC_EQ_0_REG,
						mc_asoc_kprm[K_MIC_EQ_0]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_MIC_EQ_1:
			mc_asoc_kprm[K_MIC_EQ_1] &= ~0xF0;
			mc_asoc_kprm[K_MIC_EQ_1] |= ((value << 4) & 0xF0);
			karaoke_update_reg(K_MIC_EQ_1_REG,
						mc_asoc_kprm[K_MIC_EQ_1]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_MIC_EQ_2:
			mc_asoc_kprm[K_MIC_EQ_2] &= ~0x0F;
			mc_asoc_kprm[K_MIC_EQ_2] |= (value & 0x0F);
			karaoke_update_reg(K_MIC_EQ_2_REG,
						mc_asoc_kprm[K_MIC_EQ_2]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_MIC_EQ_3:
			mc_asoc_kprm[K_MIC_EQ_3] &= ~0xF0;
			mc_asoc_kprm[K_MIC_EQ_3] |= ((value << 4) & 0xF0);
			karaoke_update_reg(K_MIC_EQ_3_REG,
						mc_asoc_kprm[K_MIC_EQ_3]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_MIC_EQ_4:
			mc_asoc_kprm[K_MIC_EQ_4] &= ~0x0F;
			mc_asoc_kprm[K_MIC_EQ_4] |= (value & 0x0F);
			karaoke_update_reg(K_MIC_EQ_4_REG,
						mc_asoc_kprm[K_MIC_EQ_4]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_MSC_VOL:
			mc_asoc_kprm[K_MSC_VOL] &= ~0xF8;
			mc_asoc_kprm[K_MSC_VOL] |= ((value << 3) & 0xF8);
			karaoke_update_reg(K_MSC_VOL_REG,
						mc_asoc_kprm[K_MSC_VOL]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_MIC_VOL:
			mc_asoc_kprm[K_MIC_VOL] &= ~0xF0;
			mc_asoc_kprm[K_MIC_VOL] |= ((value << 4) & 0xF0);
			karaoke_update_reg(K_MIC_VOL_REG,
						mc_asoc_kprm[K_MIC_VOL]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_ECH_VOL:
			mc_asoc_kprm[K_ECH_VOL] &= ~0x0F;
			mc_asoc_kprm[K_ECH_VOL] |= (value & 0x0F);
			karaoke_update_reg(K_ECH_VOL_REG,
						mc_asoc_kprm[K_ECH_VOL]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_ECH_RM_TYP:
			mc_asoc_kprm[K_ECH_RM_TYP] &= ~0x07;
			mc_asoc_kprm[K_ECH_RM_TYP] |= (value & 0x07);
			karaoke_update_reg(K_ECH_RM_TYP_REG,
						mc_asoc_kprm[K_ECH_RM_TYP]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_ECH_FB_GIN:
			if (0x07 < value) {
				err = -EINVAL;
				break;
			}
			mc_asoc_kprm[K_ECH_FB_GIN] &= ~0x70;
			mc_asoc_kprm[K_ECH_FB_GIN] |= ((value << 4) & 0x70);
			karaoke_update_reg(K_ECH_FB_GIN_REG,
						mc_asoc_kprm[K_ECH_FB_GIN]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_ECH_FB_DLY:
			if (0x07 < value) {
				err = -EINVAL;
				break;
			}
			mc_asoc_kprm[K_ECH_FB_DLY] &= ~0x07;
			mc_asoc_kprm[K_ECH_FB_DLY] |= (value & 0x07);
			karaoke_update_reg(K_ECH_FB_DLY_REG,
						mc_asoc_kprm[K_ECH_FB_DLY]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_KEY_CTR:
			if (0x0F < value) {
				err = -EINVAL;
				break;
			}
			mc_asoc_kprm[K_KEY_CTR] &= ~0x0F;
			mc_asoc_kprm[K_KEY_CTR] |= (value & 0x0F);
			karaoke_update_reg(K_KEY_CTR_REG,
						mc_asoc_kprm[K_KEY_CTR]);
			write_cache(codec, reg, value);
			break;
		case	MC_ASOC_K_V_CNC:
			if (0x03 < value) {
				err = -EINVAL;
				break;
			}
			mc_asoc_kprm[K_V_CNC] &= ~0xC0;
			mc_asoc_kprm[K_V_CNC] |= ((value << 6) & 0xC0);
			karaoke_update_reg(K_V_CNC_REG,
						mc_asoc_kprm[K_V_CNC]);
			write_cache(codec, reg, value);
			break;
#ifdef MC_ASOC_TEST
...
#ifdef PCM_EVENT_ENBALE
static int codec_dapm_dac(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
	struct snd_soc_codec *codec = card->rtd[0].codec;
	int mode = mc_asoc_read_reg(codec, MC_ASOC_AUDIO_MODE_PLAY);
	...
	case 5: /* incall */
	case 7: /* audio+incall */
	case 9: /* audioae+incall */
		mode = 5;
		break;
	case 6: /* incallvb */
	case 8: /* audio+incallvb */
	case 10: /* audioae+incallvb */
		mode = 6;
		break;
	default:
		/* 0: off, 1: audio, 2: audioae, 3: audiovbm, 4: audiovbv */
		mode = 0;
		break;
	}

	return set_audio_mode_play(codec, mode);
}

static int codec_dapm_adc(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
	struct snd_soc_codec *codec = card->rtd[0].codec;
	int mode = read_cache(codec, MC_ASOC_AUDIO_MODE_CAP);
	...
	case 5: /* audio+incall */
		...
		break;
	default:
		/* 0: off, 1: audio, 2: audiovbr, 3: audiovbv, 6: vbox */
		mode = 0;
		break;
	}

	return set_audio_mode_cap(codec, mode);
}
#endif
...
static const struct snd_soc_dapm_widget	mc_asoc_widgets[] = {
	/* DACs */
#ifdef PCM_EVENT_ENBALE
	SND_SOC_DAPM_DAC_E("DAC DUMMY", "DAC Playback", SND_SOC_NOPM, 0, 0,
		codec_dapm_dac, SND_SOC_DAPM_PRE_PMD),
#else
	SND_SOC_DAPM_DAC("DAC DUMMY", "DAC Playback", SND_SOC_NOPM, 0, 0),
#endif
	/* ADCs */
#ifdef PCM_EVENT_ENBALE
	SND_SOC_DAPM_ADC_E("ADC DUMMY", "ADC Capture", SND_SOC_NOPM, 0, 0,
		codec_dapm_adc, SND_SOC_DAPM_PRE_PMD),
#else
...
#ifdef KERNEL_3_15
	err	= snd_soc_dapm_new_widgets(codec->card);
#else
...
/* VoiceTrigger Para 16k Word(= 32k Byte) until */
#define VOICE_TRIGGER_PARAM_MAX_SIZE 32768

#define VOICE_TRIGGER_REGIST_ONOFF_AEC_POS 62

/* VoiceTrigger AEC header */
static UINT8 aec_vt_param_header[] = {
	/* Head */
	0x41, 0x45, 0x43, 0x05,
	0x00, 0x00, 0x00, 0x3A,
	0x00, 0xfd, 0x00, 0x00,
...
/* VoiceTrigger AEC regist on/off */
static UINT8 aec_vt_param_regist[] = {
	/* Head */
	0x41, 0x45, 0x43, 0x05,
	0x00, 0x00, 0x00, 0x46,
	0x00, 0xfd, 0x00, 0x00,
...
static void modify_chunk_size(UINT8 *aec, unsigned long size)
{
	aec[0] = (UINT8)((size >> 24) & 0xff);
	aec[1] = (UINT8)((size >> 16) & 0xff);
	aec[2] = (UINT8)((size >> 8) & 0xff);
	aec[3] = (UINT8)((size >> 0) & 0xff);
}

static void set_aec_fix(UINT8 *aec, int size, bool scramble, int addr)
{
	modify_chunk_size(aec + 4, size + 0x3A); /* total size */
	modify_chunk_size(aec + 14, size + 0x30); /* D7 chunk size */
	modify_chunk_size(aec + 22, size + 0x28); /* V-BOX chunk size */
	modify_chunk_size(aec + 45, size + 0x11); /* C-DSP chunk size */
	modify_chunk_size(aec + 53, size + 0x09); /* data memory chunk size */

	if (scramble)
		*(aec + 57) = 1; /* scramble */

	modify_chunk_size(aec + 58, addr); /* address */
	modify_chunk_size(aec + 62, size >> 1); /* data size (word) */
}

static int set_vt_param(struct ymc_ctrl_args *ymc_ctrl_arg)
{
	int	err	= 0;
	UINT8	*param	= NULL;

	/* create aec header */
	set_aec_fix(aec_vt_param_header, ymc_ctrl_arg->size,
			false, 0x1000);

	/* allocate buffer */
	param	= get_dsp_mem(sizeof(aec_vt_param_header)
			+ ymc_ctrl_arg->size);
	if (param == NULL) {
		err	= -ENOMEM;
		return err;
	...
	/* set aec header */
	memcpy(param, aec_vt_param_header, sizeof(aec_vt_param_header));

	/* set aec dsp data */
	if (copy_from_user(param + sizeof(aec_vt_param_header),
		ymc_ctrl_arg->param, ymc_ctrl_arg->size) == 0) {

		/* regist on */
		aec_vt_param_regist[
			VOICE_TRIGGER_REGIST_ONOFF_AEC_POS] = 1;
		err	= _McDrv_Ctrl(MCDRV_SET_DSP,
					aec_vt_param_regist,
					NULL,
					sizeof(aec_vt_param_regist));
		err	= map_drv_error(err);
		if (err != 0) {
			free_dsp_mem(param);
			return err;
		...
		err	= _McDrv_Ctrl(MCDRV_SET_DSP, param, NULL,
				sizeof(aec_vt_param_header) +
				ymc_ctrl_arg->size);
		err	= map_drv_error(err);
		if (err != 0) {
			/* regist off */
			aec_vt_param_regist[
				VOICE_TRIGGER_REGIST_ONOFF_AEC_POS] = 0;
			_McDrv_Ctrl(MCDRV_SET_DSP,
				aec_vt_param_regist, NULL,
				sizeof(aec_vt_param_regist));
			free_dsp_mem(param);
			return err;
		}
	...
	unsigned long arg)
{
	...
	case YMC_IOCTL_SET_CTRL_VT:
		...
		if (ymc_ctrl_arg.size > VOICE_TRIGGER_PARAM_MAX_SIZE) {
			err	= -EINVAL;
			break;
		}
...
#ifdef SW_DRV_VOICE_TRIGGER
#define VOICE_TRIGGER_CALLBACK_WAKEUP   (0x40)
#define VOICE_TRIGGER_CALLBACK_COMMAND  (0x41)
#define VOICE_TRIGGER_CALLBACK_ERROR	(0x42)
#define VOICE_TRIGGER_CALLBACK_TIMEOUT  (0x43)

#define VOICE_TRIGGER_CALLBACK_ANDROID_WAKEUP   (0x1000)
#define VOICE_TRIGGER_CALLBACK_ANDROID_COMMAND  (0x2000)
#define VOICE_TRIGGER_CALLBACK_ANDROID_ERROR	(0x3000)
#define VOICE_TRIGGER_CALLBACK_ANDROID_TIMEOUT  (0x4000)

static struct mutex	dsp_mutex;
#endif

#ifdef SW_DRV_VOICE_TRIGGER
static struct switch_dev	*voice_trigger_sdev;
static ssize_t voice_trigger_print_name(struct switch_dev *sdev, char *buf)
{
	int state = switch_get_state(sdev);

	switch (state & 0xF000) {
	case 0:
		return sprintf(buf, "Initial\n");
	case VOICE_TRIGGER_CALLBACK_ANDROID_WAKEUP:
		return sprintf(buf, "Wakeup: %d\n", state & 0x0FFF);
	case VOICE_TRIGGER_CALLBACK_ANDROID_COMMAND:
		return sprintf(buf, "Command: %d\n", state & 0x0FFF);
	case VOICE_TRIGGER_CALLBACK_ANDROID_ERROR:
		return sprintf(buf, "Error: %d\n", state & 0x0FFF);
	case VOICE_TRIGGER_CALLBACK_ANDROID_TIMEOUT:
		return sprintf(buf, "Timeout\n");
	default:
		return sprintf(buf, "%d state unknown\n", state);
	}
	return -EINVAL;
}
#endif	/*	SW_DRV_VOICE_TRIGGER	*/
...
static void hsdet_cb(UINT32 dFlags, struct MCDRV_HSDET_RES *psRes)
{
	...
	if (dFlags & MCDRV_HSDET_EVT_PLUGUNDET_DB_FLAG) {
		if (bCurEnPlugDetDb & MCDRV_PLUGDETDB_UNDET_ENABLE) {
			...
			err	= _McDrv_Ctrl(MCDRV_SET_HSDET,
					(void *)&stHSDetInfo, NULL, 0x010000FE);
			if (err < MCDRV_SUCCESS)
...
exit:
	...
	if (jack_status != mc_asoc_jack_status) {
		...
		if ((jack_status == SND_JACK_HEADPHONE)
		&& (mc_asoc_jack_status == SND_JACK_HEADSET)) {
			/*	Headphone->Headset	*/
			snd_soc_jack_report(&hs_jack, 0, SND_JACK_HEADSET);
		}
...
#endif  //#if !defined(CONFIG_OEM_DEFINED)

#ifdef SW_DRV_VOICE_TRIGGER
static SINT32 dsp_cb(SINT32 sdHd, UINT32 dEvtType, UINT32 dEvtPrm)
{
	struct MCDRV_DSP_PARAM	stDspParam;
	UINT8 abBuff[17];
	int	err;
	...
	dbg_info("sdHd = %ld, dEvtType=%ld, dEvtPrm=%ld\n",
		sdHd, dEvtType, dEvtPrm);
	if ((sdHd == 0) && (dEvtType == MCDRV_CDSP_EVT_PARAM)) {
		stDspParam.dType = MCDRV_DSP_PARAM_CDSP_FUNCA;
		stDspParam.dInfo = 0;
		err	= _McDrv_Ctrl(
			MCDRV_GET_DSP, (void *)&stDspParam, (void*)abBuff, 0);
		if (err < MCDRV_SUCCESS) {
			pr_err("%d: Error in MCDRV_GET_DSP\n", err);
			goto exit;
		}
		dbg_info("abBuff[0]=%X, abBuff[1]=%X\n",
			abBuff[0], abBuff[1]);

		switch (abBuff[0]) {
		case VOICE_TRIGGER_CALLBACK_WAKEUP:
			switch_set_state(voice_trigger_sdev, 0);
			switch_set_state(
				voice_trigger_sdev,
				(VOICE_TRIGGER_CALLBACK_ANDROID_WAKEUP |
					abBuff[1])
			);
			break;
		case VOICE_TRIGGER_CALLBACK_COMMAND:
			switch_set_state(voice_trigger_sdev, 0);
			switch_set_state(
				voice_trigger_sdev,
				(VOICE_TRIGGER_CALLBACK_ANDROID_COMMAND |
					abBuff[1])
			);
			break;
		case VOICE_TRIGGER_CALLBACK_ERROR:
			pr_err(
				"%d: Error in VOICE_TRIGGER_CALLBACK_ERROR\n",
				abBuff[1]);
			switch_set_state(voice_trigger_sdev, 0);
			switch_set_state(
				voice_trigger_sdev,
				(VOICE_TRIGGER_CALLBACK_ANDROID_ERROR |
					abBuff[1])
			);
			break;
		case VOICE_TRIGGER_CALLBACK_TIMEOUT:
			switch_set_state(voice_trigger_sdev, 0);
			switch_set_state(
				voice_trigger_sdev,
				VOICE_TRIGGER_CALLBACK_ANDROID_TIMEOUT
			);
			break;
		default:
			pr_err("0x%X: unknown callback command.\n", abBuff[0]);
			break;
		}
	}

exit:
	mc_asoc_unlock("dsp_cb");
...
#ifdef DEVICE_TREE_USE
static void set_pdata(struct snd_soc_codec *codec)
{
	struct mc_asoc_data *mc_asoc = NULL;
	struct snd_soc_card *soc_card = NULL;
	...
	soc_card = codec->card;
	if (soc_card != NULL)
		mc_asoc->pdata = snd_soc_card_get_drvdata(soc_card);
}
#endif
...
)
{
...
#if (BUS_SELECT == BUS_SEL_SPI) || (BUS_SELECT == BUS_SEL_I2C)
	ymu831_set_ldod(); 	//hsguy.son (add)
	...
	write_cache(codec, MC_ASOC_VOICE_PLAY_VOL_ASGN, 1);
	write_cache(codec, MC_ASOC_VOICE_REC_VOL_ASGN, 8);
	write_cache(codec, MC_ASOC_INPUT_PLAY_DIG_VOL_ASGN, 4);

	write_cache(codec, MC_ASOC_ADIF0R_SOURCE, 0);
	write_cache(codec, MC_ASOC_ADIF1R_SOURCE, 0);
	write_cache(codec, MC_ASOC_ADIF2R_SOURCE, 0);
	...
	write_cache(codec, MC_ASOC_THIRD_MIC, mc_asoc_third_mic);
...
#ifdef SW_DRV_VOICE_TRIGGER
	voice_trigger_sdev	= kzalloc(sizeof(struct switch_dev),
							GFP_KERNEL);
	voice_trigger_sdev->name	= "voice_trigger";
	voice_trigger_sdev->print_name	= voice_trigger_print_name;
	err	= switch_dev_register(voice_trigger_sdev);
	if (err < 0) {
		dev_err(dev, "%d: Error in switch_dev_register\n", err);
		goto error_set_mode;
	}
...
#ifdef SW_DRV_VOICE_TRIGGER
	err	= _McDrv_Ctrl(MCDRV_REGISTER_DSP_CB, &dsp_cb, NULL, 0);
	if (err != MCDRV_SUCCESS) {
		dev_err(dev, "%d: Error in MCDRV_REGISTER_DSP_CB\n", err);
		goto error_set_mode;
	}
#endif
	...
	hold_ldod	= 0;
#endif
	set_audio_mode_play(codec, 2); 	//hsguy.son (add)
...
#ifdef SW_DRV_VOICE_TRIGGER
	if (voice_trigger_sdev != NULL) {
		switch_dev_unregister(voice_trigger_sdev);
		kfree(voice_trigger_sdev);
		voice_trigger_sdev	= NULL;
	}
#endif
...
)
{
	...
	err	= _McDrv_Ctrl(MCDRV_SET_HSDET,
			(void *)&stHSDetInfo, NULL, 0x3fffffff);
	if (err != MCDRV_SUCCESS) {
		dev_err(codec->dev, "%d: Error in mc_asoc_suspend\n", err);
		err	= -EIO;
		goto error;
...
)
{
	...
	err	= _McDrv_Ctrl(MCDRV_SET_HSDET,
			(void *)&mc_asoc->hsdet_store, NULL, 0x3fffffff);
	if (err != MCDRV_SUCCESS) {
		dev_err(codec->dev, "%d: Error in mc_asoc_resume\n", err);
		err	= -EIO;
		goto error;
	...
	const struct i2c_device_id *id)
{
	...
	mc_asoc->pdata	=
		(struct mc_asoc_platform_data *)client->dev.platform_data;
...
void mc_asoc_set_codec_ldod(int status)
{
	...
	if ((mc_asoc != NULL)
	&& (mc_asoc->pdata != NULL)
	&& (mc_asoc->pdata->set_codec_ldod != NULL)) {
...
static const struct of_device_id mc_asoc_i2c_of_match_a[] = {
	{ .compatible = "yamaha,ymu831_a" },
	{},
};
MODULE_DEVICE_TABLE(of, mc_asoc_i2c_of_match_a);

static const struct of_device_id mc_asoc_i2c_of_match_d[] = {
	{ .compatible = "yamaha,ymu831_b" },
	{},
};
MODULE_DEVICE_TABLE(of, mc_asoc_i2c_of_match_d);
...
static struct i2c_driver	mc_asoc_i2c_driver_a = {
	.driver	= {
		.name	= "ymu831_a",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(mc_asoc_i2c_of_match_a),
	},
...
static struct i2c_driver	mc_asoc_i2c_driver_d = {
	.driver	= {
		.name	= "ymu831_d",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(mc_asoc_i2c_of_match_d),
	},
	...
	UINT32 dSize)
{
	...
	for (i = 0; i < dSize; i++)
		pbData[i]	= i2c_smbus_read_byte_data(i2c, dAddress << 1);
...
//hsguy.son (add)
static const struct of_device_id mc_asoc_spi_of_match[] = {
	{ .compatible = "yamaha,ymu831" },
	{},
};
MODULE_DEVICE_TABLE(of, mc_asoc_spi_of_match);
	...
	UINT32 dSize)
{
	...
	if ((dSize+3) > sizeof(buf)) {
		rx	= kmalloc(dSize+3, GFP_KERNEL);
		if (rx == NULL) {
...
static struct spi_driver	mc_asoc_spi_driver = {
	.driver = {
		...
		.name	= "spi0.0",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(mc_asoc_spi_of_match), 	//hsguy.son (add)
	},
...
#if defined(CONFIG_SND_SOC_MSM8960)
		sh_ch.rx[port_idx].ch_num	= port_idx + BASE_CH_NUM + 10;
#elif defined(CONFIG_SND_SOC_MSM8974)
		sh_ch.rx[port_idx].ch_num	= port_idx + BASE_CH_NUM + 16;
#endif
	...
	if (ret < 0) {
		...
		for (port_idx = 0; port_idx < MAX_PORTS; port_idx++) {
			...
			sh_ch.rx[port_idx].ch_num	=
						port_idx + BASE_CH_NUM + 10;
#elif defined(CONFIG_SND_SOC_MSM8974)
			sh_ch.rx[port_idx].ch_num	=
						port_idx + BASE_CH_NUM + 16;
#endif
