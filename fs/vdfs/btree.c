...
static int check_bnode_offset_area(struct emmcfs_bnode *bnode)
{
	...
	for (i = 0; i < recs_count; i++) {
		offset = get_offset(bnode, (unsigned)i);
 ...
 *			index
 */
static void *get_record(struct emmcfs_bnode *bnode, int index)
{
	void *ret;
	emmcfs_bt_off_t offset;
	...
	offset = get_offset(bnode, (unsigned)index);
		...
		emmcfs_bt_off_t new_val)
{
	...
	if (IS_ERR(offset) || !offset) {
		vdfs_dump_panic_remount(bnode, "offset is wrong %llu",
				offset);
		return -EFAULT;
	}

	if (!is_offset_correct(bnode, new_val)) {
		vdfs_dump_panic_remount(bnode, "new offset is wrong %llu",
				new_val);
		return -EFAULT;
	}
	*offset = new_val;
...
void bnode_sanity_check(struct emmcfs_bnode *bnode)
{
	...
	if (record_count == 0) {
		if (get_offset(bnode, 0) !=
				sizeof(struct vdfs_gen_node_descr))
			vdfs_dump_panic_remount(bnode,
					"zero offset is broken");
		return;
	}

	if (le16_to_cpu(bnode_desc->free_space) > max_bnode_size)
		vdfs_dump_panic_remount(bnode, "free is bigger than bnode %u",
			bnode_desc->free_space);
	if (record_count > 1000)
		vdfs_dump_panic_remount(bnode, "records count toooo big %d",
			record_count);

	for (count = 0; count < record_count; count++) {
		offset = get_offset(bnode, (unsigned)count);
		if (!is_offset_correct(bnode, offset)) {
			vdfs_dump_panic_remount(bnode,
	...
	if (offset != get_offset(bnode, (unsigned)record_count))
		vdfs_dump_panic_remount(bnode,
				"free space offset is broken %u",
				offset);
}
struct __emmcfs_bnode {
	struct emmcfs_bnode bnode;
	struct page *page;
};
		...
		pgoff_t index)
{
	...
	struct __emmcfs_bnode node;
	struct emmcfs_bnode *bnode = &node.bnode;

	memset(&node, 0x0, sizeof(struct __emmcfs_bnode));
		...
		struct vdfs_btree *btree, __u32 bnode_id, __u32 pos)
{
	...
	raw_record = emmcfs_get_btree_record(bnode, (int)pos);
	if (IS_ERR(raw_record)) {
		err_ret = raw_record;
		goto err_exit_put;
	}

	form_gen_record(bnode, raw_record, rec_info, (int)pos);
		...
		enum emmcfs_node_type type)
{
	...
	node_descr->magic[0] = (u8)(EMMCFS_NODE_DESCR_MAGIC & 0xff);
	node_descr->magic[1] = (u8)((EMMCFS_NODE_DESCR_MAGIC >> 8) & 0xff);
		...
		void *new_record, int insert_pos)
{
	...
	if (moving_offsets_num > 0)
		memmove(offset, (void *) (offset + 1),
			(unsigned)moving_offsets_num * sizeof(emmcfs_bt_off_t));
	/* Put new offset */
		...
		int insert_pos, int place_only_key)
{
	...
	/* Put new offset */
	err = check_set_offset(bnode, (unsigned)insert_pos,
			(emmcfs_bt_off_t)((char *)new_record_ptr
					- (char *)node_descr_ptr));
	if (err)
		return ERR_PTR(err);
	...
	free_space -= (unsigned)new_rec_len + sizeof(emmcfs_bt_off_t);
...
static int delete_from_node(struct emmcfs_bnode *bnode, int del_index)
{
	...
	del_offset = get_offset(bnode, (unsigned)del_index);

	if (!is_offset_correct(bnode, free_space_offset) ||
			!is_offset_correct(bnode, del_offset)) {
		vdfs_dump_panic_remount(bnode, "free space offset is broken %u",
				free_space_offset);
		return -EFAULT;
	}
	...
	for (i = 0; i < node->recs_count; i++) {
		emmcfs_bt_off_t cur_offs = get_offset(bnode, (unsigned)i);
	...
	for (i = del_index; i < node->recs_count; i++) {
		emmcfs_bt_off_t next_offset = get_offset(bnode, (unsigned)i +
				1lu);
	...
	node->free_space = (u16)(node->free_space +
		rec_len + sizeof(emmcfs_bt_off_t));
...
#if defined(CONFIG_VDFS_DEBUG)
{
	...
	EMMCFS_BUG_ON(node->free_space != (char *)offset
		...
		int insert_pos, struct vdfs_btree_record_info *res_rec_info)
{
	...
	int bnode_type = (int)le32_to_cpu(EMMCFS_BNODE_DSCR(left_bnode)->type);
	int recs_count = (int)EMMCFS_BNODE_RECS_NR(left_bnode);
		...
		struct list_head *todo_list_head, int force_insert)
{
	...
	struct emmcfs_bnode  *right_bnode;
	void *err_ret;
	struct emmcfs_generic_key new_gen_key;
...
#ifndef USER_SPACE
		ret = check_bnode_reserve(btree, force_insert);
		if (ret) {
			err_ret = ERR_PTR(ret);
			goto err_exit;
		}
...
static int build_new_root(struct vdfs_btree *btree, __u32 current_root_id)
{
	...
	while (curr_bnode) {
		...
		new_rec_len = (int)(VDFS_KEY_LEN(key) + sizeof(*index_val));
		...
		struct list_head *todo_list_head, int force_insert)
{
	...
	new_rec_len = (int)(VDFS_KEY_LEN(gen_key) + sizeof(*index_val));
...
static struct vdfs_btree_gen_record *
place_data_at_level(struct vdfs_btree *btree,
		struct emmcfs_generic_key *gen_key, int level, int force_insert)
{
	struct emmcfs_bnode *bnode;
	...
	record = place_key_in_bnode(traverse_data, &bnode, pos, gen_key,
			VDFS_RECORD_LEN(gen_key), &todo_list, force_insert);
	if (IS_ERR(record))
		emmcfs_put_bnode(bnode);

	while (!list_empty(&todo_list)) {
		int err;
		struct vdfs_btree_todo_list *current_task;
		...
		struct emmcfs_bnode *point_to, int level)
{
	...
	err = btree_insert_at_level(tree, new_data, level, 0);
	if (err)
		goto error_exit;
 ...
 */
static int btree_merge(struct vdfs_btree *tree, unsigned int left_bnode_id,
		unsigned int right_bnode_id, int level, int is_right_dangling)
{
	...
	if (check_bnode_offset_area(left_bnode) ||
		...
		if (check_bnode_offset_area(left_bnode))
			vdfs_dump_panic_remount(left_bnode, "offset area brk");
		if (check_bnode_offset_area(right_bnode))
			vdfs_dump_panic_remount(right_bnode, "offset area brk");
	...
	data_start = (char *)right + sizeof(struct vdfs_gen_node_descr);
	...
	for (i = 0; i <= right->recs_count; i++) {
		...
		emmcfs_bt_off_t record_offset = start_offset +
			get_offset(right_bnode, (unsigned int)i) -
			sizeof(struct vdfs_gen_node_descr);

		if (check_set_offset(left_bnode, record_index,
					record_offset)) {
			vdfs_dump_panic_remount(left_bnode,
					"offset is wrong %d - %u",
	...
	left->free_space = (u16)(left->free_space - (get_offset(right_bnode,
					right->recs_count) -
					sizeof(struct vdfs_gen_node_descr)) -
					right->recs_count *
					sizeof(emmcfs_bt_off_t));
	...
	*/
	...
	if (node->free_space > tree->node_size_bytes / 3) {
		if (bnode->node_id != emmcfs_btree_get_root_id(tree)) {
			...
			unsigned int node_fs = ((struct vdfs_gen_node_descr *)
