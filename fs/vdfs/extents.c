		...
		struct vdfs_extent_info *extent, int force_insert)
{
	...
	BUILD_BUG_ON(!IS_ALIGNED(sizeof(struct emmcfs_exttree_key), 8));
	BUILD_BUG_ON(!IS_ALIGNED(sizeof(struct emmcfs_exttree_record), 8));
	...
	ret = emmcfs_btree_insert(sbi->extents_tree, record, force_insert);
		...
		sector_t iblock, struct vdfs_extent_info *result)
{
	...
	tree = (inode_info->installed_btrees) ?
			inode_info->installed_btrees->ext_tree :
			sbi->extents_tree;
		...
		int force_insert)
{
	...
	ret = emmcfs_exttree_add(sbi, object_id, extent, force_insert);
