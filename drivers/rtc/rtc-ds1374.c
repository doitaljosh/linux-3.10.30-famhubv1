...
static void ds1374_wd_work_handler(struct work_struct *w);
...
static int ds1374_check_rtc_status(struct i2c_client *client)
{
	...
	if (stat & DS1374_REG_SR_OSF) {
		...
		ds1374->boot_flag = FALSE;
		rtc_time_to_tm(0,&tm);
		ds1374_set_time(&client->dev, &tm);
...
static int ds1374_wdt_settimeout(unsigned int timeout)
{
	...
	if (ret) {
		pr_err("rtc-ds1374 - couldn't set new watchdog time\n");
		goto out;
	}
...
static void ds1374_wd_work_handler(struct work_struct *w)
{
	...
	ret = schedule_delayed_work(&ds1374->wd_work, msecs_to_jiffies(5000));
	if (ret < 0) {
		dev_err(&ds1374->client->dev, "Failed to schedule watchdog workqueue");
		return;
	}

	ret = ds1374_wdt_settimeout(100000);
	if (ret < 0) {
		dev_err(&ds1374->client->dev, "Failed to set watchdog timeout");
		return;
	}
									...
									unsigned long code, void *unused)
{
	if (code == SYS_DOWN || code == SYS_HALT) {
		...
		/* enable Watchdog for specific system */
		ds1374_wdt_settimeout(3000);
#else
		/* Disable Watchdog */
		ds1374_wdt_disable();
#endif
						...
						const struct i2c_device_id *id)
{
	...
	/* set charge register one diode, 2kOhm resistor */ 
	
	ret = i2c_smbus_write_byte_data(client,DS1374_REG_TCR, chargeset);
	if(ret < 0 )
	{
		dev_err(&client->dev,"register tcr setting fail \n");
		return ret;

	}
#ifdef CONFIG_RTC_DRV_DS1374_WDT
	save_client = ds1374->client;
    ...
    INIT_DELAYED_WORK(&ds1374->wd_work, ds1374_wd_work_handler);
    ret = schedule_delayed_work(&ds1374->wd_work, msecs_to_jiffies(5000));
    if (ret < 0) {
        dev_err(&client->dev, "Failed to schedule watchdog workqueue");
        return ret;
    }
...
static int ds1374_remove(struct i2c_client *client)
{
    ...
    cancel_delayed_work_sync(&ds1374->wd_work);
