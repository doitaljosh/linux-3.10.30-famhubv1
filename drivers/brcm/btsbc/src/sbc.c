	...
	for (blk = 0; blk < frame->blocks; blk++) {
		for (ch = 0; ch < frame->channels; ch++) {
			for (sb = 0; sb < frame->subbands; sb++) {
				...
				/* pcoupe  replace with uint32_t do_div(uint64_t dividend, uint32_t divisor) */
//                frame->sb_sample[blk][ch][sb] = (int32_t)
//                    (((((uint64_t) audio_sample << 1) | 1) << shift) /
//                    levels[ch][sb]) - (1 << shift);
//				frame->sb_sample[blk][ch][sb] = do_div(((((uint64_t) audio_sample << 1) | 1) << shift), levels[ch][sb]);
				{
				    uint64_t tmp_u64 = ((((uint64_t) audio_sample << 1) | 1) << shift);
	                frame->sb_sample[blk][ch][sb] = do_div(tmp_u64, levels[ch][sb]) - (1 << shift);
				}
			}
		}
	}
...
SBC_EXPORT int sbc_init(sbc_t *sbc, unsigned long flags)
{
	...
	sbc->priv_alloc_base = kmalloc(sizeof(struct sbc_priv) + SBC_ALIGN_MASK, GFP_KERNEL);
			...
			void *output, size_t output_len, ssize_t *written)
{
	...
	if (!priv->init) {
		...
		printk("PCM Frame size=%d SBC Frame Size=%d\n", priv->frame.codesize, priv->frame.length);
