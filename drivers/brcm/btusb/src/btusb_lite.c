...
static ssize_t btusb_lite_read(struct file *file, char __user *p_user_buffer, size_t count, loff_t *p_off)
{
    ...
    rv = wait_event_interruptible(p_dev->rx_wait_q,
            (((p_msg = btusb_lite_msg_to_app_get(p_dev)) != NULL)) || (p_dev->bt_exit_state == 1));
    ...
    if(p_dev->bt_exit_state == 1)
    {
    	BTUSB_ERR("BTUSB Exiting\n");
 ...
 **
 *******************************************************************************/
void btusb_lite_ipc_rsp_send(struct btusb *p_dev,
