...
#define SAMSUNG_PATCH_STATE_BTUSB	1
#endif
...
#ifdef SAMSUNG_PATCH_STATE_BTUSB			// HawkM
struct {
	int suspend;
	wait_queue_head_t btusb_wait_q;
}bt_state;
#endif
...
int btusb_submit(struct btusb *p_dev, struct usb_anchor *p_anchor, struct urb *p_urb, int mem_flags)
{
	...
	if (bt_state.suspend == BTUSB_SUSPENDING) {
		BTUSB_ERR(KERN_ERR"\nBTUSB %s: urb=%p..urb trying to submit during suspend\n",__func__, p_urb);
	}
#endif	
    ...
    if (unlikely(status))
    {
    	...
    	{
    		nodev++;
    		BTUSB_ERR("usb_submit_urb failed(%d)-nodev(%d)\n", status, nodev);
    	}
    	BTUSB_ERR("Submission Fail:%d\n", status);
...
no_dfu:
    ...
    BTUSB_ARRAY_FOR_EACH_RX_TRANS(p_dev->event_array)
    {
        ...
        {
    	    spin_unlock(&p_dev->event_array_lock);        
            BTUSB_ERR("transaction allocation failed\n");
            goto error_claim;
        }
    ...
    BTUSB_ARRAY_FOR_EACH_TX_TRANS(p_dev->acl_tx_array)
    {
        ...
        if (!p_tx_trans->dma_buffer || !p_urb)
        {
    	    spin_unlock(&p_dev->acl_tx_array_lock);        
            BTUSB_ERR("transaction allocation failed\n");
            goto error_claim;
        }
    ...
    if (p_dev->p_diag_in)
    {
    	spin_lock(&p_dev->diag_rx_array_lock);        
        BTUSB_ARRAY_FOR_EACH_RX_TRANS(p_dev->diag_rx_array)
        {
            ...
            if (!p_rx_trans->dma_buffer || !p_urb)
            {
    		spin_unlock(&p_dev->diag_rx_array_lock);        
                BTUSB_ERR("transaction allocation failed\n");
                goto error_claim;
            }
    ...
    if (p_dev->p_diag_out)
    {
    	spin_lock(&p_dev->diag_tx_array_lock);        
        BTUSB_ARRAY_FOR_EACH_TX_TRANS(p_dev->diag_tx_array)
        {
            ...
            if (!p_tx_trans->dma_buffer || !p_urb)
            {
    		spin_unlock(&p_dev->diag_tx_array_lock);        
                BTUSB_ERR("transaction allocation failed\n");
                goto error_claim;
            }
    ...
    spin_lock(&p_dev->voice_rx_array_lock);        
    BTUSB_ARRAY_FOR_EACH_RX_TRANS(p_dev->voice_rx_array)
    {
        ...
        if (!p_rx_trans->dma_buffer || !p_urb)
        {
    	    spin_unlock(&p_dev->voice_rx_array_lock);        
            BTUSB_ERR("transaction allocation failed\n");
            goto error_claim;
        }
    ...
    spin_lock(&p_dev->voice_tx_array_lock);        
...
int btusb_suspend(struct usb_interface *p_interface, pm_message_t message)
{
    ...
    /* Block all the URBs submission now*/
#ifdef SAMSUNG_PATCH_STATE_BTUSB
	bt_state.suspend = BTUSB_SUSPENDING;
#endif

    if (p_dev->p_main_intf == p_interface)
    {
        BTUSB_ERR("Suspending mainInterface\n");
        BTUSB_DBG("main interface (%p)\n",p_dev->p_main_intf);
        /* stop reading data */
        BTUSB_ERR("Removing ACL RX anchored URBs\n");
        //usb_kill_anchored_urbs(&p_dev->acl_rx_submitted);
        //Added by Abhishek
        anchor = &p_dev->acl_rx_submitted;
        BTUSB_ARRAY_FOR_EACH_RX_TRANS(p_dev->acl_rx_array)
        {
        	spin_lock_irq(&anchor->lock);
    		usb_get_urb(p_rx_trans->p_urb);
    		spin_unlock_irq(&anchor->lock);
    		 /*this will unanchor the URB*/
    		usb_kill_urb(p_rx_trans->p_urb);
    		usb_put_urb(p_rx_trans->p_urb);
        }

        /* stop reading events */
        BTUSB_ERR("Removing EVENT anchored URBs\n");
        //usb_kill_anchored_urbs(&p_dev->event_submitted);
        //Added by Abhishek
        anchor = &p_dev->event_submitted;
        BTUSB_ARRAY_FOR_EACH_RX_TRANS(p_dev->event_array)
        {
        	spin_lock_irq(&anchor->lock);
    		usb_get_urb(p_rx_trans->p_urb);
        ...
        /* cancel ACL writes */
        BTUSB_ERR("Removing ACL TX anchored URBs\n");
        //usb_kill_anchored_urbs(&p_dev->acl_tx_submitted);
        //Added by Abhishek
        anchor = &p_dev->acl_tx_submitted;
        BTUSB_ARRAY_FOR_EACH_TX_TRANS(p_dev->acl_tx_array)
        {
        	spin_lock_irq(&anchor->lock);
    		usb_get_urb(p_tx_trans->p_urb);
    		spin_unlock_irq(&anchor->lock);
    		 /*this will unanchor the URB*/
    		usb_kill_urb(p_tx_trans->p_urb);
    		usb_put_urb(p_tx_trans->p_urb);
        }

        /* cancel commands */
        BTUSB_ERR("Removing CMD anchored URBs\n");
        //usb_kill_anchored_urbs(&p_dev->cmd_submitted);
        //Added by Abhishek
        anchor = &p_dev->cmd_submitted;
        BTUSB_ARRAY_FOR_EACH_TX_TRANS(p_dev->cmd_array)
        {
        	spin_lock_irq(&anchor->lock);
    		usb_get_urb(p_tx_trans->p_urb);
    ...
    {
    	BTUSB_ERR("Suspending voiceInterface\n");
        BTUSB_DBG("voice interface (%p)\n",p_dev->p_voice_intf);

        BTUSB_ERR("Removing VOICE TX anchored URBs\n");
        //usb_kill_anchored_urbs(&p_dev->voice_tx_submitted);
        //Added by Abhishek
        anchor = &p_dev->voice_tx_submitted;
        BTUSB_ARRAY_FOR_EACH_TX_TRANS(p_dev->voice_tx_array)
        {
        	spin_lock_irq(&anchor->lock);
    		usb_get_urb(p_tx_trans->p_urb);
        ...
        BTUSB_ERR("Removing VOICE RX anchored URBs\n");
        //usb_kill_anchored_urbs(&p_dev->voice_rx_submitted);
        //Added by Abhishek
        anchor = &p_dev->voice_rx_submitted;
        BTUSB_ARRAY_FOR_EACH_RX_TRANS(p_dev->voice_rx_array)
        {
        	spin_lock_irq(&anchor->lock);
    		usb_get_urb(p_rx_trans->p_urb);
    ...
    {
    	BTUSB_ERR("Suspending dfuInterface\n");
        BTUSB_DBG("dfu interface (%p)\n",p_dev->p_dfu_intf);
    }
    if (p_dev->p_diag_intf == p_interface)
    {
    	BTUSB_ERR("Suspending diagInterface\n");
        BTUSB_DBG("diag interface (%p)\n",p_dev->p_diag_intf);

        /* stop reading diags */
        BTUSB_ERR("Removing DIAG RX anchored URBs\n");
        //usb_kill_anchored_urbs(&p_dev->diag_rx_submitted);
        //Added by Abhishek
        anchor = &p_dev->diag_rx_submitted;
        BTUSB_ARRAY_FOR_EACH_RX_TRANS(p_dev->diag_rx_array)
        {
        	spin_lock_irq(&anchor->lock);
    		usb_get_urb(p_rx_trans->p_urb);
        ...
        /* cancel diags */
        BTUSB_ERR("Removing DIAG TX anchored URBs\n");
        //usb_kill_anchored_urbs(&p_dev->diag_tx_submitted);
        //Added by Abhishek
        anchor = &p_dev->diag_tx_submitted;
        BTUSB_ARRAY_FOR_EACH_TX_TRANS(p_dev->diag_tx_array)
        {
        	spin_lock_irq(&anchor->lock);
    		usb_get_urb(p_tx_trans->p_urb);
        ...
        const struct usb_device_id *p_id)
{
    ...
    pr_err("%s:initiating spin_locks for btusb-endpoint buffers\n",__func__);
    spin_lock_init(&p_dev->cmd_array_lock);
    spin_lock_init(&p_dev->event_array_lock);
    spin_lock_init(&p_dev->acl_rx_array_lock);
    spin_lock_init(&p_dev->acl_tx_array_lock);
    spin_lock_init(&p_dev->diag_rx_array_lock);
    spin_lock_init(&p_dev->diag_tx_array_lock);
    spin_lock_init(&p_dev->voice_rx_array_lock);
...
void btusb_delete(struct kref *kref)
{
    ...
    BTUSB_ERR("dequeued all remaining buffers from TX Q\n");
...
#define BTUSB_STATE_STATUS  "driver/btusb-state"

static int btusb_open_seq(struct seq_file *s, void *v)
{
	return 0;
}

static int btusb_state_file_open(struct inode *inode, struct file *file)
{
	BTUSB_ERR("Opening the /proc/%s  file :%ld\n", BTUSB_STATE_STATUS, jiffies + 2*HZ);
    return single_open(file, btusb_open_seq,  PDE_DATA(inode));
};

int btusb_state_file_read(struct file *file, char *buf, size_t count, loff_t *offp )
{
	char send;
#ifdef SAMSUNG_PATCH_STATE_BTUSB
	int retval = 0;
...
#ifdef BTSTATE_DEBUG
	BTUSB_ERR("Wait on proc state\n");
#endif
	/*Return with some timeout; This will ensure user space to not hang infintely*/
	retval = wait_event_interruptible_timeout(bt_state.btusb_wait_q, (bt_state.suspend == BTUSB_NORMAL), (jiffies + 2*HZ));

#ifdef BTSTATE_DEBUG
	BTUSB_ERR("wait break: %d\n", retval);
#endif

	if(retval > 0)					/* Case when woke up by some other thread*/
	{
		send = (char) bt_state.suspend;
		copy_to_user(buf, &send,  sizeof(send));
	}
	else if(retval == 0)			/* Case when timeout occured*/
	{
		send = 0xFF;
		copy_to_user(buf, &send, sizeof(send));
	}
	else							/* Case when interrupted*/
	{
		send = 0xFE;
		copy_to_user(buf, &send, sizeof(send));
	}

	BTUSB_ERR("read called: %d 0x%x\n", retval, send);
#else
	/*In case of HawkP return BTUSB_NORMAL always*/
	send = BTUSB_NORMAL;
	copy_to_user(buf, &send, sizeof(send));
#endif
...
static struct file_operations btusb_state_file_ops =
{
    .owner   = THIS_MODULE,
    .open    = btusb_state_file_open,
    .read    = btusb_state_file_read,
    .llseek  = seq_lseek,
    .release = single_release
};

static int btusb_state_proc_init(void)
{
	struct proc_dir_entry *entry;
	/* Create /proc/btusb entry */
    entry = proc_create_data(BTUSB_STATE_STATUS, S_IRUGO | S_IWUGO, NULL,
            &btusb_state_file_ops, NULL);
	if (entry)
    {
        BTUSB_ERR("/proc/%s entry created\n", BTUSB_STATE_STATUS);
		return 0;
    }
    else
    {
        BTUSB_ERR("Failed to create /proc/%s entry\n", BTUSB_STATE_STATUS);
        return -1;
    }
}

static int _btusb_state_proc_exit(void)
{
	remove_proc_entry(BTUSB_STATE_STATUS, NULL);
	return 0;
}
