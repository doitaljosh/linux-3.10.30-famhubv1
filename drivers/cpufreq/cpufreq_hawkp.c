/*
 *  drivers/cpufreq/cpufreq_hawkp.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_HAWKP
static struct cpufreq_governor cpufreq_gov_hawkp;
#endif
...
static void hawkp_powersave_bias_init_cpu(int cpu)
{
...
static void hawkp_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		hawkp_powersave_bias_init_cpu(i);
	}
}
...
static void od_check_cpu(int cpu, unsigned int load_freq)
{
	...
	dvfs_print("[cpu%d] curfreq=%dMHz, load=%d%%\n", 
		smp_processor_id(), policy->cur/1000, load_freq / policy->cur);
		...
		unsigned int new_rate)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (!policy)
			continue;
		if (policy->governor != &cpufreq_gov_hawkp) {
...
static struct od_ops od_ops = {
	.powersave_bias_init_cpu = hawkp_powersave_bias_init_cpu,
...
static
#endif
struct cpufreq_governor cpufreq_gov_hawkp = {
	.name			= "hawkp",
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_hawkp);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_hawkp);
}

MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_DESCRIPTION("'cpufreq_hawkp' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_HAWKP
