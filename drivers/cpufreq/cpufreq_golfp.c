/*
 *  drivers/cpufreq/cpufreq_golfp.c
 *
...
#define DEF_FREQUENCY_UP_THRESHOLD_L		(25)
#define DEF_FREQUENCY_UP_STEP_LEVEL_L		(800000)
#define DEF_FREQUENCY_DOWN_STEP_LEVEL_L		(400000)
#define DEF_FREQUENCY_DOWN_THRESHOLD_L		(15)

static DEFINE_PER_CPU(struct od_cpu_dbs_info_s, od_cpu_dbs_info);

static struct od_ops od_ops;

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_GOLFP
static struct cpufreq_governor cpufreq_gov_golfp;
#endif
...
static void golfp_powersave_bias_init_cpu(int cpu)
{
...
static void golfp_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		golfp_powersave_bias_init_cpu(i);
	}
}
...
static void od_check_cpu(int cpu, unsigned int load_freq)
{
	...
	dvfs_print("[cpu%u] curfreq=%dMHz, load=%d%%\n", 
		policy->cpu, policy->cur/1000, load_freq / policy->cur);

	/* Check for frequency increase */
	if (policy->cur < DEF_FREQUENCY_UP_STEP_ZONE) {
		/*
		 * if cur freq < 400MHz, and load freq is bigger than
		 * up_threshold_low 25, must visit 800MHz.
		 */
		if (load_freq > od_tuners->up_threshold_low * policy->cur) {
			dbs_freq_increase(policy, DEF_FREQUENCY_UP_STEP_LEVEL_L);
			dvfs_print("increase step (%dMHz)\n", DEF_FREQUENCY_UP_STEP_LEVEL_L/1000);
			return;
		}
	} else if (policy->cur >= DEF_FREQUENCY_UP_STEP_ZONE &&
		policy->cur < DEF_FREQUENCY_UP_STEP_LEVEL_L) {
		/*
	...
	if (policy->cur > DEF_FREQUENCY_DOWN_STEP_LEVEL_L) {
	 	/* if cur freq > 400MHz, apply down_threshold 80%. */
		if (load_freq < od_tuners->adj_up_threshold * policy->cur) {
	 	...
	 	/* if cur freq <= 400MHz, down threshold 15% */
		if (load_freq < od_tuners->down_threshold_low * policy->cur) {
			unsigned int freq_next;

			freq_next = load_freq / od_tuners->down_threshold_low;
		...
		unsigned int new_rate)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (!policy)
			continue;
		if (policy->governor != &cpufreq_gov_golfp) {
...
static int od_init(struct dbs_data *dbs_data)
{
	...
	tuners->up_threshold_low = DEF_FREQUENCY_UP_THRESHOLD_L;
	tuners->down_threshold_low = DEF_FREQUENCY_DOWN_THRESHOLD_L;
...
static struct od_ops od_ops = {
	.powersave_bias_init_cpu = golfp_powersave_bias_init_cpu,
...
static
#endif
struct cpufreq_governor cpufreq_gov_golfp = {
	.name			= "golfp",
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_golfp);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_golfp);
}

MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_DESCRIPTION("'cpufreq_golfp' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_GOLFP
