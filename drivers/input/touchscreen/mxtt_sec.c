...
struct class *sec_class = NULL; 	//hsguy.son (add)
		...
		const u8 *fw_data)
{
	...
	//snprintf(fw_path, MXT_MAX_FW_PATH, "/sdcard/%s", firmware_name); 	//original
	snprintf(fw_path, MXT_MAX_FW_PATH, "/mnt/SD1/%s", firmware_name); 	//hsguy.son (add)
...
static void fw_update(void *device_data)
{
	...
	//disable_irq(data->pdata->gpio_irq); 	//hsguy.son (add)
...
irq_enable:
	...
	//enable_irq(data->pdata->gpio_irq); 	//hsguy.son (add)
...
static void mxt_run_factory_cal(void *device_data)
{
	...
	disable_irq(client->irq); 	//original
	//disable_irq(data->pdata->gpio_irq); 	//hsguy.son (add)
	...
	enable_irq(client->irq); 	//original
	//enable_irq(data->pdata->gpio_irq); 	//hsguy.son (add)
...
//hsguy.son (add)
static ssize_t mxt_atmeldbg_tsp_config_crc_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	if (!user_object) {
		tsp_debug_err(true, &client->dev, "fail to get object_info\n");
		return snprintf(buf, TSP_BUF_SIZE, "NG\n");
	}
	...
	return snprintf(buf, TSP_BUF_SIZE, "AT_1121_0x%06X\n", current_crc);
}

static ssize_t mxt_atmeldbg_tsp_fw_ver_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	struct mxt_fac_data *fdata = data->fdata;
	...
	return snprintf(buf, TSP_BUF_SIZE, "AT00%02x%02x\n", ver, build);
}

static ssize_t mxt_atmeldbg_tsp_report_lock_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	int ret;

	ret = sprintf(buf, "%d\n", data->atmeldbg.tsp_report_lock);

	return ret;
}

static ssize_t mxt_atmeldbg_tsp_report_lock_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	if (sscanf(buf, "%u", &i) == 1 && i<2)
	{
		data->atmeldbg.tsp_report_lock = i;

		tsp_debug_info(true, &client->dev, "Atmel tsp_report_lock ON\n");
	}
	else
	{
		tsp_debug_info(true, &client->dev, "Error %s\n", __func__);
...
static ssize_t mxt_atmeldbg_tsp_sleep_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	int ret;

	ret = sprintf(buf, "%d\n", data->atmeldbg.tsp_sleep_mode);

	return ret;
}

static ssize_t mxt_atmeldbg_tsp_sleep_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	if(rc < 0)
	{
		tsp_debug_err(true, &client->dev, "Invalid value\n");
		goto exit;
	}

	if(value == 1 && value < 2)
	{
		data->atmeldbg.tsp_sleep_mode = value;

		tsp_debug_info(true, &client->dev, "Atmel TSP Sleep(Suspend) ON\n");
	...
	else if(value == 0)
	{
		...
		tsp_debug_info(true, &client->dev, "Atmel TSP Sleep(Resume) OFF\n");
...
static ssize_t mxt_atmeldbg_tsp_sampling_rate_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	int ret;

	ret = sprintf(buf, "%d\n", data->atmeldbg.tsp_sampling_rate);

	return ret;
}

static ssize_t mxt_atmeldbg_tsp_sampling_rate_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	if(sscanf(buf, "%u", &i) == 1 && i<2)
	{
		data->atmeldbg.tsp_sampling_rate = i;

		tsp_debug_info(true, &client->dev, "Atmel tsp_sampling_rate ON\n");
	}
	else
	{
		tsp_debug_info(true, &client->dev, "Error %s\n", __func__);
...
static ssize_t mxt_atmeldbg_tsp_coordinate_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	int ret;

	ret = sprintf(buf, "%d\n", data->atmeldbg.tsp_print_coordinate);

	return ret;
}

static ssize_t mxt_atmeldbg_tsp_coordinate_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	if (sscanf(buf, "%u", &i) == 1 && i<2)
	{
		data->atmeldbg.tsp_print_coordinate = i;

		tsp_debug_info(true, &client->dev, "Atmel tsp_print_coordinate ON\n");
	}
	else
	{
		tsp_debug_info(true, &client->dev, "Error %s\n", __func__);
...
static DEVICE_ATTR(tsp_cfg_crc, S_IRUGO, mxt_atmeldbg_tsp_config_crc_show, NULL); 	//hsguy.son (add)
static DEVICE_ATTR(tsp_fw_ver, S_IRUGO, mxt_atmeldbg_tsp_fw_ver_show, NULL); 	//hsguy.son (add)
...
static DEVICE_ATTR(tsp_print_coordinate, S_IRUGO | S_IWUSR | S_IWGRP,
	mxt_atmeldbg_tsp_coordinate_show, mxt_atmeldbg_tsp_coordinate_store); 	//hsguy.son (add)
static DEVICE_ATTR(tsp_sampling_rate, S_IRUGO | S_IWUSR | S_IWGRP,
	mxt_atmeldbg_tsp_sampling_rate_show, mxt_atmeldbg_tsp_sampling_rate_store); 	//hsguy.son (add)
static DEVICE_ATTR(tsp_sleep_mode, S_IRUGO | S_IWUSR | S_IWGRP,
	mxt_atmeldbg_tsp_sleep_mode_show, mxt_atmeldbg_tsp_sleep_mode_store); 	//hsguy.son (add)
static DEVICE_ATTR(tsp_report_lock, S_IRUGO | S_IWUSR | S_IWGRP,
	mxt_atmeldbg_tsp_report_lock_show, mxt_atmeldbg_tsp_report_lock_store); 	//hsguy.son (add)
...
#if TSP_USE_ATMELDBG
	&dev_attr_tsp_cfg_crc.attr, 		//hsguy.son (add)
	&dev_attr_tsp_fw_ver.attr, 		//hsguy.son (add)
	&dev_attr_pause_driver.attr,
	&dev_attr_tsp_print_coordinate.attr, 	//hsguy.son (add)
	&dev_attr_tsp_sampling_rate.attr, 	//hsguy.son (add)
	&dev_attr_tsp_sleep_mode.attr, 	//hsguy.son (add)
	&dev_attr_tsp_report_lock.attr, 	//hsguy.son (add)
...
static int mxt_sysfs_init(struct i2c_client *client)
{
	...
	sec_class = class_create(THIS_MODULE, "sec_touchscreen"); 	//hsguy.son (add)
	if (IS_ERR(sec_class)) {
