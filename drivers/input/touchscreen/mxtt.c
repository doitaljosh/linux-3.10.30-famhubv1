...
#ifdef FAMILY_HUB
static int touch_key_dynamic_enable = true;
#endif
static unsigned int key_code[] = {KEY_F13, KEY_BACK, KEY_PHONE, KEY_MENU, KEY_CONFIG}; 	//hsguy.son (add)

//hsguy.son (add)
static int mxt_command_reset(struct mxt_data *data, u8 value); 	//hsguy.son (add)
#ifdef ENABLE_TOUCH_KEY
static void mxt_release_all_keys(struct mxt_data *data);
#endif

//extern pca953x_port_set_value(unsigned off, int val);
//hsguy.son (add)
#if 0
static inline void writel_add(u32 val, u32 addr)
{
	u32 tmp = readl(addr) | val;
	writel(tmp, addr);
}

static inline void writel_convert(u32 val, u32 mask, u32 addr)
{
	u32 tmp = readl(addr);
...
static int mxt_get_sampling_rate(struct mxt_data *data)
{
	bool sampling_rate;

	sampling_rate = data->atmeldbg.tsp_sampling_rate;
...
#ifdef DRIVER_LEVEL_SCALING
static void mxt_report_input_data_scaling(struct mxt_data *data, int index)
{
	 data->fingers[index].x = (data->fingers[index].x * 1920) / data->pdata->max_x;
	 data->fingers[index].y = (data->fingers[index].y * 1080) / data->pdata->max_y;
}

static void mxt_report_input_data_rotate(struct mxt_data *data, int index)
{
	int tmp;
	bool flipped;
	...
	if(1)
	{
		tmp = data->fingers[index].x;
		data->fingers[index].x = data->fingers[index].y;
		data->fingers[index].y = tmp;
		flipped = true;
	}
	else
	{
	...
	if(0)
	{
		if(flipped)
		{
			//data->fingers[index].x = data->pdata->max_y - data->fingers[index].x;
			data->fingers[index].x = 1080 - data->fingers[index].x;
		}
		else
		{
			data->fingers[index].x = data->pdata->max_x - data->fingers[index].x;
		}
	}
	...
	if(0)
	{
		if(flipped)
		{
			//data->fingers[index].y = data->pdata->max_x - data->fingers[index].y;
			data->fingers[index].y = 1180 - data->fingers[index].y;
		}
		else
		{
			data->fingers[index].y = data->pdata->max_y - data->fingers[index].y;
		}
	}
}
#endif
...
static void mxt_report_input_data(struct mxt_data *data)
{
...
#if 1 	//hsguy.son (modify), because not use key event
			if (data->fingers[i].type
				 == MXT_T100_TYPE_HOVERING_FINGER)
				/* hover is reported */
				input_report_key(data->input_dev,
					BTN_TOUCH, 0); 	//hsguy.son (modify)
			else
				/* finger or passive stylus are reported */
				input_report_key(data->input_dev,
					BTN_TOUCH, 1); 	//hsguy.son (modify)
#endif
		}
		report_count++;
	...
	if (count == 0)
	{
		//input_report_key(data->input_dev, BTN_TOUCH, 0); 	//hsguy.son (modify)
//hsguy.son (add)
#ifdef TSP_PRINT_COORDINATE
		if(data->atmeldbg.tsp_print_coordinate)
		{
			printk("tsp_coordinate : - All touch release -\n");
		}
#endif
#ifdef FAMILY_HUB
		touch_key_dynamic_enable = true;
#endif
//hsguy.son (add_end)
	}
//hsguy.son (add)
	...
	{
		touch_key_dynamic_enable = false;
		mxt_release_all_keys(data);
	}
#endif

#ifdef SYSFS_SAMPLE_RATE
	if(mxt_get_sampling_rate(data))
	{
		measure_sample_rate(&(data->sample_rate), count ? 1 : 0);
	}
#endif
//hsguy.son (add_end)
	if (report_count > 0) {
...
static void mxt_release_all_finger(struct mxt_data *data)
{
	...
	if (count) {
		...
		if(!data->atmeldbg.tsp_report_lock)
		{
		...
		struct mxt_message *message)
{
	...
	if (message->message[0] == 0x00) {
		tsp_debug_dbg(true, &data->client->dev, "Normal mode\n");
#if TSP_INFORM_CHARGER
		set_charger_config(data);
#endif
...
*/
	...
	if (message->message[0] & 0x10) {
		tsp_debug_dbg(true, &data->client->dev, "Calibration is on going !!\n");
	}
	/* Signal error */
...
static void mxt_release_all_keys(struct mxt_data *data)
{
	...
	if (data->tsp_keystatus != TOUCH_KEY_NULL) {
		if (data->report_dummy_key) {
			if(!data->atmeldbg.tsp_report_lock) 	//hsguy.son (add)
			{
				for (i = 0 ; i < data->pdata->num_touchkey ; i++) {
					if (data->tsp_keystatus & data->pdata->touchkey[i].value) {
					/* report all touch-key event */
						//input_report_key(data->input_dev,
						input_report_key(data->input_dev_2,
							data->pdata->touchkey[i].keycode, KEY_RELEASE); 	//hsguy.son (modify)
						//tsp_debug_info(true, &data->client->dev,
						//	"[TSP_KEY] %s R!\n", data->pdata->touchkey[i].name); 	//hsguy.son (modify)
					}
				}
			}
		} else {
		...
		if(!data->atmeldbg.tsp_report_lock) 	//hsguy.son (add)
		{
			//input_report_key(data->input_dev, code, KEY_RELEASE); //hsguy.son (modify)
			input_report_key(data->input_dev_2, code, KEY_RELEASE); //hsguy.son (modify)
			//tsp_debug_info(true, &data->client->dev,
			//	"[TSP_KEY] %d R!\n", code); 	//hsguy.son (modify)
			//input_sync(data->input_dev);
			input_sync(data->input_dev_2); 	//hsguy.son (add)
		}
						...
						struct mxt_message *message)
{
	...
	struct input_dev *input = data->input_dev_2; 	//hsguy.son (add)
...
//hsguy.son (add)
				if(!data->atmeldbg.tsp_report_lock)
				{
					...
					if(data->atmeldbg.tsp_print_coordinate)
					{
						printk("tsp_coordinate : ID=%d x=%d, y=%d, p=%d, \n",
					   		id, data->fingers[id].x, data->fingers[id].y, 0);
					}
#endif
				}
//hsguy.son (add_end)
...
#ifdef DRIVER_LEVEL_SCALING  //hsguy.son (add)
			mxt_report_input_data_rotate(data, id);
			mxt_report_input_data_scaling(data, id);
#endif //hsguy.son (add_end)
...
//hsguy.son (add)
			if(!data->atmeldbg.tsp_report_lock)
			{
				...
				{
					printk("tsp_coordinate : ID=%d x=%d, y=%d, p=%d, \n",
				   		id, data->fingers[id].x, data->fingers[id].y, 1);
				}
#endif
			}
//hsguy.son (add_end)
	...
	do {
		...
		if (mxt_read_message(data, &message)) {
			...
			case MXT_SPT_SELFTEST_T25:
				if(message.message[0] != 0x00) 	//hsguy.son (add), Atmel guide, 0x00 is non-error
				{
...
{
	//const struct mxt_platform_data *pdata = data->pdata; 	//hsguy.son (add)
	int error = 0;
	...
	if (error)
		...
		//enable_irq(pdata->gpio_irq); 	//hsguy.son (add)
		enable_irq(data->client->irq); 	//original
	
	data->irq_state = true; 	//hsguy.son (add)
...
static int mxt_stop(struct mxt_data *data)
{
	...
	//disable_irq(pdata->gpio_irq); 	//hsguy.son (add)
	data->irq_state = false; 	//hsguy.son (add)
	disable_irq(data->client->irq); 	//original
...
static int mxt_input_open(struct input_dev *dev)
{
	...
	tsp_debug_info(true, &data->client->dev, "%s\n", __func__);
	tsp_debug_err(true, &data->client->dev, "%s --- Atmel Touch Open ---\n", __func__); 	//hsguy.son (debug)
...
static int mxt_touch_finish_init(struct mxt_data *data)
{
	...
	error = request_threaded_irq(client->irq, NULL, mxt_irq_thread,
		IRQF_DISABLED | IRQF_SHARED | IRQF_ONESHOT | IRQF_TRIGGER_HIGH, client->dev.driver->name, data);

	if (error) {
		tsp_debug_err(true, &client->dev, "Failed to register interrupt\n");
		tsp_debug_err(true, &client->dev, "irq num : %d\n", client->irq); 	//hsguy.son (debug)
		goto err_req_irq;
	}
...
static int mxt_flash_fw_on_probe(struct mxt_fw_info *fw_info)
{
...
#if 1  	//hsguy.son (add by Atmel tip)
	fw_info->fw_ver = 255;
	return 0;
#endif
	...
	if(mxt_check_config_crc(fw_info)== CONFIG_IS_SAME){
		tsp_debug_dbg(false, dev, "Firmware Config CRC is same with in IC\n");
		goto out;
	}
...
static int mxt_request_firmware_work(const struct firmware *fw,
		void *context)
{
...
static int mxt_touch_init(struct mxt_data *data)
{
	...
	ret = mxt_request_firmware_work(fw, data);
out:
	return ret;
}
...
static int mxt_power_ctrl(void *ddata, bool on)
{
	...
	{
		gpio_set_value(pdata->gpio_reset, 1);
		//pca953x_port_set_value(10, 1);
		msleep(MXT_SW_RESET_TIME);
	}
	else
	{
		gpio_set_value(pdata->gpio_reset, 0);
		//pca953x_port_set_value(10, 0);
		msleep(MXT_SW_RESET_TIME);
	}
...
static int mxt_parse_dt(struct i2c_client *client)
{
	...
	u32 boot_addr;
		...
		//u32 index[6];
		//u32 bit[6];
		//u32 key_code[5]; 	//hsguy.son (add)
		int i = 0;
		int shift_i = 0; 	//hsguy.son (add)
#endif
	int retval = 0;
	
#if 0
	//void __iomem *REG_I2C0_SEL = ioremap(0x005C1040, sizeof(u32));
	void __iomem *REG_INT_SEL = ioremap(0x005C104C, sizeof(u32));
	//writel_convert(0, 1, REG_I2C0_SEL); 	
	writel_add(1<<5, REG_INT_SEL); 	//external gpio select
#endif 	//hsguy.son (add)

#if 0
	pdata->sck = of_get_named_gpio(np, "ateml,sck", 0);
	gpio_set_value(pdata->sck, 1);
	pdata->sda = of_get_named_gpio(np, "ateml,sda", 0);
	gpio_set_value(pdata->sda, 1);
#endif 	//hsguy.son (add)
	...
	//client->irq = __gpio_to_irq(pdata->gpio_irq); //original
	//printk("---%s, BBB, client->irq : %d---\n", __func__, client->irq); 	//hsguy.son (debug)
	//client->irq = 163+32; //hsguy.son (add)
	//client->irq = 161+32; //hsguy.son (add)
	client->irq = 160 + 32 + 18; // i_EXTINT[18]
...
#if 1
	//hsguy.son (add)
	if (of_property_read_u32(np, "atmel,x-size", &pdata->max_x)) {
		tsp_debug_err(true, dev, "Failed to get x-size property\n");
		return -EINVAL;
	}
	
	if (of_property_read_u32(np, "atmel,y-size", &pdata->max_y)) {
		tsp_debug_err(true, dev, "Failed to get y-size property\n");
		return -EINVAL;
	}

	if (of_property_read_u32(np, "atmel,num_xnode", &pdata->num_xnode)) {
		tsp_debug_err(true, dev, "Failed to get x num_liness property\n");
		return -EINVAL;
	}
	
	if (of_property_read_u32(np, "atmel,num_ynode", &pdata->num_ynode)) {
		tsp_debug_err(true, dev, "Failed to get y num_liness property\n");
		return -EINVAL;
	}
	
	if (of_property_read_u32(np, "atmel,num_touchkey", &pdata->num_touchkey)) {
		tsp_debug_err(true, dev, "Failed to get y num_liness property\n");
		return -EINVAL;
	}
	
	pdata->touchkey = devm_kzalloc(&client->dev,
		sizeof(struct mxt_touchkey) * pdata->num_touchkey, GFP_KERNEL);
	...
	for(i=0; i<pdata->num_touchkey; i++)
	{
		if(i == 1)
		{
			shift_i++;
		}
		pdata->touchkey[i].keycode = key_code[i];
		pdata->touchkey[i].value = (0x1<<shift_i);
		shift_i++;
	}
	//hsguy.son (add_end)
#endif
...
#ifndef CONFIG_OF
static int mxt_get_platform_config(struct mxt_platform_data *pdata)
{
	//TODO : get sysconfig data & add sysconfig.fex config
	pdata->num_xnode = 39;
	pdata->num_ynode = 68;
		...
		const struct i2c_device_id *id)
{
	...
	struct input_dev *input_dev_2; 	//hsguy.son (add)
	u16 boot_address;
	int error = 0;
#if ENABLE_TOUCH_KEY
	int  i = 0;
...
#else
	pdata = devm_kzalloc(&client->dev,
			sizeof(struct mxt_platform_data), GFP_KERNEL); 	//hsguy.son (add)

#if 1 
	error = mxt_get_platform_config(pdata);
	if(!error)
	{
		tsp_debug_err(true, &client->dev, "Failed to get tsp configuration data\n");
		return -EINVAL;
	}
#endif

	client->dev.platform_data = pdata; 		//hsguy.son (add)
	//pdata->gpio_irq = __gpio_to_irq(config_info.int_number); 	//hsguy.son (add)
	//pdata = client->dev.platform_data; 	//original
#endif
	if (!pdata) {
...
//hsguy.son (add)	
	input_dev_2 = input_allocate_device();
	if (!input_dev) {
		error = -ENOMEM;
		tsp_debug_err(true, &client->dev, "Input device allocation failed\n");
		goto err_allocate_input_device;
...
//hsguy.son (add_end)
	...
	scnprintf(data->phys, sizeof(data->phys)-1, "%s", dev_name(&client->dev)); 	//hsguy.son (add)
	input_dev->phys = data->phys; 	//hsguy.son (add)
	input_dev->dev.parent = &client->dev;
	input_dev->open = mxt_input_open;
	input_dev->close = mxt_input_close;
	
//hsguy.son (add)
	input_dev_2->name = "sec_touchkey";
	input_dev_2->id.bustype = BUS_I2C;
	scnprintf(data->phys, sizeof(data->phys)-1, "%s", dev_name(&client->dev)); 	//hsguy.son (add)
	input_dev_2->phys = data->phys; 	//hsguy.son (add)
	input_dev_2->dev.parent = &client->dev;
//hsguy.son (add_end)

	data->client = client;
	data->input_dev = input_dev;
	data->input_dev_2 = input_dev_2; 	 //hsguy.son (add)
	data->pdata = pdata;

#if 1 	//hsguy.son (modify)	
	input_set_drvdata(input_dev, data);
	input_set_drvdata(input_dev_2, data); 	//hsguy.son (add)
	i2c_set_clientdata(client, data);
#endif

	set_bit(EV_ABS, input_dev->evbit);
	set_bit(EV_KEY, input_dev->evbit);
	set_bit(EV_SYN, input_dev->evbit); 	//hsguy.son (add)
	//set_bit(INPUT_PROP_DIRECT, input_dev->propbit);
	set_bit(BTN_TOUCH, input_dev->keybit);

//hsguy.son (add)
 	set_bit(EV_ABS, input_dev_2->evbit);
	input_dev_2->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS);
	input_dev_2->keybit[BIT_WORD(BTN_TOUCH)] = BIT_MASK(BTN_TOUCH);
	for (i = 0 ; i < data->pdata->num_touchkey ; i++)
		set_bit(data->pdata->touchkey[i].keycode, input_dev_2->keybit);
	data->report_dummy_key = true; /*Disable dummy key!*/
//hsguy.son (add_end)
#if ENABLE_TOUCH_KEY
#if 0
	for (i = 0 ; i < data->pdata->num_touchkey ; i++)
...
//hsguy.son (add)
	input_set_abs_params(input_dev, ABS_X, 0, pdata->max_x, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, pdata->max_y, 0, 0);
	input_set_abs_params(input_dev, ABS_PRESSURE, 0, MXT_AMPLITUDE_MAX, 0, 0);
//hsguy.son (add_end)
...
//hsguy.son (add)
	error = input_register_device(input_dev_2);
	if (error)
		goto err_register_input_device;
//hsguy.son (add_end)
...
err_register_input_device:
	...
	input_unregister_device(input_dev_2); 	//hsguy.son (add)
	input_dev = NULL;
	input_dev_2 = NULL; 	//hsguy.son (add)
err_touch_init:
	mxt_power_off(data);
err_power_on:
...
err_create_sub_client:
	...
	input_free_device(input_dev_2); 	//hsguy.son (add)
...
static int mxt_remove(struct i2c_client *client)
{
	...
	input_unregister_device(data->input_dev_2); 	//hsguy.son (add)
	i2c_unregister_device(data->client_boot);
	mxt_sysfs_remove(data);
	mxt_power_off(data);
	if (client->dev.of_node) {
