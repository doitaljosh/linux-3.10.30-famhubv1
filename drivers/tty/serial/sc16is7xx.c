...
static int sc16is7xx_set_baud(struct uart_port *port, int baud)
{
	...
	u8 lcr;
	u8 prescaler = 0;
	unsigned long clk = port->uartclk, div = clk / 16;

	if (clk == 27000000)
		div = DIV_ROUND_UP(div, baud);
	else
		div = div / baud;
...
static void sc16is7xx_tx_proc(struct kthread_work *ws)
{
	...
	if ((one->rs485.flags & SER_RS485_ENABLED) &&
		...
		msleep(one->rs485.delay_rts_before_send);
...
static void sc16is7xx_reconf_rs485(struct uart_port *port)
{
	...
	u32 efcr = 0;
	struct serial_rs485 *rs485 = &one->rs485;
			   ...
			   struct regmap *regmap, int irq, unsigned long flags)
{
	...
	u32 freq, *pfreq = dev_get_platdata(dev);
	...
	ret = of_property_read_u32(node, "ext-clock", &freq);
	if (ret) {
		...
		dev_dbg(dev, "%s: clock %dHz loading..Success\n", __func__, freq);
	}
	...
	/* check device */
	if (regmap_read(s->regmap,
		    (SC16IS7XX_IER_REG << SC16IS7XX_REG_SHIFT) | 0, &val) < 0)  {
		    ret = -1;
		    dev_err(dev, "Detect UART device error\n");
	...
	if (!ret) {
		dev_dbg(dev, "%s: Probing sc16is7xx..Success\n", __func__);
		return 0;
	}
...
err_chip_id_or_i2c_error:
	...
	dev_err(dev, "%s: Probing sc16is7xx..Fail !!!\n", __func__);
...
static int sc16is7xx_spi_probe(struct spi_device *spi)
{
	...
	dev_dbg(&spi->dev, "%s: Probing sc16is7xx_spi\n", __func__);
	...
	ret = spi_setup(spi);
	if (ret) {
		dev_err(&spi->dev, "spi_setup error\n");
		return ret;
	}
	...
	if (of_property_read_u32(spi->dev.of_node, "ext-int", &interrupt_num)) {
		dev_err(&spi->dev, "could not get interrupt number\n");
	} else {
		spi->irq = 160 + interrupt_num + 32; 
		dev_dbg(&spi->dev, "%s: interrupt number is EXTINT%d\n", __func__, interrupt_num);

		if (of_property_read_u32(spi->dev.of_node, "ext-reg", &isel_reg)) {
			dev_err(&spi->dev, "could not get interrupt isel register\n");
		} else {
			...
			dev_dbg(&spi->dev, "%s: interrupt isel_reg (0x%08x) is (%08x)\n",
							__func__, isel_reg, tmp);
		}
	}
			       ...
			       const struct i2c_device_id *id)
{
	...
	dev_dbg(&i2c->dev, "%s: Probing sc16is7xx..\n", __func__);
	...
	if (of_property_read_u32(i2c->dev.of_node, "ext-int", &interrupt_num)) {
		dev_err(&i2c->dev, "could not get interrupt number\n");
	} else {
		i2c->irq = 160 + interrupt_num + 32; 
		dev_dbg(&i2c->dev, "%s: interrupt number is EXTINT%d\n", __func__, interrupt_num);
	}
