...
int tty_ldisc_setup(struct tty_struct *tty, struct tty_struct *o_tty)
{
	...
	int retval;
#ifdef CONFIG_T2D_DEBUGD
	char t2d_buf[6];    /* console name of one digit port number */
#endif
...
#ifdef CONFIG_T2D_DEBUGD
#if CONFIG_T2D_SERIAL_INPUT_PORTNUM >= 10
#error  "Serial port number is over one digit, check CONFIG_T2D_SERIAL_INPUT_PORTNUM"
#endif
	snprintf(t2d_buf, 6, "ttyS%d", CONFIG_T2D_SERIAL_INPUT_PORTNUM);

	if ((ld->ops->open) && (!strncmp(t2d_buf, tty->name, strlen(t2d_buf)))) {
		T2DINPUT_tty = tty;
	}
#endif
...
#endif
#ifdef CONFIG_T2D_DEBUGD
		if((T2DINPUT_tty != NULL) && (!strncmp(tty->name, buf, strlen(t2d_buf)))) {
			T2DINPUT_tty = NULL;
		}
#endif
		return retval;
