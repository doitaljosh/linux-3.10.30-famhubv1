...
{
	oprofile_set_ulong(&oprofile_backtrace_depth, val);
}
#endif
...
#ifdef CONFIG_ADVANCE_OPROFILE
static void aop_dcookie_release(void *dcookie_user_data)
{
	dcookie_unregister(dcookie_user_data);
}


static int kdebugd_aop_oprofile_init(void)
{
...
static void kdebugd_aop_oprofile_exit(void)
{
	aop_dealloc();
}
