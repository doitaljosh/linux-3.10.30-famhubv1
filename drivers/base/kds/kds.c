		...
		unsigned long         jiffies_timeout)
{
	...
	if (!triggered)
	{
		...
		if (timeout)
		{
			wait_res = wait_event_timeout(rset->wake,
...
#ifndef FOX_MALI_ORG
struct kds_dma_buf_register kds_dma_buf_register_t ={
                .kds_callback_term=kds_callback_term,
                .kds_resource_term=kds_resource_term,
                .kds_waitall=kds_waitall,
                .kds_resource_set_release=kds_resource_set_release,
                .kds_async_waitall=kds_async_waitall,
                .kds_resource_init=kds_resource_init,
                .kds_callback_init=kds_callback_init,
        };
int kds_initialize_dma_buf(void)
{
	kds_dma_buf_module_register(&kds_dma_buf_register_t);
	return 0;
}
EXPORT_SYMBOL(kds_initialize_dma_buf);
#endif
