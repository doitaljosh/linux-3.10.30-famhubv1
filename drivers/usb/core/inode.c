...
#define USBFS_NEW_DEFAULT_DEVMODE 	(S_IRWXUGO)
#define USBFS_NEW_DEFAULT_BUSMODE 	(S_IRWXUGO)
#define USBFS_NEW_DEFAULT_LISTMODE 	(S_IRWXUGO)
...
static int parse_options(struct super_block *s, char *data)
{
	...
	busgid = 0;
	listgid = 0;
	devmode = USBFS_NEW_DEFAULT_DEVMODE;
	busmode = USBFS_NEW_DEFAULT_BUSMODE;
	listmode = USBFS_NEW_DEFAULT_LISTMODE;
...
static int usbfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!inode) {
		printk(KERN_DEBUG"%s: could not get inode!",__func__);
		return -ENOMEM;
	}
...
//	root = d_alloc_root(inode);
	...
	if (!root) {
		printk(KERN_DEBUG"%s: could not get root dentry!",__func__);
		iput(inode);
		return -ENOMEM;
	}
	sb->s_root = root;
