	...
	struct 			usb_endpoint_descriptor *desc)
{
	...
	if (!urb)
		return urb;
	if(is_interrupt)
		usb_fill_int_urb(urb, udev, pipe, NULL, bytes, simple_callback, NULL,
				(1<<(desc->bInterval-1)));
	else
...
static struct urb *simple_alloc_urb(
	...
	unsigned long		bytes)
{
	return usbtest_alloc_urb(udev, pipe, bytes, URB_NO_TRANSFER_DMA_MAP, 0,false,NULL);
}
...
#else
	/* ij.jang: it seems that dma_alloc_coherent from dma_pool via hcd_alloc_buffer is 
	 * not conformance with ARCH_DMA_MINALIGN. this can be broken when 
	 * DMA_MINALIGN is > hcd's pool's minimum alloc size.
	 * so apply temporary workaround here. */
	return (unsigned long)buf & (15);
#endif
}
...
)
{
	...
	while (retval == 0 && iterations-- > 0) {
		...
		if (retval != 0) {
			dev_err(&udev->dev, "usb_submit_urb() failed %d\n",retval);			
			break;
		}
...
test_ctrl_queue(struct usbtest_dev *dev, struct usbtest_param *param)
{
	...
	urb = kzalloc(param->sglen * sizeof(struct urb *), GFP_KERNEL);
	if (!urb)
		return -ENOMEM;
	...
	const char *label)
{
	...
	struct urb *urb = usbtest_alloc_urb(
		testdev_to_usbdev(tdev), pipe, length, transfer_flags, 1,false,NULL);
...
usbtest_ioctl(struct usb_interface *intf, unsigned int code, void *buf)
{
	...
	case 2:
		...
		simple_free_urb(urb);
		break;
	/* Bulk write with varying packet size */	
	case 3:
		...
		simple_free_urb(urb);
		break;
	/* Bulk read with varying packet size */	
	case 4:
	...
	case 6:
		...
		free_sglist(sg, param->sglen);
		break;
	/* Queued bulk write  with varying packet size */	
	case 7:
		...
		free_sglist(sg, param->sglen);
		break;
	/* Queued bulk read  with varying packet size */	
	case 8:
	...
	/* Simple non-queued unlinks (ring with one urb) - read */
	case 11:
		...
		if (retval)
			dev_err(&intf->dev, "unlink reads failed %d, "
				"iterations left %d\n", retval, i);
		break;
	/* Simple non-queued unlinks (ring with one urb) - write */	
	case 12:
	...
	/* Test for bulk I/O using DMA mapping by core and odd address - write*/
	case 17:
		if (dev->out_pipe == 0)
			break;
		dev_info(&intf->dev,
		...
		retval = test_unaligned_bulk(
				...
				0, "test17");
		break;
    /* Test for bulk I/O using DMA mapping by core and odd address - read*/
	case 18:
		if (dev->in_pipe == 0)
			break;
		dev_info(&intf->dev,
	...
	/* Test for bulk I/O using premapped coherent buffer and odd address - write  */
	case 19:
		if (dev->out_pipe == 0)
			break;
		dev_info(&intf->dev,
		...
		break;
    /* Test for bulk I/O using premapped coherent buffer and odd address - read  */ 
	case 20:
		if (dev->in_pipe == 0)
			break;
		dev_info(&intf->dev,
	...
	/* Control write test with unaligned buffer */
	case 21:
		if (!dev->info->ctrl_out)
			break;
		dev_info(&intf->dev,
	...
	/* Unlink URBs from a bulk-OUT queue - write */
	case 24:
		...
		break;
       /* Simple non-queued interrupt write */
	case 25:
		...
		dev_info(&intf->dev,
				...
				param->length, param->iterations);
		        urb = usbtest_alloc_urb(udev, dev->out_int_pipe, param->length, URB_NO_TRANSFER_DMA_MAP, 0,true,dev->int_out);
		if (!urb) {
			retval = -ENOMEM;
			break;
		}
		...
		simple_free_urb(urb);
		break;
      /* Simple non-queued interrupt read */
	case 26:
		...
		dev_info(&intf->dev,
				...
				param->length, param->iterations);
		        urb = usbtest_alloc_urb(udev, dev->in_int_pipe, param->length, URB_NO_TRANSFER_DMA_MAP, 0,true,dev->int_in);
		if (!urb) {
			retval = -ENOMEM;
			break;
		}
...
usbtest_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (force_interrupt || udev->speed == USB_SPEED_LOW) {
		...
		if (dev->out_iso_pipe)
			iwtest = " iso-out";
        if(dev->in_int_pipe)
            intrtest = " interrupt-in";
        if(dev->out_int_pipe)
            intwtest = " interrupt-out";
	}
