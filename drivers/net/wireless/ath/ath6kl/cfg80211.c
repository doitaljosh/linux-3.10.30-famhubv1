...
char sem_call_stack[4][64];
int sem_call_index = 0;

void store_sem_call_stack(const char* func_name, int line_num, int count)
{
    sprintf( sem_call_stack[sem_call_index], "%s[%d](%d)", func_name, line_num, count);
    sem_call_index =(sem_call_index+1)%4;
}

void print_sem_call_stack(void)
{
	ath6kl_err("sem_call_stack[%d]: %s, %s, %s, %s\n",
			sem_call_index, sem_call_stack[0],
			sem_call_stack[1],sem_call_stack[2],sem_call_stack[3]);
	printk(KERN_ERR "[QCA Wi-Fi Driver Error] sem_call_stack[%d]: %s, %s, %s, %s\n",
			sem_call_index, sem_call_stack[0],
			sem_call_stack[1],sem_call_stack[2],sem_call_stack[3]);
}
				   ...
				   struct cfg80211_connect_params *sme)
{
	...
	if (down_interruptible(&ar->sem)) {
		...
		print_sem_call_stack();
		return -ERESTARTSYS;
	}
	store_sem_call_stack(__func__, __LINE__, ar->sem.count);
						     ...
						     struct key_params *))
{
	...
	store_sem_call_stack(__func__, __LINE__, vif->ar->sem.count);
...
#define MAX_RD_WAIT_CNT	(20)	/* 20 * 100 = 2 sec. */
	...
	if (!ath6kl_mod_debug_quirks(ar,
		...
		if (!IS_STA_AP_ONLY(ar)) {
			...
			rtnl_unlock();
			printk (KERN_ERR "ath6kl: Create dedicated p2p interface completed.\n");
		}
