...
{
    struct SDA_Descriptor *tx_desc = (struct SDA_Descriptor *) pBuffer;
	struct ath6kl_vif *vif;
	u32	entry_time;
	if (DA_context.ar == NULL) {
		ath6kl_dbg(ATH6KL_DBG_DA,"%s: DA_context.ar should not be NULL\n", __func__);
        SET_SDA_BUF_READY(tx_desc); 
        return;
    }

    if (DA_context.vif_id != 1) {
        ath6kl_dbg(ATH6KL_DBG_DA,"%s: DA_context.vif_id[%d] should be 1\n", __func__, DA_context.vif_id);
        SET_SDA_BUF_READY(tx_desc); 
        return;
    }
