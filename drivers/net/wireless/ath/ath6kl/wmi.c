					  ...
					  struct ath6kl_vif *vif)
{
	...
	//ath6kl_dbg(ATH6KL_DBG_WMI, // original code
	printk(KERN_ERR              // modified by max.park
...
int ath6kl_wmi_set_swol_indoor_info_cmd(struct wmi *wmi, u8 enable, u16 length, u8 *password)
{
    struct sk_buff *skb;
    struct wmi_set_swol_indoor_info_cmd *cmd;

	skb = ath6kl_wmi_get_new_buf(sizeof(*cmd));
	if (!skb)
		return -ENOMEM;

	cmd = (struct wmi_set_swol_indoor_info_cmd *)skb->data;
	cmd->wlan_swol_indoor = enable;
	cmd->swol_indoor_key_len = (u8) length;
	if (cmd->swol_indoor_key_len)
        memcpy(cmd->swol_indoor_key, password, cmd->swol_indoor_key_len);  

	return ath6kl_wmi_cmd_send(wmi, 0, skb, WMI_SET_SWOL_INDOOR_INFO,
				NO_SYNC_WMIFLAG);
}

/*TBD: not completed function*/
int ath6kl_wmi_set_swol_outdoor_info_cmd(struct wmi *wmi, u8 enable, u16 length, u8 *password)
{
    struct sk_buff *skb;
	...
	cmd = (struct wmi_set_swol_outdoor_info_cmd *)skb->data;
  /*assign setting here*/
