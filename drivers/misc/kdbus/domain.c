...
void kdbus_domain_disconnect(struct kdbus_domain *domain)
{
	...
	if (domain->major > 0) {
		idr_remove(&kdbus_domain_major_idr, (int)domain->major);
...
struct kdbus_domain *kdbus_domain_find_by_major(unsigned int major)
{
	...
	mutex_lock(&kdbus_subsys_lock);
	domain = idr_find(&kdbus_domain_major_idr, (int)major);
	if (domain)
...
exit_idr:
	idr_remove(&kdbus_domain_major_idr, (int)d->major);
exit_chrdev:
 ...
 * Return: 0 on success, negative errno on failure
 */
int kdbus_domain_make_user(struct kdbus_cmd_make *cmd, const char **name)
{
...
static void __kdbus_domain_user_free(struct kref *kref)
{
	...
	idr_remove(&user->domain->user_idr, (int)user->idr);
