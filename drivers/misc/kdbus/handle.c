			     ...
			     size_t size_max)
{
	...
	ptr = memdup_user(user_ptr, (size_t)size);
	if (IS_ERR(ptr))
		return PTR_ERR(ptr);
...
static int kdbus_handle_memfd(void __user *buf)
{
	...
	ret = kdbus_memfd_new(n, (size_t)m->file_size, &fd);
	if (ret < 0)
		goto exit;

	/* return fd number to caller */
	addr = (int* __user)(((char *)buf) + offsetof(struct kdbus_cmd_memfd_make, fd));
	if (put_user(fd, addr)) {
				       ...
				       void __user *buf)
{
	...
	case KDBUS_CMD_BUS_MAKE: {
		...
		mutex_lock(&handle->domain->bus_start_lock);
		ret = kdbus_bus_new(handle->domain, make, name, &bloom,
				    mode, current_fsuid(), gid, &bus);
		mutex_unlock(&handle->domain->bus_start_lock);
		if (ret < 0)
			break;
					    ...
					    void __user *buf)
{
	...
	case KDBUS_CMD_NAME_ACQUIRE:
		...
		if (copy_to_user(buf, p, (unsigned long int)size))
			ret = -EFAULT;
	...
	case KDBUS_CMD_NAME_LIST: {
		...
		cmd_list = p;
		ret = kdbus_cmd_name_list(conn->bus->name_registry, conn, cmd_list);
		if (ret < 0)
			break;
	...
	case KDBUS_CMD_CONN_INFO: {
		...
		cmd_info = p;
		ret = kdbus_cmd_conn_info(conn, cmd_info, (size_t)size);
		if (ret < 0)
			break;
	...
	case KDBUS_CMD_FREE: {
		...
		slice = kdbus_pool_slice_find(conn->pool, (size_t)off);
		if (!slice)
			return -ENXIO;
					...
					void __user *buf)
{
	...
	case KDBUS_CMD_EP_UPDATE: {
		struct kdbus_cmd_update *cmd_upd;
		...
		cmd_upd = p;
		ret = kdbus_ep_policy_set(ep, cmd_upd->items,
					  KDBUS_ITEMS_SIZE(cmd_upd, items));
		break;
	}
