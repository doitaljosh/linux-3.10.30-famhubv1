				...
				const void __user *from, size_t count)
{
	...
	if (fault_in_pages_readable(from, (int)count) < 0)
		return -EFAULT;
			      ...
			      size_t off_src, size_t len)
{
	...
	while (rem > 0) {
		...
		if (data)
			ret = kdbus_pool_copy_data(p, o, (const void __user *)((const char __user *)data + pos), n);
		else
...
{
	return (ssize_t)kdbus_pool_copy(slice, off, data, NULL, 0, len);
}
			      ...
			      const void *data, size_t len)
{
	...
	old_fs = get_fs();
	set_fs(get_ds());
	ret = (ssize_t)kdbus_pool_copy(slice, off,
			  ...
			  struct kdbus_pool_slice **slice)
{
	...
	old_fs = get_fs();
	set_fs(get_ds());
	ret = (int)kdbus_pool_copy(slice_new, 0, NULL,
...
void kdbus_pool_slice_flush(const struct kdbus_pool_slice *slice)
{
	...
	for (i = first; i < last; i++) {
		...
		/* swapped-out pages are already flushed */
