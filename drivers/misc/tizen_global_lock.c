...
/* #define TGL_DEBUG_ENABLED */
#ifdef TGL_DEBUG_ENABLED
#define TGL_DEBUG(x, y...) pr_debug(x, ##y)
#else
...
struct tgl_lock {
	...
	unsigned int		session_refcnt;	/* ref count for the session */
			 ...
			 unsigned int key)
{
	...
	if ((unsigned int)session_data == lock->owner) {
		mutex_lock(&lock->waiting_list_mutex);
	...
	if (ret == 0) {
		/**
		* FIXME: TGL timeout is due to Mali-t604 DDK improper locking mechanism.
		* This is temp warning disable. Enable it when DDK locking is fixed.
		*/

		/*
		TGL_WARN("timed out (%d), key: %d, sesseion_data(%d),\n"
			"requester(%d), owner_seesion_data(%d), owner(%d, %d)",
			lock->timeout_ms, key, (unsigned int)session_data,
			current->pid, lock->owner, lock->owner_pid,
			lock->owner_tid);
		*/
		mutex_lock(&lock->data_mutex);
		lock->refcnt--;
...
/* Adding a function wrapper to call TGL from kernel */
long tgl_ioctl_wrapper(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TGL_INIT_LOCK:
		...
		TGL_DEBUG("[%d] %s, tgl_init_lock, file: 0x%x, err: %d\n",
				__LINE__, __func__, file, err);
		break;
	case TGL_DESTROY_LOCK:
		/* destroy lock with id(=arg) */
		err = tgl_destroy_lock(session_data, (unsigned int)arg);
		TGL_DEBUG("[%d] %s, tgl_destroy_lock, file: 0x%x, err: %d\n",
				__LINE__, __func__, file, err);
		break;
	case TGL_LOCK_LOCK:
		/* lock lock with id(=arg) */
		err = tgl_lock_lock(session_data, (unsigned int)arg);
		TGL_DEBUG("[%d] %s, tgl_lock_lock, file: 0x%x, err: %d\n",
				__LINE__, __func__, file, err);
		break;
	case TGL_UNLOCK_LOCK:
		/* unlock lock with id(=arg) */
		err = tgl_unlock_lock(session_data, (unsigned int)arg);
		TGL_DEBUG("[%d] %s, tgl_unlock_lock, file: 0x%x, err: %d\n",
				__LINE__, __func__, file, err);
		break;
	case TGL_SET_DATA:
