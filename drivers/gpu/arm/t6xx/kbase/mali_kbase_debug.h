...
/* TGL debug print messages */
#ifdef CONFIG_TGL_KERNEL

#ifdef TGL_DEBUG
#define KBASE_TGL_DEBUG_PRINT(module, ...)\
		do {\
			printk(KERN_WARNING __VA_ARGS__);\
			printk(KERN_WARNING "\n");\
		} while (MALI_FALSE)
#else
#define KBASE_TGL_DEBUG_PRINT(module, ...) CSTD_NOP()
#endif
...
#define KBASE_DEBUG_PRINT_WARN(module, ...)\
	do {\
		printk(KERN_WARNING "\e[42m  Mali<WARN, %s>: %s function:%s\e[0m", kbasep_debug_module_to_str(module), KBASEP_DEBUG_PRINT_TRACE, KBASEP_DEBUG_PRINT_FUNCTION);\
		printk(KERN_WARNING __VA_ARGS__);\
		printk(KERN_WARNING "\n");\
	} while (MALI_FALSE)
#endif
...
#define KBASE_DEBUG_PRINT_ERROR(module, ...)\
	do {\
		printk(KERN_ERR "\e[41m Mali<ERROR, %s>: %s function:%s \e[0m ", kbasep_debug_module_to_str(module), KBASEP_DEBUG_PRINT_TRACE, KBASEP_DEBUG_PRINT_FUNCTION);\
		printk(KERN_ERR __VA_ARGS__);\
		printk(KERN_ERR "\n");\
	} while (MALI_FALSE)
