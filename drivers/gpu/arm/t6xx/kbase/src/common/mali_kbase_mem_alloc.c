...
static unsigned int kbase_carveout_pages = 0;
...
{
	if ( kbase_carveout_pages &&
		( page_to_pfn(p) >= kbase_carveout_start_pfn &&
...
void kbase_carveout_init(struct kbase_device *kbdev)
{
	unsigned long pfn;
	...
	debugfs = debugfs_create_file("kbase_carveout", S_IRUGO, NULL, NULL,
		    &kbase_carveout_debugfs_fops);

	if (!debugfs)
		dev_warn(kbdev->osdev.dev, "Failed to create kbase_carveout debugfs file\n");
}

int kbase_carveout_mem_register( phys_addr_t mem, phys_addr_t size )
{
	if (mem == 0 && size == 0)
	{
		pr_err("%s: Tried to register memory at %x of size %d for kbase carveout\n",__func__, (unsigned int)mem, (unsigned int)size);
		return -EINVAL ;
	}

	/* only allowed to register carve out once */
	if ( kbase_carveout_pages != 0 )
	{
		pr_err("Memory already registered for kbase carveout\n");
		return -EPERM;
	}
