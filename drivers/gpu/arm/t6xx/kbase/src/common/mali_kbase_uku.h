...
/* 
This structure should be added to katom
 */
typedef struct kbase_uk_tmem_tgl_params{
	/* TGL parameters */
	int tgl_key;
	int tgl_fd;
}kbase_uk_tmem_tgl_params;


typedef struct kbase_umm_tgl_params{
	/* TGL parameters */
	unsigned int is_locked;
	int tgl_key;
	int tgl_fd;
	struct file* tgl_filep; /* TGL kernel IOCTL wrapper call */
	u64 ext_resource; /* address of the external resource being used here */
	
}kbase_umm_tgl_params;
...
typedef struct kbase_uk_tmem_import {
	...
	u64 pages;
#ifdef CONFIG_TGL_KERNEL	 
	kbase_uk_tmem_tgl_params tgl_params;
#endif /* CONFIG_TGL_KERNEL */ 
...
#ifndef FOX_MALI_ORG
typedef struct kbase_uk_get_mem_info
{
	uk_header header;
}kbase_uk_get_mem_info;

typedef struct kbase_uk_get_fb_addr
{
	uk_header header;
	void *param;
}kbase_uk_get_fb_addr;
typedef struct kbase_uk_set_dvfs
{
	uk_header header;
	u32 dvfs_info;
}kbase_uk_set_dvfs;

typedef struct kbase_uk_set_maxfreq
{
	uk_header header;
	u32 dvfs_maxfreq;
}kbase_uk_set_maxfreq;

typedef struct kbase_uk_get_gpu_status
{
	uk_header header;
	void *param;
}kbase_uk_get_gpu_status;
#endif
...
typedef enum kbase_uk_function_id {
	...
	,KBASE_FUNC_MEM_INFO
#ifndef FOX_MALI_ORG
	,KBASE_FUNC_GET_FB_ADDR
	,KBASE_FUNC_GET_NEXT_FB_ADDR
#endif
	,KBASE_FUNC_SET_DVFS_MAXFREQ
	,KBASE_FUNC_SET_DVFS_INFO
	,KBASE_FUNC_GET_GPU_STATUS
