...
#ifdef CONFIG_TGL_KERNEL
	KBASE_TGL_DEBUG_PRINT(KBASE_JD,
		"kbase_jd_umm_map, reg->start_pfn %d, pa %016llX, nr_alloc_pages %u, dma_buf size %u  \n",
		reg->start_pfn,
		reg->nr_alloc_pages,
...
#ifdef CONFIG_KDS
extern void kds_resource_set_release_sync(struct kds_resource_set **pprset);
#endif
...
#ifdef CONFIG_TGL_KERNEL
	/* Unlock TGL here */
	if( katom->tgl_params_list){
		for( res_no = 0; res_no < katom->nr_extres; res_no++){
			if( NULL != katom->tgl_params_list[res_no].tgl_filep && 1 == katom->tgl_params_list[res_no].is_locked){
				int tgl_ret=0;
				KBASE_TGL_DEBUG_PRINT(KBASE_JD,
					"kbase_jd_post_external_resources going to release TGL lock filep 0x%x, fd %d, TGL_Key %u \n",
					katom->tgl_params_list[res_no].tgl_filep,
					katom->tgl_params_list[res_no].tgl_fd,
					katom->tgl_params_list[res_no].tgl_key);

				tgl_ret=tgl_ioctl_wrapper(katom->tgl_params_list[res_no].tgl_filep,
										TGL_UNLOCK_LOCK,
										katom->tgl_params_list[res_no].tgl_key);
				if( 0 != tgl_ret){
					KBASE_TGL_DEBUG_PRINT(KBASE_JD,
						"kbase_jd_post_external_resources  TGL unlock FAILED!!! fd %d TGL_Key %u return value %d \n",
						katom->tgl_params_list[res_no].tgl_fd,
						katom->tgl_params_list[res_no].tgl_key,
						tgl_ret);
				}
				else{
					katom->tgl_params_list[res_no].is_locked=1;
					KBASE_TGL_DEBUG_PRINT(KBASE_JD,
						"kbase_jd_post_external_resources  TGL unlock SUCCESSFUL!!! fd %d TGL_Key %u return value %d \n",
						katom->tgl_params_list[res_no].tgl_fd,
						katom->tgl_params_list[res_no].tgl_key,
						tgl_ret);
		...
		/* Free the resources allocated earlier */
		kfree(katom->tgl_params_list);
	}
#endif /* CONFIG_TGL_KERNEL */	
}
...
static mali_error kbase_jd_pre_external_resources(kbase_jd_atom *katom, const base_jd_atom_v2 *user_atom)
{
...
#ifndef FOX_MALI_ORG
	unsigned long lflags = KDS_FLAG_LOCKED_WAIT;
#endif
#endif				/* CONFIG_KDS */
...
#ifdef CONFIG_TGL_KERNEL
	KBASE_TGL_DEBUG_PRINT(
			KBASE_JD,
			"kbase_jd_pre_external_resources, katom->nr_extres %d, user_atom->nr_extres %d \n",
			katom->nr_extres, user_atom->nr_extres);

	/* Allocate memory to hold TGL parameters for all resources for this atom */
	katom->tgl_params_list=NULL;
	katom->tgl_params_list = kmalloc(sizeof(struct kbase_umm_tgl_params) * katom->nr_extres, GFP_KERNEL);
	if (NULL == katom->tgl_params_list) {
		err_ret_val = MALI_ERROR_OUT_OF_MEMORY;
		goto early_err_out;
	}
#endif /* CONFIG_TGL_KERNEL */
...
#ifdef CONFIG_TGL_KERNEL
	KBASE_TGL_DEBUG_PRINT(KBASE_JD,
		"kbase_jd_pre_external_resources, res_no %d, reg->imported_type %d ext_resource %016llX, tgl_key %d, tgl_fd %u, tgl_filep 0x%x \n",
		res_no,
		reg->imported_type,
		res->ext_resource,
		reg->imported_metadata.umm.tgl_key,
		reg->imported_metadata.umm.tgl_fd,
		reg->imported_metadata.umm.tgl_filep);
#endif /* CONFIG_TGL_KERNEL */
...
#ifdef CONFIG_TGL_KERNEL
				/* Add TGL resources */
				katom->tgl_params_list[res_no].tgl_filep=reg->imported_metadata.umm.tgl_filep;
				if( NULL != katom->tgl_params_list[res_no].tgl_filep){
					katom->tgl_params_list[res_no].tgl_fd=reg->imported_metadata.umm.tgl_fd;
					katom->tgl_params_list[res_no].tgl_key=reg->imported_metadata.umm.tgl_key;
					katom->tgl_params_list[res_no].is_locked=0;
					/* TODO: might need to remove it later */
					katom->tgl_params_list[res_no].ext_resource=katom->extres[res_no].ext_resource;
...
#ifdef CONFIG_TGL_KERNEL
				KBASE_TGL_DEBUG_PRINT(KBASE_JD,
					"kbase_jd_pre_external_resources, res_no %d, current_mapping_usage_count %d \n",
					res_no,
					reg->imported_metadata.umm.current_mapping_usage_count);
#endif /* CONFIG_TGL_KERNEL */
	...
	if( katom->tgl_params_list){
		for( res_no = 0; res_no < katom->nr_extres; res_no++){
			if( NULL != katom->tgl_params_list[res_no].tgl_filep  && 0 == katom->tgl_params_list[res_no].is_locked){
				int tgl_ret=0;
				KBASE_TGL_DEBUG_PRINT(KBASE_JD,
					"kbase_jd_pre_external_resources going to apply TGL lock filep 0x%x, fd %d, TGL_Key %u \n",
					katom->tgl_params_list[res_no].tgl_filep,
					katom->tgl_params_list[res_no].tgl_fd,
					katom->tgl_params_list[res_no].tgl_key);
				tgl_ret=tgl_ioctl_wrapper(katom->tgl_params_list[res_no].tgl_filep,
							TGL_LOCK_LOCK, katom->tgl_params_list[res_no].tgl_key);
				if( 0 != tgl_ret){
					KBASE_TGL_DEBUG_PRINT(KBASE_JD,
						"kbase_jd_pre_external_resources  TGL lock FAILED!!! fd %d TGL_Key %u return value %d \n",
						katom->tgl_params_list[res_no].tgl_fd,
						katom->tgl_params_list[res_no].tgl_key,
						tgl_ret);
				}
					...
					katom->tgl_params_list[res_no].is_locked=1;
					KBASE_TGL_DEBUG_PRINT(KBASE_JD,
						"kbase_jd_pre_external_resources  TGL lock SUCCESSFUL!!! fd %d TGL_Key %u return value %d \n",
						katom->tgl_params_list[res_no].tgl_fd,
						katom->tgl_params_list[res_no].tgl_key,
						tgl_ret);
				}
...
#else
		wait_failed = kds_async_waitall(&katom->kds_rset,lflags,
#endif
