...
#if 0
#ifndef FOX_MALI_ORG
static unsigned int cur_usage;
static struct mutex mem_usage_lock;
#endif
#endif
...
#if 0
#ifndef FOX_MALI_ORG
	mutex_lock(&mem_usage_lock);
	cur_usage +=  nr_pages;
	mutex_unlock(&mem_usage_lock);
#endif
#endif
...
#ifdef CONFIG_TGL_KERNEL
/* modified the function to accept tgl_params */
static struct kbase_va_region *kbase_tmem_from_umm(kbase_context *kctx, int fd, u64 * const pages, kbase_uk_tmem_tgl_params tgl_params)
#else
...
#endif /* CONFIG_TGL_KERNEL */
{
...
#ifdef CONFIG_TGL_KERNEL	
	/*Add tgl parameters here */
	reg->imported_metadata.umm.tgl_key=tgl_params.tgl_key;
	reg->imported_metadata.umm.tgl_fd=tgl_params.tgl_fd;
	reg->imported_metadata.umm.tgl_filep=fget_light(tgl_params.tgl_fd, &fput_needed);
	if(NULL == reg->imported_metadata.umm.tgl_filep){
		KBASE_TGL_DEBUG_PRINT(KBASE_MEM,"kbase_tmem_from_umm fget_light failed for fd %d \n", tgl_params.tgl_fd);
		fput_light(reg->imported_metadata.umm.tgl_filep, fput_needed);
	}
#endif
	...
	*pages = nr_pages;
#ifdef CONFIG_TGL_KERNEL	
	/* Where the memory is imported from DMA_BUF */
	KBASE_TGL_DEBUG_PRINT(KBASE_MEM,"kbase_tmem_from_umm fd %d, pages %llu, size %d\n",
		fd,
		*pages,
		dma_buf->size);
#endif /* CONFIG_TGL_KERNEL */
...
#ifdef CONFIG_TGL_KERNEL
/* Sending TGL parameters */
struct kbase_va_region *kbase_tmem_import(kbase_context *kctx, base_tmem_import_type type, int handle, u64 * const pages, kbase_uk_tmem_tgl_params tgl_params)
#else
...
#ifdef CONFIG_TGL_KERNEL
		/* Send tgl_params */
		return kbase_tmem_from_umm(kctx, handle, pages, tgl_params);
#else
