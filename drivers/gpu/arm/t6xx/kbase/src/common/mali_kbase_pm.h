...
typedef enum kbase_GPUfreq_level_index {
	Lev0=0,	 Lev1,	Lev2,	
	Lev3,	Lev4,	Lev5,	Lev6,	Lev7,	 Lev8
}kbase_GPUfreq_level_index;

#define GPUFREQ_ENTRY_INVALID ~0
#define GPUFREQ_TABLE_END     ~1
#define GPUFREQ_TEMP_LEVEL		3
#define GPUFREQ_LEVEL_END		8
#define MIN_GPU_FREQ_LEVEL		7
#define MAX_GPU_FREQ_LEVEL		2
#define GPUFREQ_ASV_COUNT		7
#define DEFAULT_GPU_ASV_GROUP	0
#define MAX_GPU_ASV_GROUP		7 /* must be same as GPUFREQ_ASV_COUNT */
#define GPUFREQ_EMA_COUNT		4	/* MAX EMA COUNT + 1 */


typedef struct gpufreq_frequency_table {
	unsigned int	index;     /* any */
	unsigned int	frequency; /* kHz - doesn't need to be in ascending */
	unsigned int 	min_threshold; 
	unsigned int 	max_threshold; 
}gpufreq_frequency_table;

typedef struct gpufreq_info {
	unsigned int	frequency;  /* kHz - doesn't need to be in ascending */
	unsigned int	pll_val; /* pll_val */
}gpufreq_info;

/* Forward definition - see mali_kbase.h */
typedef struct gputemp_frequency_table {
	unsigned int	index;     /* any */
	unsigned int	frequency; /* kHz - doesn't need to be in ascending */
	unsigned int 	ratio; 
}gputemp_frequency_table;

struct kbase_device;

typedef enum kbase_pm_dvfs_status
{
	DVFS_OFF=0,
	DVFS_ON,
	DVFS_FIX
} kbase_pm_dvfs_status;
...
mali_error kbasep_pm_metrics_resume(struct kbase_device *kbdev);
