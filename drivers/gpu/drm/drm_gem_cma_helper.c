...
void drm_gem_cma_describe(struct drm_gem_cma_object *cma_obj, struct seq_file *m)
{
	...
	seq_printf(m, "%2d (%2d) %08llx %08x %p %d",
			obj->name, obj->refcount.refcount.counter,
			off, (unsigned int)cma_obj->paddr, cma_obj->vaddr, obj->size);
				  ...
				  struct sg_table *sgt)
{
	...
	DRM_DEBUG_PRIME("dma_addr = 0x%08x, size = %zu\n", (unsigned int)cma_obj->paddr, size);
