...
inline int store_user_bt(struct kbdg_bt_trace *trace, unsigned int depth)
{
	...
	return (int)user_bt(&bt_regs, current->mm, trace, depth);
}
	...
	struct kbdg_bt_trace *trace, unsigned int depth)
{
	...
	unsigned int call_depth = 0, start_addr = 0, end_addr =
	    0, no_start_jump = 0;
	int fp_updated = 0, lr_updated = 0, lr_saved = 0;
	int first_inst = 1;
	int inst_ctr = 0;
	int nop_count = 0;
	...
	if (kdbg_elf_get_symbol_and_lib_by_mm(tsk_mm,
	...
	while (1) {
		...
		if (unlikely(first_inst)) {
		...
		} else if (inst == 0xe58de004) { /* str     lr, [sp, #4] */
			/* lr is stored update flag to adjust lr_offset later */
			lr_saved = 1;
		} else if ((inst & 0xffff0fff) == 0xe52d0008) {
			/* str instruction: str     lr, [sp, #4] */
			int offset = inst & 0xff;

			/* updated fp offset if fp is enabled */
			if (inst == 0xe52db008) {
				fp_offset = sp_offset;
				fp_updated = 1;
			}

			/* update lr offset if it is saved earlier */
			if (lr_saved) {
				/* update lr offset */
				lr_offset = sp_offset + 4;
				lr_updated = 1;
			...
			/* update sp with the offset */
			sp_offset += (unsigned int)offset;
			...
			if (0 == start_addr)
				no_start_jump = 1;
		} else if (inst == 0x00000000) {
			const int MAX_NOP_ALLOWED = 2;
			dprintk("NOP at addr: addr %x, inst %08x\n", addr,
				inst);
			if (++nop_count > MAX_NOP_ALLOWED) {
				dprintk("consecutive %d nop, skipping backtrace\n",
						nop_count);
				goto exit_stop_bt;
			}
		} else {
			/* we check only consecutive NOPs
			 * only stack operations and NOP are not possible
			 */
			nop_count = 0;
			/* Other instructions */
			dprintk("inst: %x skipped at addr: %x...\n", inst, addr);
		}
		...
		if (no_start_jump || (addr < start_addr)) {
			...
			lr_updated = 0;
			fp_updated = 0;
			nop_count = 0; /* reset for next frame */
			...
			symbol_info.pdf_info = &df_info;
				...
				if (kdbg_elf_get_symbol_and_lib_by_mm
						(tsk_mm, addr, lib_name,
			...
			/* this algo is based only for ARM32 instructions.
			 * if address is unaligned, return err,
			 * junk seen in case of dbus-daemon/acm-service
			 */
			if (lr_address & 1) {
				dprintk("unaligned address:%x skipping backtrace\n",
						lr_address);
				goto exit_stop_bt;
			}
 ...
 *
 * during task exit before memory drop, kernel make task->mm NULL.
 * so can not relying in task struct for mm.
 * Since this function can be used at the time of coredump Better to take on mm argument as argument.
 */
void show_user_backtrace_task(struct task_struct *tsk, int load_elf, struct mm_struct *tsk_mm)
{
	struct pt_regs *regs;
	...
	ts = tsk->se.exec_start;
	nsec = do_div(ts, NSEC_PER_SEC);
	sec = ts;

#ifdef CONFIG_SMP
	PRINT_KD("\n\nPid: %d, comm: %20s[%d] exec_start[%u.%09u]\n", tsk->pid, tsk->comm, task_cpu(tsk), sec, nsec);
#else
	PRINT_KD("\n\nPid: %d, comm: %20s\n", tsk->pid, tsk->comm);
#endif
...
#ifdef USE_KDEBUGD_BACKTRACE
		PRINT_KD
			("task stack info : stack area (0x%08lx ~ 0x%08lx)\n"
			 "REGS: FP: 0x%08x, PC: 0x%08x, RA: 0x%08x,"
...
#else
		kubt_kdbgd_show_trace(tsk_mm, &tsk_regs);
#endif
	}
}
