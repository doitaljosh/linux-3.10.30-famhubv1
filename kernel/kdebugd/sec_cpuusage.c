...
/* CPU usage BUG_ON */
#define cpuusage_bug(arg) \
	kdbg_bug(KDBG_CMD_CM_CPU_USAGE, arg)

/* CPU usage WARN_ON */
#define cpuusage_warn(arg) \
	kdbg_warn(KDBG_CMD_CM_CPU_USAGE, arg)

/* CPU usage ERROR */
#define cpuusage_error(fmt, args...) \
	kdbg_error(KDBG_CMD_CM_CPU_USAGE, fmt, ##args)

/* CPU usage output data */
#define cpuusage_write(data, len) \
	agent_write(KDBG_CMD_CM_CPU_USAGE, data, len)
...
/*
 * This function is called at 1 second interval from timer.
 */
static int sec_cpuusage_interrupt(void)
{
	int cpu = 0;
...
#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
/*
 * This function is called at 20 milisecond interval from timer.
 * Data will provided to Agent
 */
void sec_cpuusage_agent_interrupt(void)
{
	size_t ret = 0;
	int cpu = 0;
	struct sec_cpuusage_buf cpuusage;

	cpuusage.sec_count = kdbg_get_uptime();

	/* calculate usage percentage of each cpu and store in table */
	for_each_online_cpu(cpu) {

		/*
		 * FIXME: This is based on assumption that the counter doesn't cross 32-bit
		 * in 1 second. The receiver/reader should consider wrap-around.
		 */
		cpuusage.cpu_data[cpu].user = (int)(kcpustat_cpu(cpu).cpustat[CPUTIME_USER] +
			kcpustat_cpu(cpu).cpustat[CPUTIME_NICE]);
		cpuusage.cpu_data[cpu].system = (int)
			(kcpustat_cpu(cpu).cpustat[CPUTIME_SYSTEM] +
			kcpustat_cpu(cpu).cpustat[CPUTIME_SOFTIRQ] +
			kcpustat_cpu(cpu).cpustat[CPUTIME_IRQ]);
	}

	ret  = cpuusage_write(&cpuusage, SEC_CPUUSAGE_BUF_SIZE);
	if (!ret)
		cpuusage_error("Error in writing to RB.\n");
...
static int sec_cpuusage_set_state_impl(sec_counter_mon_state_t new_state)
{
	...
	case E_INITIALIZED:
		if (prev_state == E_NONE || prev_state == E_DESTROYED) {
			...
			atomic_set(&g_sec_cpuusage_state, E_INITIALIZED);
			/* Unregister interuppt function*/
				if (unregister_counter_monitor_func (sec_cpuusage_interrupt) < 0) {
	...
	case E_RUNNING:
		if (prev_state == E_INITIALIZED) {
			...
			/*Register interuppt function*/
			if (register_counter_monitor_func (sec_cpuusage_interrupt) < 0) {
				PRINT_KD("WARN: Fail to Register Counter Monitor function\n");
				PRINT_KD("WARN: Staus will not be changed\n");
				ret = -ERR_INVALID;
				break;
			}
...
void sec_cpuusage_on_off(void)
{
	...
	if (state == E_NONE || state == E_DESTROYED) {
		...
		/* Toggle cpu usage status in case of kdebugd running with agent */
	} else {
...
static struct timer_list cpuusage_auto_timer;

static void sec_cpuusage_timer_start(void)
{
	int state_ret = 0;
...
static void sec_cpuusage_timer_stop(void)
{
	int state_ret = 0;
	/* Make the status running */
...
static void cpuusage_auto_start(unsigned long duration)
{
	static int started;
	...
	if (!started) {
		...
		sec_cpuusage_start_work.data = (void *)NULL;
		sec_cpuusage_start_work.pwork =
		    (void *)sec_cpuusage_timer_stop;
		kdbg_workq_add_event(sec_cpuusage_start_work, NULL);
		started = 0;
		del_timer(&cpuusage_auto_timer);
	}
...
int kdbg_cpuusage_init(void)
{
	...
	/* setup your timer to call cpuusage_auto_timer_callback */
	  setup_timer(&cpuusage_auto_timer, cpuusage_auto_start, jiffies + msecs_to_jiffies(CONFIG_COUNTER_MON_FINISHED_SEC * 1000));
	  cpuusage_auto_timer.expires = jiffies + msecs_to_jiffies(CONFIG_COUNTER_MON_START_SEC * 1000);
	  /* setup timer interval to 200 msecs */
	  add_timer(&cpuusage_auto_timer);
#else
	if (sec_cpuusage_set_state(E_INITIALIZED, 1) < 0) {	/* Send with sync */
