/*
 *  kernel/kdebugd/aop/aop_report.c
 *
...
static void aop_cp_add_node(void)
{
	...
	if (g_aop_trace_depth >= 0 &&  (unsigned int)g_aop_trace_depth < sizeof(g_aop_cp_chain)/sizeof(*g_aop_cp_chain)) {
...
/* Show AOP cp report */
static int aop_cp_show_report(int option)
{
	int operation = 0;
	aop_caller_head_list *itr_head = NULL;
	int app_count = 0;
	...
	do {
		...
		if (operation <= 0 || (operation >  (app_count) && operation != 99)) {
...
static int aop_cp_report_menu(void)
{
	int operation = 0;
	int ret = 1;
...
static int aop_cp_generate_trace_samples(void)
{
	aop_caller_head_list *itr_head = NULL;
	...
	if (aop_nr_user_samples) {
		...
		aop_app_caller_list_head = (aop_caller_head_list **)KDBG_MEM_DBG_KMALLOC
			(KDBG_MEM_REPORT_MODULE, sizeof(aop_caller_head_list *) * ((size_t)app_count+1UL), GFP_KERNEL);
...
static void  aop_cp_process_current_call_sample(void)
{
	aop_caller_head_list *itr_head = NULL;
	aop_caller_head_list *tmp_head = NULL;
	aop_caller_head_list *prev_head = NULL;
...
static void aop_sort_image_list(int type, int cpu)
{
	...
	unsigned int sample_count_a = 0;
	unsigned int sample_count_b = 0;
...
static void aop_init_processing(void)
{
	...
	aop_samples_remaining = aop_cache.wr_offset / (int)AOP_ULONG_SIZE;
...
static void *aop_create_node(int type)
{
	void *ret_buf = NULL;
	aop_pid_list *tmp_tgid_data = NULL;
	aop_pid_list *tmp_tid_data = NULL;
...
static void aop_add_sample_tgid(void)
{
	aop_pid_list *tmp_tgid_data = NULL;
...
{
	if ((size_t)aop_samples_remaining >= size)
		return 1;
...
int aop_op_generate_app_samples(void)
{
	...
	if (aop_nr_user_samples) {
		...
		while (tmp_app_data) {
			sample_count =
			    (unsigned long)COUNT_SAMPLES(tmp_app_data, samples_count, NR_CPUS);
			perc = sample_count * 100UL / aop_nr_total_samples;
			PRINT_KD("%d\t%8lu %3lu%%\t%s\n", index, sample_count,
				 perc,
		...
		while (1) {
			...
			PRINT_KD("Select Option (1 to %d & Exit - 9999)==>",
				 index - 1);
			choice = (int)debugd_get_event_as_numeric(NULL, NULL);
...
int aop_op_generate_lib_samples(void)
{
	...
	if (aop_nr_user_samples) {
		...
		while (tmp_lib_data) {
			sample_count =
			    (unsigned long)COUNT_SAMPLES(tmp_lib_data, samples_count, NR_CPUS);
...
int aop_op_generate_report_tgid(void)
{
	...
	if (aop_nr_total_samples) {
		...
		while (tmp_tgid_data) {
			sample_count =
			    (unsigned long)COUNT_SAMPLES(tmp_tgid_data, samples_count,
					  NR_CPUS);
			perc = sample_count * 100 / aop_nr_total_samples;
			PRINT_KD("%8lu %3lu%%\t%d\t%.20s\n", sample_count,
					...
					while (tmp_tid_data) {
						if (tmp_tid_data->
							...
							perc =
							    ...
							    samples_count
							    [cpu_option] * 100U /
							    (unsigned int)aop_nr_total_samples;
							PRINT_KD
							    ("%8u %3u%%\t%d\t%d\t%.20s\n",
							     tmp_tid_data->
							     samples_count
