/*
 *  kernel/kdebugd/aop/aop_oprofile.c
 *
 *  Advance OProfile buffer sampling & configuration module
 *
...
#define AOP_CONFIG_DEFAULT_EVENT_BUFF		(1*AOP_KB*AOP_KB)
#define AOP_CONFIG_MIN_EVENT_BUFF			(256*AOP_KB)
...
enum {
	...
	AOP_CONFIG_SAMPLING_PERIOD,	/* to configure sampling interval for
							interrupts option */
	AOP_SHOW_SAMPLING_PERIOD,	/* to show sampling interval for
							interrupts option */
#endif
};
...
static void aop_free_event_cache(void)
{
	...
	if (aop_cache.buffer) {
		KDBG_MEM_DBG_VFREE(aop_cache.buffer);
...
{
	/* allocat memory for given buffer size */
	aop_cache.buffer = KDBG_MEM_DBG_VMALLOC(KDBG_MEM_SAMPLING_BUFFER_MODULE,
						aop_event_buffer_size);

	/* BUG ON when failed to allocate memory */
	WARN_ON(!aop_cache.buffer);
	if (!aop_cache.buffer && (aop_event_buffer_size > AOP_CONFIG_MIN_EVENT_BUFF)) {
		/*return -ENOMEM;*/	/* no memory!! */
		aop_errk ("Failed memory alloc aop_alloc_event_cache %lu\n", aop_event_buffer_size);
		PRINT_KD("Allocating Minimum buffer size of  %dKB  required "
			"for oprofiling\n", AOP_CONFIG_MIN_EVENT_BUFF/AOP_KB);
		aop_event_buffer_size = AOP_CONFIG_MIN_EVENT_BUFF;
		/* allocat memory for given buffer size */
		aop_cache.buffer = KDBG_MEM_DBG_VMALLOC(KDBG_MEM_SAMPLING_BUFFER_MODULE,
						aop_event_buffer_size);
		if (!aop_cache.buffer)
...
static int aop_put_data(void)
{
	...
	remain_buff_size = aop_event_buffer_size - (unsigned long)(aop_cache.wr_offset);
	if (!remain_buff_size) {
...
#define AOP_BUFFER_FULL_EVENT "q"
...
static int aop_buff_task_sync(void *arg)
{
	...
	if (buff_full) {
		/* sending "q" key as an event to pop up AOP menu after
		 * AOP stops due to buffer full */
		debugd_event_t temp_event = {AOP_BUFFER_FULL_EVENT};
...
static int aop_config_set_buffer_size(void)
{
	...
	int cur_buff_size = (int)aop_event_buffer_size / AOP_KB;
...
struct counter_map {
	const char *name; /* Name to show in the menu */
	const u64 perf_counter; /* corresponding perf counter in kernel */
	int interval; /* number of counters per interrupt */
};

static struct counter_map aop_counter_map[PMU_MAX_TYPE] = {
	{
		"CPU Cycles",
	...
	{
		"L1D Cache Miss",
		PERF_COUNT_HW_CACHE_L1D | (PERF_COUNT_HW_CACHE_RESULT_MISS << 16),
		CONFIG_AOP_L1D_MISSES_SAMPLING_PERIOD
	},
	{
		"L1I Cache Miss",
		PERF_COUNT_HW_CACHE_L1I | (PERF_COUNT_HW_CACHE_RESULT_MISS << 16),
		CONFIG_AOP_L1I_MISSES_SAMPLING_PERIOD
	},
	{
		"DTLB Miss",
		PERF_COUNT_HW_CACHE_DTLB | (PERF_COUNT_HW_CACHE_RESULT_MISS << 16),
		CONFIG_AOP_DTLB_MISSES_SAMPLING_PERIOD
	},
	{
		"ITLB Miss",
		PERF_COUNT_HW_CACHE_ITLB | (PERF_COUNT_HW_CACHE_RESULT_MISS << 16),
		CONFIG_AOP_ITLB_MISSES_SAMPLING_PERIOD
	},
	{
		"BPU Error",
		PERF_COUNT_HW_CACHE_BPU | (PERF_COUNT_HW_CACHE_RESULT_MISS << 16),
		CONFIG_AOP_BPU_ERRORS_SAMPLING_PERIOD
	}
};

static int aop_get_sampling_interval(enum pmu_counter counter)
{
	if ((counter < PMU_CYCLES) || (counter >= PMU_MAX_TYPE))
		return 0;

	return aop_counter_map[counter].interval;
}

static const char *aop_get_counter_name(enum pmu_counter counter)
{
	if ((counter < PMU_CYCLES) || (counter >= PMU_MAX_TYPE))
		return NULL;

	return aop_counter_map[counter].name;
}

static int aop_set_sampling_interval(enum pmu_counter counter, int interval)
{
	if ((counter < PMU_CYCLES) || (counter >= PMU_MAX_TYPE))
		return -1;
	...
	aop_counter_map[counter].interval = interval;
	return 0;
}

static void aop_config_sampling_period(void)
{
	int interval = 0;
	enum pmu_counter counter;

	for (counter = PMU_CYCLES; counter < PMU_MAX_TYPE; counter++) {
		PRINT_KD("\n");
		PRINT_KD("%s Sampling period (current: %d) ==> ",
				aop_get_counter_name(counter),
				aop_get_sampling_interval(counter));

		interval = debugd_get_event_as_numeric(NULL, NULL);
		PRINT_KD("\n");

		if (aop_set_sampling_interval(counter, interval) < 0)
...
static void aop_show_sampling_period(void)
{
	enum pmu_counter counter;

	for (counter = PMU_CYCLES; counter != PMU_MAX_TYPE; counter++) {
		PRINT_KD("\n");
		PRINT_KD("%s Sampling period : %d\n",
				aop_get_counter_name(counter),
				aop_get_sampling_interval(counter));
	}
	PRINT_KD("=======================================================\n");
...
static void aop_config_menu(void)
{
...
#ifdef CONFIG_CACHE_ANALYZER
	PRINT_KD("4)  aop config: set sampling interval for PMU\n");
	PRINT_KD("5)  aop config: show sampling interval for PMU\n");
#endif
	...
	case AOP_CONFIG_BUFF_SIZE:
		...
		if (aop_config_set_buffer_size() < 0)
			aop_errk("Failed memory alloc aop_config_set_buffer_size\n");
		break;
...
#ifdef CONFIG_CACHE_ANALYZER
	case AOP_CONFIG_SAMPLING_PERIOD:
		aop_config_sampling_period();
		break;

	case AOP_SHOW_SAMPLING_PERIOD:
		aop_show_sampling_period();
		break;
#endif
...
#endif
			if (!strncmp(event.input_string, AOP_BUFFER_FULL_EVENT,
				strlen(AOP_BUFFER_FULL_EVENT))) {
				aop_printk("AOP buffer Full\n");
				ret = 1;
				break; /*AOP buffer Full */
			}
...
static struct timer_list aop_auto_timer;

/* Auto start loging the cpu usage data */
static void aop_auto_start(unsigned long duration)
{
	static int started;
	...
	if (!started) {
		...
		mod_timer(&aop_auto_timer, duration);
		started = 1;
...
static void aop_auto_start_wr(void *data)
{
	...
	setup_timer(&aop_auto_timer, aop_auto_start, jiffies + msecs_to_jiffies(CONFIG_AOP_DURATION * 1000));
	aop_auto_timer.expires = jiffies + msecs_to_jiffies(CONFIG_AOP_START_SEC * 1000);
	/* setup timer interval to 200 msecs */
	add_timer(&aop_auto_timer);
