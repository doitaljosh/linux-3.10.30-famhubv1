				...
				unsigned long address, unsigned long *offset)
{
	...
	if ((size_t)buf_len < (AOP_MAX_SYM_LEN)) {
	...
	*offset = 0;
	...
	/* On overflow of buffer, it would have already corrupted
	   ...
	   the beginning. */
	WARN_ON(strlen(buffer) >= (size_t)buf_len);
}
...
static struct list_head *__aop_sym_report_alloc_list_head(void)
{
	...
	if (count) {
		...
		*app_cookie =
		    ...
		    KDBG_MEM_DBG_KMALLOC(KDBG_MEM_REPORT_MODULE,
					 (size_t)count * sizeof(aop_cookie_t),
					 GFP_KERNEL);
		if (!*app_cookie) {
			aop_errk("No memory\n");
			return 0;
...
int aop_sym_report_per_tid(pid_t pid_user)
{
	...
	list_for_each(pos, aop_sym_list_head) {
		...
		if (tmp_sym_data && (tmp_sym_data->tid == pid_user)) {
			unsigned long offset = 0;
			unsigned int sym_addr_local = tmp_sym_data->addr;
			struct aop_sym_list *plist;
			...
			if (tmp_sym_data->in_kernel) {
				aop_decode_cookie_kernel_symbol(func_name,
								...
								tmp_sym_data->
								addr, &offset);
				sym_addr_local = tmp_sym_data->addr - offset;
			} else {
				struct aop_symbol_info symbol_info_local;
				symbol_info_local.pfunc_name = func_name;
				/* get library image name */
				aop_decode_cookie_without_path(tmp_sym_data->
							       img_cookie,
							       img_name,
...
#ifdef CONFIG_ELF_MODULE
#ifdef CONFIG_DWARF_MODULE
				symbol_info_local.df_info_flag = 0;
				symbol_info_local.pdf_info = NULL;
#endif /* CONFIG_DWARF_MODULE */
				kdbg_elf_get_symbol(img_name,
						    tmp_sym_data->addr,
						    AOP_MAX_SYM_NAME_LENGTH,
						    &symbol_info_local);
				sym_addr_local = symbol_info_local.start_addr;
				virt_addr = symbol_info_local.virt_addr;
#else
				sym_addr_local = tmp_sym_data->addr;
#endif /* CONFIG_ELF_MODULE */
			}
			...
			if (add_new_entry) {
				...
				if (new_sym_data) {
					/* assign the sample information to  new sym data */
					new_sym_data->addr = sym_addr_local;
...
int aop_sym_report_system_wide_function_samples(void)
{
	...
	list_for_each(pos, func_list_head) {
		...
		if (plist) {
			unsigned int virt_addr = 0;
			unsigned int perc =
				((plist->samples * 100U) / (unsigned int)aop_nr_total_samples);
