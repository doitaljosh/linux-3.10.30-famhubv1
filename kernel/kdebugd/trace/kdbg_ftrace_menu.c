 ...
 * handles the tab feature for displaying the
 */
void kdbg_ftrace_handle_tab_event(void)
{
...
static int is_pseudo_tracer(struct kdbg_ftrace_conf *pfconf)
{
	...
	for (ctr = 0; ctr < FTRACE_SUPPORTED_TRACERS && pfconf->supported_list[ctr].trace_name; ctr++) {
...
static int append_func_list(char **func_list, char *func_name, int *size)
{
	...
	len_used = (int)(len_list + len_buf + 2);
	len_unused  =  len_total - len_used;

	debug("len_buf %u, len_list %u, len_total %d, len_used %d, len_unused %d\n"
	...
	if (len_unused <= 0) {
		*func_list = KDBG_MEM_DBG_KREALLOC(KDBG_MEM_FTRACE_MODULE, *func_list,
				(size_t)((*size + 1) * KDEBUGD_FTRACE_FUNC_NAME_LEN), GFP_KERNEL);
		if (*func_list == NULL) {
...
/*
 * get_tabbed_buffer
 * gets the tabbed buffer from the user.
 */
static char *get_tabbed_buffer(void)
{
	debugd_event_t event;
	static char *buf;
	size_t len = 0;

	/* enable tab feature of ftrace */
	kdbg_set_tab_action(KDBG_TAB_FTRACE);

	debugd_get_event(&event);
	menu("\n");

	/* disable tab feature of ftrace */
...
static void handle_ftrace_pid_filter(struct kdbg_ftrace_conf *pfconf)
{
	...
	menu("PID/Name (-1 to Reset) ==>  ");

	/* enable kdebugd tab feature */
	kdbg_set_tab_action(KDBG_TAB_SHOW_TASK);

	pid = debugd_get_event_as_numeric(&core_event, &is_number);

	if (pid < 0)
		pid = kdbg_tab_process_event(&core_event);

	menu("\n");

	/* disable kdebugd tab feature */
	kdbg_set_tab_action(KDBG_TAB_NO_ACTION);
 ...
 */
static int reset_transient_config(struct kdbg_ftrace_conf *pfconf, char *trace_name)
{
	int ret = 0;
		...
		case E_SUBMENU_OPTION_BUFFER_SIZE:
			{
				...
				for_each_possible_cpu(cpu) {
				ret = kdbg_ftrace_set_trace_buffer_size((unsigned long)buffer_size_kb, cpu);
				if (ret < 0) {
...
static int handle_ftrace_config(void)
{
	...
	if (CONFIG_FTRACE_DEFAULT_TRACE_LIST[0] != '\0') {
		...
		pfconf->trace_list = KDBG_MEM_DBG_KMALLOC(KDBG_MEM_FTRACE_MODULE,
				(size_t)(pfconf->trace_list_size * KDEBUGD_FTRACE_FUNC_NAME_LEN), GFP_KERNEL);
	...
	if (CONFIG_FTRACE_DEFAULT_TRACE_NOT_LIST[0] != '\0') {
		...
		pfconf->trace_not_list = KDBG_MEM_DBG_KMALLOC(KDBG_MEM_FTRACE_MODULE,
				(size_t)(pfconf->trace_not_list_size * KDEBUGD_FTRACE_FUNC_NAME_LEN), GFP_KERNEL);
	...
	if (CONFIG_FTRACE_DEFAULT_TRACE_GRAPH_LIST[0] != '\0') {
		...
		pfconf->trace_graph_list = KDBG_MEM_DBG_KMALLOC(KDBG_MEM_FTRACE_MODULE,
				(size_t)(pfconf->trace_graph_list_size * KDEBUGD_FTRACE_FUNC_NAME_LEN), GFP_KERNEL);
