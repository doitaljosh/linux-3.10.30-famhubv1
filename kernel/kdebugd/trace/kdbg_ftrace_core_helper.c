...
static int kdbg_ftrace_print_log_banner(struct kdbg_ftrace_conf *pfconf,	struct trace_iterator *iter, int lat)
{
	...
	struct trace_array_cpu *data = per_cpu_ptr(tr->trace_buffer.data, tr->trace_buffer.cpu);
	struct tracer *type = global_trace.current_trace;
	unsigned long total;
...
#define BANNER_APPEND(x, ...)	do { if (TRACE_MAX_PRINT - ret > 0) { \
	ret += snprintf(trace_banner + ret, (size_t)(TRACE_MAX_PRINT - ret), x, ##__VA_ARGS__); \
} \
	} while (0)
	...
	if (lat) {
		...
		entries = ring_buffer_entries(iter->tr->trace_buffer.buffer);
		total = entries +
			ring_buffer_overruns(iter->tr->trace_buffer.buffer);

		debug("entries %lu, overuns %lu, total %lu\n", entries,
				ring_buffer_overruns(iter->tr->trace_buffer.buffer), total);
...
static int kdbg_trace_init_global_iter(struct trace_iterator *iter)
{
	/* reset iterator */
	memset(iter, 0, sizeof(*iter));

#ifdef CONFIG_TRACER_MAX_TRACE
	if (global_trace.current_trace && global_trace.allocated_snapshot && global_trace.current_trace->print_max)
		iter->trace_buffer = &global_trace.max_buffer;
	else
#endif
...
int kdbg_ftrace_dump(struct kdbg_ftrace_conf *pfconf)
{
	...
	trace_flags &= (unsigned long)~TRACE_ITER_SYM_USEROBJ;
	...
	/* set nano second timestamp flag */
...
int kdbg_ftrace_set_trace_name(char *trace_name)
{
	...
	for (i = (int)strlen(trace_name); i > 0 && isspace(trace_name[i]); i--)
		trace_name[i] = 0;
...
unsigned long kdbg_ftrace_get_trace_buffer_size(int cpu)
{
	...
	val = per_cpu_ptr(tr->trace_buffer.data, cpu)->entries >> 10 ;
...
int kdbg_ftrace_set_trace_buffer_size(unsigned long val, int cpu)
{
	...
	if (val != per_cpu_ptr(global_trace.trace_buffer.data, cpu)->entries) {
...
int kdbg_ftrace_set_trace_cpu_mask(char *buf)
{
	...
	for_each_tracing_cpu(cpu) {
		 ...
		 */
		if (cpumask_test_cpu((unsigned int)cpu, tracing_cpumask) &&
				!cpumask_test_cpu((unsigned int)cpu, tracing_cpumask_new)) {
			atomic_inc(&per_cpu_ptr(global_trace.trace_buffer.data, cpu)->disabled);
		}
		if (!cpumask_test_cpu((unsigned int)cpu, tracing_cpumask) &&
				cpumask_test_cpu((unsigned int)cpu, tracing_cpumask_new)) {
			atomic_dec(&per_cpu_ptr(global_trace.trace_buffer.data, cpu)->disabled);
		}
	}
...
int kdbg_ftrace_set_trace_option(char *buf)
{
	...
	for (i = 0; trace_options[i]; i++) {
		if (strcmp(cmp, trace_options[i]) == 0) {
			ret = set_tracer_flag(&global_trace, (unsigned int)(1 << i), !neg);
			break;
		}
	}
...
static inline void kdbg_ftrace_save_stack_trace_user(struct stack_trace *trace)
{
	...
	store_user_bt((struct kbdg_bt_trace *)trace, (unsigned int)(pfconf->trace_call_depth));
}
