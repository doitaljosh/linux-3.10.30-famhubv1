...
/* global variables taken for appending the profile dump output */
static char *trace_profile_dump;
static int trace_profile_ret;

/* macros to append to trace_profile_dump */
#define TRACE_MAX_PRINT 1000
#define PROFILE_DUMP_APPEND(x, ...)   do { if (TRACE_MAX_PRINT - trace_profile_ret > 0) { \
	trace_profile_ret += snprintf(trace_profile_dump + trace_profile_ret, TRACE_MAX_PRINT - trace_profile_ret, x, ##__VA_ARGS__); \
} \
} while (0)
...
static void print_graph_duration(unsigned long long duration)
{
	...
	/* Print msecs */
	PROFILE_DUMP_APPEND("%s", msecs_str);
	...
	if (len < 7) {
		...
		PROFILE_DUMP_APPEND(".%s", nsecs_str);
		len += strlen(nsecs_str);
	}

	PROFILE_DUMP_APPEND(" us ");
...
int kdbg_ftrace_profile_dump_stat(void *v)
{
	...
	trace_profile_dump = (char *)KDBG_MEM_DBG_KMALLOC(KDBG_MEM_FTRACE_MODULE,
	TRACE_MAX_PRINT, GFP_KERNEL);

	if (!trace_profile_dump) {
	...
	PROFILE_DUMP_APPEND("  %-30.30s  %10lu", str, rec->counter);
	...
	/* de-allocate the memory and reset the counter */
	KDBG_MEM_DBG_KFREE(trace_profile_dump);
	trace_profile_dump = NULL;
	trace_profile_ret = 0;
