...
static void futex_config_menu(void)
{
	...
	do {
		...
		case 4:
			if (kdbg_futex_dbg_pid == -1) {
				PRINT_KD("Enter the PID/NAME (current: No PID)==> ");
			} else {
				PRINT_KD("Enter the PID/NAME (current: %d)==> ",
						kdbg_futex_dbg_pid);
			}

			/* Enable tab feature of kdebugd */
			kdbg_set_tab_action(KDBG_TAB_SHOW_TASK);
			dbg_pid = debugd_get_event_as_numeric(&event, NULL);
			if (dbg_pid < 0)
				dbg_pid = kdbg_tab_process_event(&event);
