...
/* disk usage ERROR */
#define diskusage_error(fmt, args...) \
	kdbg_error(KDBG_CMD_CM_DISK_USAGE, fmt, ##args)

/* disk usage output data */
#define diskusage_write(data, len) \
	agent_write(KDBG_CMD_CM_DISK_USAGE, data, len)
...
static void diskusage_show_header(void)
{
			     ...
			     struct sec_diskdata *pDisk_data)
{
	...
	if (hd) {
		...
		pDisk_data->nsect_read += (long)part_stat_read(hd, sectors[0]);
		pDisk_data->nsect_write += (long)part_stat_read(hd, sectors[1]);
...
#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
void kdbg_get_disk_stat_agent(void)
{
	size_t ret = 0;
	struct sec_diskdata_agent disk_data_agent = { 0};

	sec_diskusage_update(&disk_data_agent.disk_data);
	disk_data_agent.sec = kdbg_get_uptime();

	ret = diskusage_write(&disk_data_agent, sizeof(disk_data_agent));
	if (!ret)
		diskusage_error("Error in writing to RB.\n");
}
#endif
...
static void dump_disk_result(void)
{
	...
	/* No Need to take the lock as we are only reading the data*/
	if (sec_diskusage_init_flag) {
...
bool sec_diskusage_init(void)
{
	...
	if (register_counter_monitor_func(sec_diskstats_dump) < 0) {
...
void sec_diskusage_destroy(void)
{
	...
	if (unregister_counter_monitor_func(sec_diskstats_dump) < 0) {
 ...
 */
static void turnoff_diskusage(void)
{
	if (sec_diskusage_status) {
		sec_diskusage_status = 0;
		PRINT_KD("\n");
...
static struct timer_list diskusage_auto_timer;
/* Auto start loging the cpu usage data */
static void diskusage_auto_start(unsigned long duration)
{
	static int started;
	...
	if (!started) {
		...
		mod_timer(&diskusage_auto_timer, duration);
		started = 1;
		...
		if (!sec_diskusage_status)
			sec_diskusage_run_state = 0;
		started = 0;
		del_timer(&diskusage_auto_timer);
	}
	return;
...
int kdbg_diskusage_init(void)
{
	...
	/* setup your timer to call diskusage_auto_timer_callback */
	setup_timer(&diskusage_auto_timer, diskusage_auto_start, jiffies + msecs_to_jiffies(CONFIG_COUNTER_MON_FINISHED_SEC * 1000));
	diskusage_auto_timer.expires = jiffies + msecs_to_jiffies(CONFIG_COUNTER_MON_START_SEC * 1000);
	/* setup timer interval to 200 msecs */
	add_timer(&diskusage_auto_timer);
#else
	sec_diskusage_run_state = 1;
#endif
