...
/* CPU usage ERROR */
#define topthread_error(fmt, args...) \
	kdbg_error(KDBG_CMD_CM_THREAD_USAGE, fmt, ##args)

/* CPU usage output data */
#define topthread_write(data, len) \
	agent_write(KDBG_CMD_CM_THREAD_USAGE, data, len)
...
#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
static void agent_send_thread_usage(int buf_index)
{
	size_t written;

	written = topthread_write(g_thread_info_table_chain[buf_index],
				sizeof(*g_thread_info_table_chain[buf_index]));
	if (!written)
		topthread_error("Error in writing to RB.\n");
}
#endif /* CONFIG_KDEBUGD_AGENT_SUPPORT */
...
#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
		if (CM_START == agent_topthread_get_status()) {
			/* Send the data to TVis */
			agent_send_thread_usage(buf_index);
...
/*This function called by tothread_timer at one second interval*/
static int sec_topthread_process_data(void)
{
	spin_lock_irq(&sec_topthread_write_lock);
	...
	spin_unlock_irq(&sec_topthread_write_lock);
	return 0;
}
...
void sec_topthread_timer_interrupt_handler(int cpu)
{
	...
	/* called from interrupt context, irqs already disabled */
...
int sec_topthread_init(void)
{
	...
	for (i = 0; i < THREADINFO_CHAIN_LENGTH; i++) {
		...
		g_thread_info_table_chain[i] = (struct thread_info_table *)
			KDBG_MEM_DBG_VMALLOC(KDBG_MEM_KDEBUGD_MODULE,
...
ERR:
	...
	for (i = 0; i < THREADINFO_CHAIN_LENGTH; i++) {
		if (g_thread_info_table_chain[i]) {
			KDBG_MEM_DBG_VFREE(g_thread_info_table_chain[i]);
			g_thread_info_table_chain[i] = NULL;
		}
	}

	if (g_ptopthread_proc_buff_) {
		KDBG_MEM_DBG_VFREE(g_ptopthread_proc_buff_);
		g_ptopthread_proc_buff_ = NULL;
	}

	if (g_ptopprocess_proc_buff_) {
		KDBG_MEM_DBG_VFREE(g_ptopprocess_proc_buff_);
	...
	SEC_TOPTHREAD_DEBUG("old state %d --> new state %d\n", prev_state,
		...
		case E_INITIALIZED:
			if (prev_state == E_NONE || prev_state == E_DESTROYED) {
				...
				if (unregister_counter_monitor_func (sec_topthread_process_data) < 0) {
		...
		case E_RUNNING:
			if (prev_state == E_INITIALIZED) {
				if (register_counter_monitor_func (sec_topthread_process_data) < 0) {
...
static struct timer_list topthread_auto_timer;

static void sec_topthread_timer_start(void)
{
	int state_ret = 0;
...
static void sec_topthread_timer_stop(void)
{
	int state_ret = 0;
	/* Make the status running */
	sec_counter_mon_state_t state = sec_topthread_get_state();
...
static void topthread_auto_start(unsigned long duration)
{
	static int started;
	sec_counter_mon_state_t state = sec_topthread_get_state();
	...
	if (state == E_DESTROYING) {
		...
		del_timer(&topthread_auto_timer);
		return;
	}
...
#else
#define BUFF_APPEND(x, ...)   do { int pos = TOPTHREAD_MAX_PRINT - ret - 1;\
	if (pos > 0) {\
		ret += snprintf(print_buff + ret, pos, x, ##__VA_ARGS__);\
		print_buff[ret] = 0;\
	} \
	if (row_len > (pos) || (ret == (TOPTHREAD_MAX_PRINT - 1))) {\
		print_buff[ret] = 0;\
		printk("%s", print_buff);\
		ret = 0;\
		print_buff[0] = 0;\
...
static void csf_show_entry(int index)
{
...
#endif /*CONFIG_SMP */
	{
		...
		for (i = 0; i < ptop_proc_buff_copy[index].item_per_cpu[cpu].
				max_thread; i++) {
			/*Take RCU read lock register can be changed */
			rcu_read_lock();
			/* get the data from rearranged array of thread cpu usage */
...
int kdbg_topthread_init(void)
{
	...
	/* setup your timer to call topthread_auto_timer_callback */
	setup_timer(&topthread_auto_timer, topthread_auto_start, jiffies + msecs_to_jiffies(CONFIG_COUNTER_MON_FINISHED_SEC * 1000));
	topthread_auto_timer.expires = jiffies + msecs_to_jiffies(CONFIG_COUNTER_MON_START_SEC * 1000);
	/* setup timer interval to 200 msecs */
	add_timer(&topthread_auto_timer);
