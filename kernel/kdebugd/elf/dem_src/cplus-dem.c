...
__always_inline char *kdbg_elf_sym_demangle_wrap(const char *buff);
...
#define atoi(str) ((int)simple_strtoul(((str != NULL) ? str : ""), NULL, 0))	/* atio not defined in mips : Namit */
#endif

static char *aop_strdup(const char *str)
{
	...
	if (*size < min_size) {
		*size *= 2;
		if (*size < min_size)
			*size = min_size;
		*old_vect =
		    XRESIZEVAR(char, *old_vect, old_size, *size * (size_t)element_size);
	}
}
...
static char *ada_demangle(const char *mangled, int option ATTRIBUTE_UNUSED)
{
	...
	grow_vect(&demangled, &demangled_size, 2 * (size_t)len0 + 1, sizeof(char));
...
work_stuff_copy_to_from(struct work_stuff *to, struct work_stuff *from)
{
	...
	if (from->typevec_size)
		to->typevec = XNEWVEC(char *, (size_t)from->typevec_size);

	if (to->typevec) {
		for (i = 0; i < from->ntypes; i++) {
			size_t len = strlen(from->typevec[i]) + 1;
	...
	if (from->ksize)
		to->ktypevec = XNEWVEC(char *, (size_t)from->ksize);

	if (to->ktypevec) {
		for (i = 0; i < from->numk; i++) {
			size_t len = strlen(from->ktypevec[i]) + 1;
	...
	if (from->bsize)
		to->btypevec = XNEWVEC(char *, (size_t)from->bsize);

	if (to->btypevec) {
		for (i = 0; i < from->numb; i++) {
			size_t len = strlen(from->btypevec[i]) + 1;
	...
	if (from->ntmpl_args)
		to->tmpl_argvec = XNEWVEC(char *, (size_t)from->ntmpl_args);

	if (to->tmpl_argvec) {
		for (i = 0; i < from->ntmpl_args; i++) {
			size_t len = strlen(from->tmpl_argvec[i]) + 1;
	...
	if (**mangled == 'Y') {
		...
		if (**mangled == 'Q')
			...
			if (symbol_len == 0)
				...
				char *p = XNEWVEC(char, (size_t)symbol_len + 1), *q;
				if (p) {
		  ...
		  string *tname, string *trawname, int is_type, int remember)
{
	...
	if (!is_type) {
		...
		work->tmpl_argvec = XNEWVEC(char *, (size_t)r);
		work->ntmpl_args = r;
		for (i = 0; i < r; i++)
	...
	for (i = 0; i < r; i++) {
		...
		/* z for template parameters */
			...
			if (!is_type) {
				int len = s->p - s->b;
				work->tmpl_argvec[i] = XNEWVEC(char, (size_t)len + 1);
				...
				while (**mangled != '\0') {
					...
					default:
						if (ISDIGIT((unsigned char)*mangled[0])) {
							...
							n = (int)strcspn(*mangled, cplus_markers);
						}
						string_appendn(declp, *mangled, n);
						(*mangled) += n;
					}
		     ...
		     string *result, int namelength)
{
	...
	recurse = XNEWVEC(char, (size_t)namelength + 1);
	if (recurse) {
			  ...
			  string *result)
{
	...
	recurse = XNEWVEC(char, (size_t)literal_len + 1);
	if (recurse) {
...
static void remember_type(struct work_stuff *work, const char *start, int len)
{
	...
	if (work->ntypes >= work->typevec_size) {
		if (work->typevec_size == 0) {
			work->typevec_size = 3;
			work->typevec = XNEWVEC(char *, (size_t)work->typevec_size);
		} else {
			int old_size = work->typevec_size;
			work->typevec_size *= 2;
			work->typevec
				=
				XRESIZEVEC(char *, work->typevec, (size_t)old_size,
						(size_t)work->typevec_size);
		}
	}
	tem = XNEWVEC(char, (size_t)len + 1);
	if (tem) {
...
static void remember_Ktype(struct work_stuff *work, const char *start, int len)
{
	...
	if (work->numk >= work->ksize) {
		if (work->ksize == 0) {
			work->ksize = 5;
			work->ktypevec = XNEWVEC(char *, (size_t)work->ksize);
		} else {
			int old_size = work->ksize;
			work->ksize *= 2;
			work->ktypevec
			    =
			    XRESIZEVEC(char *, work->ktypevec, (size_t)old_size,
				       (size_t)work->ksize);
		}
	}
	tem = XNEWVEC(char, (size_t)len + 1);
...
static int register_Btype(struct work_stuff *work)
{
	...
	if (work->numb >= work->bsize) {
		if (work->bsize == 0) {
			work->bsize = 5;
			work->btypevec = XNEWVEC(char *, (size_t)work->bsize);
		} else {
			int old_size = work->bsize;
			work->bsize *= 2;
			work->btypevec
				=
				XRESIZEVEC(char *, work->btypevec, (size_t)old_size,
						(size_t)work->bsize);
		}
	}
	ret = work->numb++;
...
static void string_need(string *s, int n)
{
	...
	if (s->b == NULL) {
		...
		n += tem;
		n *= 2;
		s->b = XRESIZEVEC(char, s->b, (size_t)old_size, (size_t)n);
		s->p = s->b + tem;
		s->e = s->b + n;
	}
}
...
int kdbg_elf_sym_demangle(const char *buff, char *new_buff, int max_len)
{
	...
	if (ptr) {
		strncpy(new_buff, ptr, (size_t)max_len);
		KDBG_MEM_DBG_KFREE(ptr);
	} else {
		strncpy(new_buff, buff, (size_t)max_len);
	}
...
/*
 * wrapper function for public interface without allocation..
 */
__always_inline char * kdbg_elf_sym_demangle_wrap(const char *buff)
{
		return cplus_demangle(buff, DMGL_AUTO | DMGL_PARAMS);;
}
