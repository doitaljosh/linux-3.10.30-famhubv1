...
{
	const byte *addr = (const byte *) p;
	return (unsigned long)((addr[1] << 8) | addr[0]);
}
		...
		unsigned int *bytes_read_ptr)
{
	...
	num_read = 0;
	do {
		byte_read = (*(unsigned char *) (buf) & 0xff)  /*get8 (buf)*/;
		buf++;
		num_read++;
		result |= (((unsigned long) byte_read & 0x7f) << shift);
		shift += 7;
		...
		unsigned int *bytes_read_ptr)
{
	...
	if (shift < 8 * sizeof(result) && (byte_read & 0x40))
...
static void *df_malloc(size_t size)
{
	char *ptr = NULL;
		...
		int end_sequence)
{
	...
	if (filename && filename[0]) {
		...
		info->filename = (char *) df_malloc(file_len + 1UL);
		if (info->filename) {
...
decode_line_info(char *buff, int size, int *offset)
{
	...
	static size_t file_alloc_size;
	static size_t dir_alloc_size;
	...
	*offset += (int)(lh.total_length);
	...
	if (lh.total_length == 0xffffffff) {
		lh.total_length = (unsigned long)read_8_bytes(line_ptr);
		line_ptr += 8;
		offset_size = 8;
	}
	...
	lh.version = (unsigned short)read_2_bytes(line_ptr);
	line_ptr += 2;
	if (offset_size == 4)
		lh.prologue_length = read_4_bytes(line_ptr);
	else
		lh.prologue_length = (unsigned long)read_8_bytes(line_ptr);
	line_ptr += offset_size;
	lh.minimum_instruction_length = (unsigned char)read_1_byte(line_ptr);
	line_ptr += 1;
	lh.default_is_stmt = (unsigned char)read_1_byte(line_ptr);
	line_ptr += 1;
	lh.line_base = read_1_signed_byte(line_ptr);
	line_ptr += 1;
	lh.line_range = (unsigned char)read_1_byte(line_ptr);
	line_ptr += 1;
	lh.opcode_base = (unsigned char)read_1_byte(line_ptr);
	line_ptr += 1;
	...
	for (i = 1; i < lh.opcode_base; ++i) {
		lh.standard_opcode_lengths[i] = (unsigned char)read_1_byte(line_ptr);
		line_ptr += 1;
	}
	...
	while (line_ptr < line_end) {
		...
		while (!end_sequence) {
			...
			op_code = (unsigned char)read_1_byte(line_ptr);
			line_ptr += 1;
			...
			if (op_code >= lh.opcode_base) {
				...
				address += (unsigned long)((adj_opcode / lh.line_range)
					* lh.minimum_instruction_length);
				line += (unsigned int)(lh.line_base + (int)(adj_opcode % lh.line_range));
				...
				case DW_LNS_extended_op:
					/* Ignore length.  */
					line_ptr += 1;
					extended_op = (unsigned char)read_1_byte(line_ptr);
					line_ptr += 1;
					...
					case DW_LNE_set_address:
						address = (unsigned long)read_address(line_ptr);
				...
				case DW_LNS_advance_line:
					line += (unsigned int)(read_signed_leb128(line_ptr, &bytes_read));
					line_ptr += bytes_read;
					break;
				...
				case DW_LNS_const_add_pc:
					address += (unsigned long)(lh.minimum_instruction_length
				...
				default:
					{
						...
						for (j = 0; j < lh.standard_opcode_lengths[op_code]; j++) {
							(void) read_unsigned_leb128(line_ptr, &bytes_read);
							line_ptr += bytes_read;
						}
					}
	...
	while (line_ptr < line_end) {
		...
		while (!end_sequence) {
			...
			if (op_code >= lh.opcode_base) {
				...
				/* Special operand.  */
				adj_opcode = (unsigned char)(op_code - lh.opcode_base);
				address += (uint32_t)((adj_opcode / lh.line_range)
					* lh.minimum_instruction_length);
				...
				case DW_LNS_extended_op:
					...
					case DW_LNE_end_sequence:
						...
						break;
					case DW_LNE_set_address:
						address = (uint32_t)read_address(line_ptr);
						line_ptr += 4; /*unit->addr_size; */
						break;
					case DW_LNE_define_file:
						cur_file = read_string(line_ptr, &bytes_read);
				...
				case DW_LNS_set_basic_block:
					break;
				case DW_LNS_const_add_pc:
					address += (uint32_t)(lh.minimum_instruction_length
						* ((255 - lh.opcode_base) / lh.line_range));
					break;
				case DW_LNS_fixed_advance_pc:
					address += read_2_bytes(line_ptr);
