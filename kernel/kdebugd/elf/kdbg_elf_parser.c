...
static int kdbg_elf_chk_status(kdbg_elf_usb_elf_list_item *plist, char *filename)
{
	 ...
	 * whose interpretation depends on the section type.
	 */
	uint32_t dyn_symtab_str_link = 0;
	...
	bytesread = elf_filp->f_op->read(elf_filp, (char *)&pEhdr, sizeof(Elf32_Ehdr),
					&elf_filp->f_pos);
	if ((size_t)bytesread  < sizeof(Elf32_Ehdr)) {
	...
	if ((size_t)bytesread < sizeof(Elf32_Shdr)*pEhdr.e_shnum) {
...
kdbg_elf_gnu_debuglink_crc32 (unsigned long crc, unsigned char *buf, size_t len)
{
...
static int kdbg_elf_get_file_crc (kdbg_elf_usb_elf_list_item *plist_elf)
{
	...
	sym_printk("file name -> %s\n", plist_elf->elf_name_actual_path);

	/* File Open */
	elf_filp = filp_open(plist_elf->elf_name_actual_path, O_RDONLY | O_LARGEFILE, 0);
	...
	while ((count = elf_filp->f_op->read (elf_filp, buffer, sizeof(buffer), &elf_filp->f_pos)) > 0) {
		file_crc = kdbg_elf_gnu_debuglink_crc32 (file_crc, buffer, (size_t)count);
	}

	sym_printk("[BFD] - file Name = %s, CRC = 0x%08lx\n", plist_elf->elf_name_actual_path, file_crc);

	plist_elf->file_crc = file_crc;
		...
		char *filename, kdbg_elf_usb_elf_list_item *plist, int load_from_usb)
{
	...
	if ((size_t)bytesread < sizeof(Elf32_Phdr)*pEhdr.e_phnum) {
	...
	for (idx = 0; idx < pEhdr.e_shnum; idx++, lShdr++) {
		...
		if (idx == pEhdr.e_shstrndx) {
			...
			if ((uint32_t)bytesread < strtab_size) {
	...
	for (idx = 0; idx < pEhdr.e_shnum; idx++, lShdr++) {
		...
		if (!strcmp(KDBG_ELF_SECTION_NAME(lShdr, strtab_buf, strtab_size), ".gnu_debuglink")) {
			sym_printk(" %s %d\n", KDBG_ELF_SECTION_NAME(lShdr, strtab_buf, strtab_size), lShdr->sh_type);

			plist->gnu_debug_buf = KDBG_MEM_DBG_VMALLOC(KDBG_MEM_ELF_MODULE, strtab_size);
			if (plist->gnu_debug_buf == NULL) {
				sym_errk("[gnu_debug_buf] out of memory\n");
				goto ERR;
			}

			elf_filp->f_pos = (loff_t)lShdr->sh_offset;
			if (elf_filp->f_pos <= 0) {
				sym_errk("[%s]: error reading ""gnu_debug_buf_offset""file\n", filename);
				err = -EIO;
				goto ERR;
			}

			bytesread = elf_filp->f_op->read (elf_filp, plist->gnu_debug_buf, sizeof(char)*lShdr->sh_size, &elf_filp->f_pos);
			if (bytesread < (int) (sizeof(char)*lShdr->sh_size)) {
				sym_errk("byteread is not accurate %d %u\n", bytesread,
						lShdr->sh_size);
				goto ERR;
			}
			...
			crc_offset = (int)strlen (plist->gnu_debug_buf) + 1;
			crc_offset = (crc_offset + 3) & ~3;
			plist->gnu_dbg_file_crc = *((unsigned long *) (plist->gnu_debug_buf+crc_offset));

#if SYM_DEBUG_ON
			if (!strcmp(KDBG_ELF_SECTION_NAME (lShdr, strtab_buf, strtab_size), ".gnu_debuglink"))
				sym_printk(" %s\n", plist->gnu_debug_buf);
			sym_printk(" %s - 0x%08lx\n", plist->gnu_debug_buf, *((unsigned long *) (plist->gnu_debug_buf+crc_offset)));
	...
	if (sym_str_size) {
		...
		if ((kdbg_elf_elf_load_option == KDBG_ELF_LOAD_DEFAULT && load_from_usb) ||
			...
			if ((size_t)bytesread < sizeof(char)*sym_str_size) {
		...
		for (idx = 0; idx < symtab_ent_no; idx++) {
			bytesread = elf_filp->f_op->read(elf_filp, (char *)&isyms, sizeof (Elf32_Sym),
							&elf_filp->f_pos);
			if ((size_t)bytesread < sizeof (Elf32_Sym)) {
		...
		kdbg_elf_sym_buff = (kdbg_elf_kernel_symbol_item *)KDBG_MEM_DBG_VMALLOC
					(KDBG_MEM_ELF_MODULE, (size_t)symbol_count *
		...
		for (idx = 0; idx < symtab_ent_no; idx++) {
			...
			if ((ELF_ST_TYPE (isyms.st_info) == KDBG_ELF_STT_FUNC)
				 ...
				 * count */
				if (item_count >= (uint32_t)symbol_count)
					break;
			}
		}
		...
		if ((kdbg_elf_elf_load_option == KDBG_ELF_LOAD_DEFAULT && strstr(plist->elf_name_actual_path, "usb")) ||
			...
			if ((size_t)bytesread < sizeof(char)*dyn_str_size) {
				sym_errk("SectionHeader: %d read bytes out of required %u\n",
						bytesread,
						sizeof(char)*dyn_str_size);
				err = -EIO;
	...
	plist->elf_symbol_count = (int)item_count; /* actual items read */

DONE:

	sym_printk ("file loading is completed %s (%u functions)\n", filename, item_count);
#ifdef CONFIG_DWARF_MODULE
	if (strtab_buf) {
...
ERR_2:
	...
	if (plist->gnu_debug_buf) {
		KDBG_MEM_DBG_VFREE(plist->gnu_debug_buf);
		plist->gnu_debug_buf = NULL;
	}
#endif /* CONFIG_DWARF_MODULE */
...
(kdbg_elf_usb_elf_list_item *plist, unsigned int idx, char *elf_sym_buff, unsigned int sym_len)
{
	...
	if ((unsigned int)bytesread < total_bytes) {
...
DONE:
	BUG_ON(bytesread <= 0 || (unsigned int)bytesread > sym_len);
 ...
 */
int kdbg_elf_sym_find(unsigned int address,
		...
		unsigned int symbol_len,  struct aop_symbol_info *symbol_info)
{
	kdbg_elf_usb_elf_list_item *plist;
#ifdef CONFIG_DWARF_MODULE
	kdbg_elf_usb_elf_list_item *gnu_plist;
#endif
	struct list_head *pos;
	int retval;
...
#ifdef CONFIG_DWARF_MODULE
			if (!plist->dbg_info && plist->gnu_debug_buf) {
				list_for_each(pos, &kdbg_elf_usb_head_item)
				{
					gnu_plist = list_entry(pos, kdbg_elf_usb_elf_list_item, usb_elf_list);
					if (!gnu_plist) {
						sym_errk ("[%s] : pList is NULL\n", __FUNCTION__);
						return -1;
					}

					if (!strcmp(gnu_plist->elf_name, plist->gnu_debug_buf)) {
						sym_printk("ELF_Name - %s\n", gnu_plist->elf_name);
						sym_printk("%s - 0x%08x, %s - 0x%08x\n", plist->elf_name, plist->gnu_dbg_file_crc,
								gnu_plist->elf_name, gnu_plist->file_crc);
						if (plist->gnu_dbg_file_crc == gnu_plist->file_crc) {
							plist->gnu_link = (void *)gnu_plist;
							plist = gnu_plist;
							break;
						} else{
							sym_errk("the debug information found in \"%s\""
		   " does not match \"%s\" (CRC mismatch).\n",  gnu_plist->elf_name, plist->elf_name);
							return -1;
						}
					}
				}
...
static void swap_symbol_info(void *va, void *vb, int size)
{
	...
	memcpy(&tItem, a, (size_t)size);
	memcpy(a, b, (size_t)size);
	memcpy(b, &tItem, (size_t)size);
}

/* compare function for topthread_info using cpu_time */
static int cmp_symbol_info(const void *va, const void *vb)
{
	const kdbg_elf_kernel_symbol_item *a = (const kdbg_elf_kernel_symbol_item *)va, *b = (const kdbg_elf_kernel_symbol_item  *)vb;
...
{
	sym_printk("enter\n");
	sort(kdbg_elf_sym_buff, (size_t)symbol_count, sizeof(kdbg_elf_kernel_symbol_item), cmp_symbol_info, swap_symbol_info);
}
...
static void kdbg_elf_sym_index_heapSort(int *isyms, int sym_count)
{
	sort(isyms, (size_t)sym_count, sizeof(isyms[0]), kdbg_elf_cmp_name_index_info, kdbg_elf_swap_symbol_index);
}

static int kdbg_elf_bin_search(int *buff, int len, int value, int *pindex)
{
...
static int kdbg_elf_prune_symstr_for_func(kdbg_elf_usb_elf_list_item *plist)
{
	char *symstrbuf = NULL;
	int si = 0;
	...
	for (si = 0; si < plist->elf_symbol_count; si++, psyms++)
		if (psyms->st_value != (unsigned int)prev) {
			sym_printk("[Before] [%d <-- %d]:  prev = %08x,  (%08x <-- %08x)\n",
					last_index, si, prev, plist->kdbg_elf_sym_head[last_index].st_value,  psyms->st_value);
			plist->kdbg_elf_sym_head[last_index].st_value  = psyms->st_value;
			prev = (int)psyms->st_value;
	...
	g_arr = (int *)KDBG_MEM_DBG_KMALLOC
				(KDBG_MEM_ELF_MODULE, sizeof(int)*((size_t)plist->elf_symbol_count), GFP_KERNEL);
	if (!g_arr) {
	...
	p_arr = (int *)KDBG_MEM_DBG_KMALLOC
				(KDBG_MEM_ELF_MODULE, sizeof(int)*((size_t)last_index), GFP_KERNEL);
	if (!p_arr) {
	...
	for (si = 0; si < last_index; si++) {
		if ((uint32_t)g_arr[si] >= plist->sym_str_size) {
			p_arr[si] = corruptindex;
		} else{
			p_arr[si] = (int)newSymBuffSize;
	...
	for (si = 0; si < plist->elf_symbol_count; si++, psyms++) {
		ret = kdbg_elf_bin_search(g_arr, last_index, (int)(psyms->st_name), &new_index);
		BUG_ON(!ret);

		if ((uint32_t)p_arr[new_index] < newSymBuffSize) {
			size_t len = strlen(plist->sym_buff + psyms->st_name) + 1;
			if ((uint32_t)len >= (newSymBuffSize - (uint32_t)p_arr[new_index]))
				len = newSymBuffSize - (uint32_t)p_arr[new_index] - 1;
			strncpy(symstrbuf + p_arr[new_index], plist->sym_buff + psyms->st_name, len);
		}
		psyms->st_name = (unsigned int)p_arr[new_index];
	}
...
kdbg_elf_usb_elf_list_item * lookup_elf_file_in_db(char *elf_file)
{
	...
	list_for_each(usb_pos, &kdbg_elf_usb_head_item) {
		...
		if (!plist) {
			sym_errk("plist is NULL\n");
			sym_printk("%s:%u entry not found\n",__FILE__,__LINE__);
			return NULL;
		}
		...
		if (!strcmp(plist->elf_name, elf_name)) {
			...
			sym_printk("%s:%u file entry found\n",__FILE__,__LINE__);
			return plist;
		}
	}
...
kdbg_elf_usb_elf_list_item * load_elf_db_by_elf_file(char *elf_file, int elf_load_from_usb, int elf_match)
{
    ...
    sym_printk("%s:%u  plist added success fully\n",__FILE__,__LINE__);
...
static int kdbg_elf_load_from_usb(void)
{
	...
	for (index = 0; (index < usb_path_list.num_usb) && (index < KDBG_ELF_MAX_USB);
		...
		for (i = 0; i < dir_list.num_files; i++) {
			...
			PRINT_KD (KERN_INFO "[%s][%d] - File Name => %s\n", pfull_elf_name, __FILE__, __LINE__);
...
static void kdbg_elf_recscandir(const char *root_path)
{
	...
	int ii = 0;
	static struct kdbg_elf_dir_list dir_list;
	const size_t PATHLIST_ARRAY_SIZE = sizeof(char *) * KDBG_ELF_MAX_PATHLIST_ENTRIES;
	int fd = -1;
...
static void kdbg_elf_elf_loading_scenario(void)
{
	int operation = 0;
	...
	if (operation < KDBG_ELF_LOAD_DEFAULT ||
			operation > KDBG_ELF_LOAD_NO_ELF){
			PRINT_KD("Invalid Option \n");
			return;
	} else
		kdbg_elf_elf_load_option = operation;

	if (kdbg_elf_elf_load_option < KDBG_ELF_LOAD_DEFAULT ||
			kdbg_elf_elf_load_option > KDBG_ELF_LOAD_NO_ELF) {
		PRINT_KD ("Invalid option Loding default\n");
		kdbg_elf_elf_load_option = KDBG_ELF_LOAD_DEFAULT;
	}
}
#ifdef CONFIG_DWARF_MODULE
#if defined(KDBG_ELF_DEBUG_ON) && (KDBG_ELF_DEBUG_ON != 0)
static int kdbg_elf_get_word(struct file *fp, char *buf)
{
	char ch = 0;
	int i = 0;
	int ret = 0;
...
static unsigned int kdbg_elf_hex2dec(char *buf)
{
	char ch = 0;
	int digit = 0;
	unsigned value = 0;
 ...
 **********************************************/
static int kdbg_elf_sym_kdmenu(void)
{
	int operation = 0;
	static struct kdbg_elf_usb_path usb_path_list;
	int ret = 1;
	...
	do {
		if (ret) {
			...
			PRINT_KD (" 4 Search USB Mount paths\n");
			PRINT_KD (" 5 Load symbols with pid\n");
			PRINT_KD (" 6 Enable/ Disable ELF reading in file (for low-memory) for symbol name\n");
			PRINT_KD (" 7 ELF Loading options\n");
			PRINT_KD (" 8 Runtime Setting to Enable/ Disable ELF Module\n");
			PRINT_KD (" 9 Set ELF Update as Additive or Destructive(Default is Additive)\n");
#if defined(KDBG_ELF_DEBUG_ON) && (KDBG_ELF_DEBUG_ON != 0)
			PRINT_KD (" 10 List USB (aop_bin/) files\n");
