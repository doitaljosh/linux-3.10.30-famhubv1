...
#define WRITE_RAW_KLOG_VERSION 3

#define _WRITE_RAW_KLOG_PART(x) ("/dev/mmcblk0p" #x)
#define __WRITE_RAW_KLOG_PART(x) _WRITE_RAW_KLOG_PART(x)
#define WRITE_RAW_KLOG_PART \
	__WRITE_RAW_KLOG_PART(CONFIG_EMRG_SAVE_KLOG_PARTITION_N)

static void *emrg_klog_buf;
static int emrg_klog_buf_len;
static sector_t part_abs_offset;
...
void fill_raw_klog_head(struct write_raw_klog_head *head, u32 buf_len,
...
{
	return sdp_mmch_polling_request(mmc_host, part_abs_offset + offset,
			sectors_n,  data, true);
}
...
void write_emrg_klog(struct pt_regs *regs)
{
	u32 first, next;
	int ret;
	int sectors_to_write = (log_buf_len + 511) >> 9;
	unsigned long flags;
	__u32 klog_buf_crc;
	...
	if (emrg_klog_buf_len < log_buf_len) {
	...
	raw_spin_lock_irqsave(&logbuf_lock, flags);
	memcpy(emrg_klog_buf, log_buf, log_buf_len);
	next = log_next_idx;
	first = log_first_idx;
	raw_spin_unlock_irqrestore(&logbuf_lock, flags);

	/* Write the log buffer itself */
	ret = write_emrg_klog_data(emrg_klog_buf, sectors_to_write, 1);
	if (ret) {
		pr_err("can not write emrg klog data\n");
		return;
	}

	klog_buf_crc = crc32(0, emrg_klog_buf, log_buf_len);

	/* Since preallocated buffer already written, and no more needed
	 * it can be reused for klog head */
	fill_raw_klog_head(emrg_klog_buf, log_buf_len, first, next, regs,
			klog_buf_crc);

	/* And write the header */
	ret = write_emrg_klog_data(emrg_klog_buf, 1, 0);
	if (ret) {
		pr_err("can not write emrg klog data\n");
		return;
	}
...
__init void init_emrg_klog_save(void)
{
	 ...
	 */
	BUILD_BUG_ON(sizeof(struct write_raw_klog_head) != 33*4);
	BUILD_BUG_ON(sizeof(struct write_raw_klog_head) >
			WRITE_RAW_KLOG_HEAD_SIZE);

	bdev = blkdev_get_by_path(WRITE_RAW_KLOG_PART, mode,
			init_emrg_klog_save);
	...
	part_abs_offset = get_bdev_mmc_offset(bdev);
	mmc_host = get_queue_host((bdev));

	blkdev_put(bdev, mode);

	emrg_klog_buf = kmalloc(log_buf_len, GFP_KERNEL);
	if (!emrg_klog_buf) {
	...
	/* Just double check. Currently, it is impossible that log_buf_len
	 * going to be changed after boot. Just protection in case
	 * of future modifications */
	emrg_klog_buf_len = log_buf_len;
